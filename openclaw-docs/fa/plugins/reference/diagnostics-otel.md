---
read_when:
    - در حال نصب، پیکربندی یا ممیزی Plugin ‏diagnostics-otel هستید
summary: صادرکننده OpenTelemetry برای عیب‌یابی OpenClaw، جهت ارسال سنجه‌ها، ردگیری‌ها و گزارش‌ها.
title: Plugin تشخیص OpenTelemetry
x-i18n:
    generated_at: "2026-07-27T15:54:41Z"
    model: gpt-5.6
    postprocess_version: locale-links-v1
    prompt_version: 32
    provider: openai
    source_hash: 1fdbe6d3e52856e0db8e472aa2607622c60d3f0cadd5db0ae1eac5eedb2d4591
    source_path: plugins/reference/diagnostics-otel.md
    workflow: 16
---

# Plugin تشخیص OpenTelemetry

صادرکننده تشخیص OpenTelemetry برای معیارها، ردیابی‌ها و گزارش‌های OpenClaw.

## توزیع

- بسته: `@openclaw/diagnostics-otel`
- مسیر نصب: npm؛ ClawHub: `clawhub:@openclaw/diagnostics-otel`

## سطح

Plugin
