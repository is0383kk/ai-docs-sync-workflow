...
read_when:
    ...
    - OpenClaw را نصب کرده‌اید، اما `openclaw` با خطای «فرمان یافت نشد» مواجه می‌شود
    - نصب سراسری با `npm install -g` به‌دلیل مشکلات مجوز یا PATH ناموفق است
summary: نصب و پیکربندی Node.js برای OpenClaw — الزامات نسخه، گزینه‌های نصب و عیب‌یابی PATH
title: Node.js
x-i18n:
    generated_at: "2026-07-27T14:13:34Z"
    model: gpt-5.6
...
`v24.15.0` یا نسخه جدیدتر 24.x، پیش‌فرض توصیه‌شده است. `v22.22.3` یا نسخه جدیدتر 22.x، مسیر پشتیبانی‌شده Node 22 LTS است؛ Node `v25.9.0+` نیز پشتیبانی می‌شود. Node 23 پشتیبانی نمی‌شود. اگر Node نصب نیست یا نسخه آن خارج از محدوده پشتیبانی‌شده است، یکی از روش‌های نصب زیر را انتخاب کنید.
...
<Accordion title="استفاده از مدیر نسخه (nvm، fnm، mise، asdf)">
  مدیران نسخه امکان جابه‌جایی آسان بین نسخه‌های Node را فراهم می‌کنند. گزینه‌های محبوب:
...
این خطا تقریباً همیشه به این معنی است که پوشه bin سراسری npm در PATH شما قرار ندارد.

<Steps>
  <Step title="پیشوند سراسری npm را پیدا کنید">
    ```bash
    npm prefix -g
    ```
  </Step>
  <Step title="بررسی کنید که در PATH شما باشد">
    ```bash
    echo "$PATH"
    ```
  ...
  <Step title="آن را به فایل راه‌اندازی پوسته خود اضافه کنید">
    <Tabs>
...
اگر خطاهای `EACCES` را مشاهده می‌کنید، پیشوند سراسری npm را به پوشه‌ای تغییر دهید که کاربر اجازه نوشتن در آن را دارد:
