---
read_when:
    - استقرار OpenClaw در Northflank
    - استقرار ابری با یک کلیک و رابط کاربری کنترل مبتنی بر مرورگر می‌خواهید
summary: استقرار OpenClaw در Northflank با قالب یک‌کلیکی
title: Northflank
x-i18n:
    generated_at: "2026-07-27T14:16:45Z"
    model: gpt-5.6
    postprocess_version: locale-links-v1
    prompt_version: 32
    provider: openai
    source_hash: 16bb96fdf470999e15e163b6227d228ce8b60b9a172eb74cadc87bddd3955957
    source_path: install/northflank.mdx
    workflow: 16
---

OpenClaw را با یک قالب تک‌کلیکی روی Northflank مستقر کنید و از طریق رابط وب Control UI به آن دسترسی داشته باشید. این ساده‌ترین مسیر «بدون ترمینال روی سرور» است: Northflank، Gateway را برای شما اجرا می‌کند.

## نحوه شروع

1. برای باز کردن قالب، روی [استقرار OpenClaw](https://northflank.com/stacks/deploy-openclaw) کلیک کنید.
2. اگر از قبل حسابی ندارید، یک [حساب در Northflank](https://app.northflank.com/signup) ایجاد کنید.
3. روی **Deploy OpenClaw now** کلیک کنید.
4. متغیر محیطی الزامی را تنظیم کنید: `OPENCLAW_GATEWAY_TOKEN` (از یک مقدار تصادفی قوی استفاده کنید).
5. برای ساخت و اجرای قالب OpenClaw، روی **Deploy stack** کلیک کنید.
6. منتظر بمانید تا استقرار تکمیل شود، سپس روی **View resources** کلیک کنید.
7. سرویس OpenClaw را باز کنید.
8. نشانی عمومی OpenClaw را در `/openclaw` باز کنید و با استفاده از راز مشترک پیکربندی‌شده متصل شوید. این قالب به‌طور پیش‌فرض از `OPENCLAW_GATEWAY_TOKEN` استفاده می‌کند؛ اگر آن را با احراز هویت مبتنی بر گذرواژه جایگزین کردید، به‌جای آن از همان گذرواژه استفاده کنید.

## آنچه دریافت می‌کنید

- Gateway و Control UI میزبانی‌شده OpenClaw
- ذخیره‌سازی پایدار از طریق یک Volume در Northflank ‏(`/data`) تا `openclaw.json`، ‏`auth-profiles.json` مختص هر عامل، وضعیت کانال/ارائه‌دهنده، نشست‌ها و فضای کاری پس از استقرارهای مجدد حفظ شوند

## اتصال یک کانال

برای دستورالعمل‌های راه‌اندازی کانال، از Control UI در `/openclaw` استفاده کنید یا `openclaw onboard` را از طریق SSH اجرا کنید:

- [Telegram](/fa/channels/telegram) (سریع‌ترین گزینه، فقط یک توکن ربات)
- [Discord](/fa/channels/discord)
- [همه کانال‌ها](/fa/channels)

## گام‌های بعدی

- کانال‌های پیام‌رسانی را راه‌اندازی کنید: [کانال‌ها](/fa/channels)
- Gateway را پیکربندی کنید: [پیکربندی Gateway](/fa/gateway/configuration)
- OpenClaw را به‌روز نگه دارید: [به‌روزرسانی](/fa/install/updating)
