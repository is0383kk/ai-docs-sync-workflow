---
read_when:
    - استنتاج متمرکز بر حریم خصوصی را در OpenClaw می‌خواهید
    - راهنمای راه‌اندازی Venice AI را می‌خواهید
summary: استفاده از مدل‌های متمرکز بر حریم خصوصی Venice AI در OpenClaw
title: Venice AI
x-i18n:
    generated_at: "2026-07-27T17:04:19Z"
    model: gpt-5.6
...
بدون ثبت گزارش اجرا می‌شوند، به‌علاوه دسترسی پروکسی ناشناس‌شده به Claude، GPT، Gemini و Grok.
...
| حالت           | رفتار                                                         | مدل‌ها                                                        |
| -------------- | ---------------------------------------------------------------- | ------------------------------------------------------------- |
| **خصوصی**    | پرامپت‌ها/پاسخ‌ها هرگز ذخیره یا ثبت نمی‌شوند. موقتی است.         | Llama، Qwen، DeepSeek، Kimi، MiniMax، Venice Uncensored و غیره |
| **ناشناس‌شده** | پیش از ارسال، از طریق Venice پروکسی می‌شود و فراداده‌ها حذف می‌شوند. | Claude، GPT، Gemini، Grok                                     |

<Warning>
مدل‌های ناشناس‌شده کاملاً خصوصی نیستند. Venice پیش از ارسال، فراداده‌ها را حذف می‌کند، اما ارائه‌دهنده زیربنایی (OpenAI، Anthropic، Google، xAI) همچنان درخواست را پردازش می‌کند. هنگامی که حریم خصوصی کامل الزامی است، از مدل‌های خصوصی استفاده کنید.
</Warning>
...
<Steps>
  ...
  <Step title="پیکربندی OpenClaw">
    <Tabs>
      <Tab title="تعاملی (توصیه‌شده)">
        ...
        کلید API را درخواست می‌کند (یا از `VENICE_API_KEY` موجود دوباره استفاده می‌کند)، مدل‌های Venice در دسترس را فهرست می‌کند و مدل پیش‌فرض را تنظیم می‌کند.
      </Tab>
  ...
  <Step title="تأیید راه‌اندازی">
    ```bash
    openclaw agent --model venice/kimi-k2-5 --message "سلام، کار می‌کنی؟"
    ```
  </Step>
</Steps>
...
همچنین می‌توانید `openclaw configure` را اجرا کنید و **ارائه‌دهنده مدل/احراز هویت > Venice AI** را انتخاب کنید.

<Tip>
| کاربرد              | مدل                                        | دلیل                                    |
| --------------------- | -------------------------------------------- | -------------------------------------- |
| گفت‌وگوی عمومی (پیش‌فرض) | `kimi-k2-5`                                  | استدلال خصوصی قدرتمند به‌همراه بینایی   |
| بهترین کیفیت کلی   | `claude-opus-4-6`                            | قوی‌ترین گزینه ناشناس‌شده Venice     |
| حریم خصوصی + کدنویسی       | `qwen3-coder-480b-a35b-instruct-turbo`       | مدل کدنویسی خصوصی با زمینه بزرگ |
| سریع + ارزان           | `llama-3.2-3b`                               | مدل خصوصی کم‌حجم                  |
| وظایف خصوصی پیچیده  | `deepseek-v3.2`                              | استدلال قدرتمند؛ فراخوانی ابزار غیرفعال است |
| بدون سانسور             | `venice-uncensored-1-2`                      | مدل بدون سانسور فعلی Venice        |
</Tip>

## کاتالوگ داخلی (30 مدل)

<AccordionGroup>
  <Accordion title="مدل‌های خصوصی (20) — کاملاً خصوصی، بدون ثبت گزارش">
  ...
  <Accordion title="مدل‌های ناشناس‌شده (10) — از طریق پروکسی Venice">
    | شناسه مدل                        | نام                           | زمینه | یادداشت‌ها                      |
    | -------------------------------- | -------------------------------- | ------- | ---------------------------- |
...
مدل‌های Venice مبتنی بر Grok (`grok-4-3` و موارد مشابه) همان وصله سازگاری
طرح‌واره ابزار را که ارائه‌دهنده بومی xAI دارد دریافت می‌کنند، زیرا قالب فراخوانی ابزار
بالادستی یکسانی دارند.

## کشف مدل

کاتالوگ همراه بالا، فهرست اولیه‌ای مبتنی بر مانیفست است. OpenClaw هنگام اجرا
آن را از API ‏`/models` متعلق به Venice تازه‌سازی می‌کند و اگر
API در دسترس نباشد، به فهرست اولیه بازمی‌گردد. نقطه پایانی `/models` عمومی است (برای
فهرست‌کردن نیازی به احراز هویت نیست)، اما استنتاج به کلید API معتبر نیاز دارد.

ممکن است Venice همچنان شناسه‌های مدل بازنشسته را به‌عنوان نام‌های مستعار متعلق به ارائه‌دهنده بپذیرد.
کاتالوگ OpenClaw فقط شناسه‌های متعارف مدل را که `/models` برمی‌گرداند، اعلام می‌کند.

## رفتار بازپخش DeepSeek V4

اگر Venice مدل‌های DeepSeek V4 مانند `deepseek-v4-pro` یا
`deepseek-v4-flash` را ارائه کند، OpenClaw در صورتی که Venice فیلد بازپخش الزامی
`reasoning_content` را حذف کرده باشد، آن را در پیام‌های دستیار تکمیل می‌کند و `thinking`/
`reasoning`/`reasoning_effort` را از محموله درخواست حذف می‌کند (Venice کنترل بومی
`thinking` متعلق به DeepSeek را در این مدل‌ها رد می‌کند). این اصلاح بازپخش
از کنترل‌های تفکر خود ارائه‌دهنده بومی DeepSeek جداست.

## پشتیبانی از جریان و ابزار

| قابلیت          | پشتیبانی                                           |
| ---------------- | ------------------------------------------------- |
| جریان        | همه مدل‌ها                                        |
| فراخوانی تابع | بیشتر مدل‌ها؛ در موارد اشاره‌شده در بالا، برای هر مدل غیرفعال است |
...
قیمت‌گذاری مستقیم API به‌علاوه کارمزد اندکی برای Venice است. برای نرخ‌های فعلی،
...
```bash
# مدل خصوصی پیش‌فرض
openclaw agent --model venice/kimi-k2-5 --message "بررسی سریع سلامت"

# Claude Opus از طریق Venice (ناشناس‌شده)
openclaw agent --model venice/claude-opus-4-6 --message "این وظیفه را خلاصه کن"

# مدل بدون سانسور
openclaw agent --model venice/venice-uncensored-1-2 --message "گزینه‌ها را پیش‌نویس کن"

# مدل بینایی با تصویر
openclaw agent --model venice/qwen3-vl-235b-a22b --message "تصویر پیوست‌شده را بررسی کن"

# مدل کدنویسی
openclaw agent --model venice/qwen3-coder-480b-a35b-instruct-turbo --message "این تابع را بازآرایی کن"
```
...
<AccordionGroup>
  ...
  <Accordion title="مدل در دسترس نیست">
    برای دیدن مدل‌هایی که اکنون در دسترس‌اند، `openclaw models list --all --provider venice` را اجرا کنید؛
    با افزودن یا بازنشسته‌کردن مدل‌ها توسط Venice، کاتالوگ تغییر می‌کند.
  </Accordion>

  <Accordion title="مشکلات اتصال">
    API ‏Venice در `https://api.venice.ai/api/v1` قرار دارد. تأیید کنید که شبکه شما HTTPS به آن میزبان را مجاز می‌داند.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Venice AI" href="https://venice.ai" icon="globe">
    صفحهٔ اصلی Venice AI و ثبت‌نام حساب.
  </Card>
