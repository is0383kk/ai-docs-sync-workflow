...
read_when:
    ...
    - به ارجاع‌های مدل زمان اجرا برای مدل‌های میزبانی‌شده در Go نیاز دارید
summary: از کاتالوگ OpenCode Go با راه‌اندازی مشترک OpenCode استفاده کنید
title: OpenCode Go
x-i18n:
    generated_at: "2026-07-27T17:03:43Z"
    model: gpt-5.6
...
OpenCode Go کاتالوگ Go درون [OpenCode](/fa/providers/opencode) است. این کاتالوگ
اعتبارنامهٔ `OPENCODE_API_KEY` را با کاتالوگ Zen به‌اشتراک می‌گذارد، اما شناسهٔ
ارائه‌دهندهٔ زمان اجرای مستقل خود (`opencode-go`) را حفظ می‌کند تا مسیریابی بالادستی هر مدل
درست باقی بماند.
...
برای فهرست فعلی مدل‌ها، `openclaw models list --provider opencode-go` را اجرا کنید.
ردیف‌های همراه:

| مرجع مدل                       | نام              | زمینه   | حداکثر خروجی | ورودی تصویر |
| ------------------------------- | ----------------- | --------- | ---------- | ----------- |
...
| `opencode-go/hy3-preview`       | HY3 Preview       | 262,144   | 32,768     | خیر          |
...
<AccordionGroup>
  <Accordion title="رفتار مسیریابی">
    OpenClaw هر مرجع مدل `opencode-go/...` را به‌طور خودکار مسیریابی می‌کند. هیچ
    پیکربندی اضافی برای ارائه‌دهنده لازم نیست.
  </Accordion>

  <Accordion title="قرارداد مرجع زمان اجرا">
    مراجع زمان اجرا صریح باقی می‌مانند: `opencode/...` برای Zen و `opencode-go/...` برای
    Go. این کار مسیریابی بالادستی هر مدل را در هر دو کاتالوگ درست نگه می‌دارد.
  </Accordion>

  <Accordion title="اعتبارنامه‌های مشترک">
    یک `OPENCODE_API_KEY` هر دو کاتالوگ Zen و Go را پوشش می‌دهد. واردکردن
    کلید هنگام راه‌اندازی، اعتبارنامه‌ها را برای هر دو ارائه‌دهندهٔ زمان اجرا ذخیره می‌کند.
  </Accordion>
</AccordionGroup>

<Tip>
برای نمای کلی فرایند آغازین مشترک و مرجع کامل کاتالوگ
Zen + Go، به [OpenCode](/fa/providers/opencode) مراجعه کنید.
</Tip>
...
<CardGroup cols={2}>
  <Card title="OpenCode (والد)" href="/fa/providers/opencode" icon="server">
    فرایند آغازین مشترک، نمای کلی کاتالوگ و نکات پیشرفته.
  </Card>
