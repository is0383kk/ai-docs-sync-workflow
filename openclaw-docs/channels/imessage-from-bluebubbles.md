...
* A one-time per-`chat_id` `warn` the first time a specific group is dropped at runtime, naming the chat\_id and the exact key to add to `groups` to allow it.
...
| Catchup of inbound messages received while gateway is down | ✅ (webhook replay + history fetch) | *(not yet — tracked at [#78649](https://github.com/openclaw/openclaw/issues/78649))* |
