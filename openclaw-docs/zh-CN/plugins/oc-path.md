...
read_when:
    ...
    - 你正在编写针对工作区状态的脚本，需要一种稳定且与类型无关的寻址方案
    - 你正在决定是否要在自托管 Gateway 网关上启用可选的 `oc-path` 插件
summary: 内置 `oc-path` 插件：提供用于 `oc://` 工作区文件寻址方案的 `openclaw path` CLI
title: OC Path 插件
x-i18n:
    generated_at: "2026-07-26T06:52:26Z"
    model: gpt-5.6
...
内置的 `oc-path` 插件为 `oc://` 工作区文件寻址方案添加了 [`openclaw path`](/zh-CN/cli/path) CLI。它随 OpenClaw 仓库一起提供，位于
...
[CLI 参考](/zh-CN/cli/path)；本页介绍启用该插件的原因和方法。
