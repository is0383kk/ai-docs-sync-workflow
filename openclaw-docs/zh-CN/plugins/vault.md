---
read_when:
    - 你希望 OpenClaw 从 HashiCorp Vault 读取 API key
    - 你正在本地计算机或服务器上设置 SecretRefs
    - 你需要配置由 Vault 支持的模型提供商凭据
summary: 使用内置的 Vault 插件从 HashiCorp Vault 解析 SecretRef
title: Vault SecretRefs
x-i18n:
    generated_at: "2026-07-26T06:28:23Z"
    model: gpt-5.6
...
内置的 Vault 插件允许 OpenClaw 在 Gateway 网关启动和重新加载时从 HashiCorp Vault 解析 `exec` SecretRefs。OpenClaw 将 Vault 引用存储在配置中，将解析后的值保留在内存中的密钥快照中，并且不会将解析后的 API 密钥写回 `openclaw.json`。

如果你已经在运行 Vault，或希望将模型提供商密钥存放在 OpenClaw 配置文件之外，请使用此功能。有关 SecretRef 运行时模型，请参阅[密钥管理](/zh-CN/gateway/secrets)。

## 开始之前

你需要：

- 具有可用内置 `vault` 插件的 OpenClaw
- 可访问的 Vault 服务器
- 能够生成客户端令牌的 Vault 身份验证，该令牌对 OpenClaw 应解析的密钥路径具有读取权限
- 启动 Gateway 网关的环境必须包含 `VAULT_ADDR`，以及 `VAULT_TOKEN`、带有 `VAULT_TOKEN_FILE` 的 `OPENCLAW_VAULT_AUTH_METHOD=token_file`，或已配置的 JWT/Kubernetes 登录方式之一

解析器通过 Node 使用 HTTP 与 Vault 通信。Gateway 网关无需 Vault CLI 即可解析 SecretRefs。
...
OpenClaw 默认使用挂载在 `secret` 的 KV v2，这与 Vault 开发服务器示例一致。对于生产环境中的 Vault，请在创建 SecretRef ID 前，将 `OPENCLAW_VAULT_KV_MOUNT` 设置为实际的 KV 挂载路径。使用 OpenClaw 默认设置时，此 SecretRef ID：
...
如果 Vault Agent 写入令牌接收文件，请使用令牌文件身份验证：
...
JWT 文件应为投射的工作负载令牌，例如受 Vault 角色接受其受众的 Kubernetes 服务账号令牌。
交互式 OIDC 浏览器登录适合人类用户，但 Gateway 网关运行时需要非交互式 JWT 登录或令牌文件。

对于 Vault 的 Kubernetes 身份验证方法，请使用 `kubernetes`。此方法适用于以 Pod 形式运行的 Gateway 网关；默认挂载点为 `kubernetes`，默认 JWT 文件为标准服务账号令牌路径：
...
配置了多个由 Vault 支持的密钥提供商时，请按别名选择一个：
...
<Warning>
如果 Gateway 网关作为服务、LaunchAgent、systemd 单元、定时任务或容器运行，该运行时环境必须接收相同的 Vault 变量。仅在交互式 shell 中设置变量，只能证明该 shell 中存在这些变量，无法证明已在运行的 Gateway 网关中也存在。
</Warning>

## 生成并应用 SecretRef 计划

创建一个将 OpenRouter 的模型提供商 API 密钥映射到 Vault 的计划：
...
请使用 `--allow-exec`，因为 Vault 插件通过 OpenClaw 管理的 exec SecretRef 提供商进行解析。
...
对于没有快捷选项的内置提供商，或已配置的 OpenAI 兼容及自定义模型提供商，请使用 `--provider-key`：
...
每个 `--provider-key <provider=id>` 都会将 SecretRef 写入 `models.providers.<provider>.apiKey`。对于自定义提供商，它不会创建该提供商的 `baseUrl`、`api` 或 `models` 设置；请先配置这些设置。
...
不带限定符的目标路径应用于 `openclaw.json`。对现有 `auth-profiles.json` 目标使用 `auth-profiles:<agentId>:<path>`。目标路径必须是已注册的 OpenClaw SecretRef 目标。setup 命令不会在 OpenClaw 中创建任意命名的密钥；Vault 仍是密钥存储，而 OpenClaw 只在受支持的配置字段中存储 SecretRefs。
...
- 一种身份验证来源：
  ...
  - `OPENCLAW_VAULT_AUTH_METHOD=kubernetes` 加上 `OPENCLAW_VAULT_AUTH_ROLE`；可选择覆盖 `OPENCLAW_VAULT_AUTH_MOUNT` 或 `OPENCLAW_VAULT_JWT_FILE`
- 可选的 `VAULT_NAMESPACE`、`OPENCLAW_VAULT_KV_MOUNT` 和 `OPENCLAW_VAULT_KV_VERSION`

使用 Kubernetes 时，如果 Vault 已为集群配置 Kubernetes 身份验证，请优先使用 `OPENCLAW_VAULT_AUTH_METHOD=kubernetes`。仅当 Vault 配置为将集群视为通用 JWT/OIDC 颁发者时，才使用 `OPENCLAW_VAULT_AUTH_METHOD=jwt`。两种选择都优于在 Kubernetes Secret 中存放长期有效的 Vault 令牌。Vault Agent sidecar 或注入器部署可以改用 `token_file`。
