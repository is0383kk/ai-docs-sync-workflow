...
summary: Zalo Personal 插件：通过原生 zca-js 实现二维码登录和消息传递（插件安装 + 渠道配置 + 工具）
title: Zalo Personal 插件
x-i18n:
    generated_at: "2026-07-26T06:59:23Z"
    model: gpt-5.6
...
通过使用原生 `zca-js` 的插件为 OpenClaw 提供 Zalo Personal 支持，以自动操作普通 Zalo 用户账户。无需外部 `zca`/`openzca` CLI 二进制文件。

<Warning>
非官方自动化可能导致账户被暂停或封禁。使用风险自负。
</Warning>

## 命名

渠道 ID 为 `zalouser`，以明确表示它自动操作的是**个人 Zalo 用户账户**（非官方）。另一个 `zalo` 渠道 ID 对应官方内置的 Zalo Bot/webhook 集成——请参阅 [Zalo](/zh-CN/channels/zalo)。
...
有关私信/群组访问控制、多账户设置、环境变量和故障排查，请参阅 [Zalo 个人渠道配置](/zh-CN/channels/zalouser)。
...
openclaw directory groups list --channel zalouser --query "名称"
...
渠道消息操作（不是智能体工具）还支持使用 `react` 进行消息表情回应。
