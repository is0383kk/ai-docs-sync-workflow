...
read_when:
    ...
    - 你需要规范的内部与外部插件列表
summary: OpenClaw 内置发布、外部发布或仅保留源代码的插件清单（自动生成）
title: 插件清单
x-i18n:
    generated_at: "2026-07-26T05:54:47Z"
    model: gpt-5.6
...
本页面根据 `extensions/*/package.json`、`openclaw.plugin.json`
...
- **核心 npm 软件包：**内置于 `openclaw` npm 软件包中，无需单独安装插件即可使用。
- **官方外部软件包：**由 OpenClaw 维护、不包含在核心 npm 软件包中的插件，会保留在此官方清单中，并通过 ClawHub 和/或 npm 按需安装。
- **仅限源码检出：**仅存在于仓库本地、不包含在已发布 npm 工件中，也不作为可安装软件包推广的插件。

源码检出与 npm 安装不同：执行 `pnpm install` 后，内置
插件会从 `extensions/<id>` 加载，因此可以使用本地编辑内容和软件包本地的工作区
依赖项。

## 安装插件

根据每个条目中的安装方式判断是否需要安装。标有
`included in OpenClaw` 的插件已包含在核心软件包中。
官方外部软件包需要安装一次，然后重启 Gateway 网关。
...
在发布切换期间，普通的裸软件包说明符仍会从 npm 安装。
需要明确指定来源时，请使用 `clawhub:@openclaw/discord` 或 `npm:@openclaw/discord`。
安装后，请按照插件的设置文档（例如 [Discord](/zh-CN/channels/discord)）添加凭据和频道配置。有关
更新、卸载和发布命令，请参阅
...
- **[anthropic](/zh-CN/plugins/reference/anthropic)**（`@openclaw/anthropic-provider`）- 包含在 OpenClaw 中。提供 Anthropic 模型、Claude CLI 和原生 Claude 会话目录。
...
- **[bonjour](/zh-CN/plugins/reference/bonjour)**（`@openclaw/bonjour`）- 包含在 OpenClaw 中。通过 Bonjour/mDNS 通告本地 OpenClaw Gateway 网关。
...
- **[crabbox](/zh-CN/plugins/reference/crabbox)**（`@openclaw/crabbox-provider`）- 包含在 OpenClaw 中。由 Crabbox CLI 支持的云端工作节点提供商。

- **[cua-computer](/zh-CN/plugins/reference/cua-computer)**（`@openclaw/cua-computer`）- 包含在 OpenClaw 中。为 Windows 和 Linux 节点主机提供实验性 cua-driver 计算机控制。

- **[deepgram](/zh-CN/plugins/reference/deepgram)**（`@openclaw/deepgram-provider`）- 包含在 OpenClaw 中。添加媒体理解提供商支持。添加实时转录提供商支持。

- **[document-extract](/zh-CN/plugins/reference/document-extract)**（`@openclaw/document-extract-plugin`）- 包含在 OpenClaw 中。从本地文档附件中提取文本和作为回退的页面图像。
...
- **[file-transfer](/zh-CN/plugins/reference/file-transfer)**（`@openclaw/file-transfer`）- 包含在 OpenClaw 中。通过专用节点命令获取、列出和写入已配对节点上的文件。对于最大 16 MB 的二进制文件，通过 node.invoke 使用 base64，从而绕过 bash stdout 截断。
...
- **[imessage](/zh-CN/plugins/reference/imessage)**（`@openclaw/imessage`）- 包含在 OpenClaw 中。添加 iMessage 频道界面，用于发送和接收 OpenClaw 消息。

- **[linux-canvas](/zh-CN/plugins/reference/linux-canvas)**（`@openclaw/linux-canvas`）- 包含在 OpenClaw 中。用于 OpenClaw Linux 桌面应用的 Canvas 渲染桥接器。

- **[linux-node](/zh-CN/plugins/reference/linux-node)**（`@openclaw/linux-node`）- 包含在 OpenClaw 中。为 Linux 节点主机提供桌面通知、摄像头采集和位置功能。
...
- **[logbook](/zh-CN/plugins/reference/logbook)**（`@openclaw/logbook`）- 包含在 OpenClaw 中。自动工作日志：定期从已配对节点采集屏幕快照，并将其转换为可回顾的一日时间线。

- **[memory-core](/zh-CN/plugins/reference/memory-core)**（`@openclaw/memory-core`）- 包含在 OpenClaw 中。添加可由智能体调用的工具。

- **[memory-wiki](/zh-CN/plugins/reference/memory-wiki)**（`@openclaw/memory-wiki`）- 包含在 OpenClaw 中。适用于 OpenClaw 的持久化 wiki 编译器和兼容 Obsidian 的知识库。

- **[meta](/zh-CN/plugins/reference/meta)**（`@openclaw/meta-provider`）- 包含在 OpenClaw 中；npm；ClawHub：`clawhub:@openclaw/meta-provider`。为 OpenClaw 添加 Meta 模型提供商支持。
...
- **[migrate-claude](/zh-CN/plugins/reference/migrate-claude)**（`@openclaw/migrate-claude`）- 包含在 OpenClaw 中。将 Claude Code 和 Claude Desktop 的指令、MCP 服务器、Skills 及安全配置导入 OpenClaw。

- **[migrate-hermes](/zh-CN/plugins/reference/migrate-hermes)**（`@openclaw/migrate-hermes`）- 包含在 OpenClaw 中。将 Hermes 配置、记忆、Skills 和受支持的凭据导入 OpenClaw。
...
- **[oc-path](/zh-CN/plugins/reference/oc-path)**（`@openclaw/oc-path`）- 包含在 OpenClaw 中。添加 openclaw path CLI，用于通过 oc:// 对工作区文件进行寻址。

- **[ollama](/zh-CN/plugins/reference/ollama)**（`@openclaw/ollama-provider`）- 包含在 OpenClaw 中。为 OpenClaw 添加 Ollama、Ollama Cloud 模型提供商支持。

- **[onepassword](/zh-CN/plugins/reference/onepassword)**（`@openclaw/onepassword`）- 包含在 OpenClaw 中。精选的 1Password 机密代理，提供审批策略和 SQLite 审计历史记录。
...
- **[policy](/zh-CN/plugins/reference/policy)**（`@openclaw/policy`）- 包含在 OpenClaw 中。添加由策略支持的 Doctor 检查，以验证工作区的合规性。

- **[reef](/zh-CN/plugins/reference/reef)**（`@openclaw/reef`）- 包含在 OpenClaw 中。受保护的端到端加密 Claw 频道。
...
- **[teams-meetings](/zh-CN/plugins/reference/teams-meetings)**（`@openclaw/teams-meetings`）- 包含在 OpenClaw 中。以 Chrome 浏览器访客身份加入 Microsoft Teams 会议。

- **[telegram](/zh-CN/plugins/reference/telegram)**（`@openclaw/telegram`）- 包含在 OpenClaw 中。添加 Telegram 频道界面，用于发送和接收 OpenClaw 消息。
...
- **[vault](/zh-CN/plugins/reference/vault)** (`@openclaw/vault`) - OpenClaw 内置。HashiCorp Vault SecretRef 提供商集成。

- **[vllm](/zh-CN/plugins/reference/vllm)** (`@openclaw/vllm-provider`) - OpenClaw 内置。为 OpenClaw 添加 vLLM 模型提供商支持。

- **[volcengine](/zh-CN/plugins/reference/volcengine)** (`@openclaw/volcengine-provider`) - OpenClaw 内置。为 OpenClaw 添加 Volcengine、Volcengine Plan 模型提供商支持。

- **[voyage](/zh-CN/plugins/reference/voyage)** (`@openclaw/voyage-provider`) - OpenClaw 内置。添加记忆嵌入提供商支持。

- **[vydra](/zh-CN/plugins/reference/vydra)** (`@openclaw/vydra-provider`) - OpenClaw 内置。为 OpenClaw 添加 Vydra 模型提供商支持。

- **[web-readability](/zh-CN/plugins/reference/web-readability)** (`@openclaw/web-readability-plugin`) - OpenClaw 内置。从本地 HTML 网页抓取响应中提取可读的文章内容。

- **[webhooks](/zh-CN/plugins/reference/webhooks)** (`@openclaw/webhooks`) - OpenClaw 内置。经过身份验证的入站 Webhooks，可将外部自动化绑定到 OpenClaw TaskFlows。

- **[workboard](/zh-CN/plugins/reference/workboard)** (`@openclaw/workboard`) - OpenClaw 内置。用于管理由智能体负责的问题和会话的工作看板仪表盘。

- **[xai](/zh-CN/plugins/reference/xai)** (`@openclaw/xai-plugin`) - OpenClaw 内置。为 OpenClaw 添加 xAI 模型提供商支持。

- **[xiaomi](/zh-CN/plugins/reference/xiaomi)** (`@openclaw/xiaomi-provider`) - OpenClaw 内置。为 OpenClaw 添加 Xiaomi、Xiaomi Token Plan 模型提供商支持。

- **[zoom-meetings](/plugins/reference/zoom-meetings)** (`@openclaw/zoom-meetings`) - OpenClaw 内置。以 Chrome 浏览器访客身份加入 Zoom 会议。

## 官方外部软件包

72 个插件

- **[acpx](/zh-CN/plugins/reference/acpx)** (`@openclaw/acpx`) - npm；ClawHub。OpenClaw ACP 运行时后端，由插件管理会话和传输协议。
...
- **[baseten](/plugins/reference/baseten)** (`@openclaw/baseten-provider`) - npm；ClawHub：`clawhub:@openclaw/baseten-provider`。OpenClaw Baseten 提供商插件。
...
- **[codex](/zh-CN/plugins/reference/codex)** (`@openclaw/codex`) - npm；ClawHub。Codex app-server harness 和原生会话目录。
...
- **[diffs](/zh-CN/plugins/reference/diffs)** (`@openclaw/diffs`) - npm；ClawHub。OpenClaw 只读 Diffs 查看器插件和面向智能体的文件渲染器。
...
- **[discord](/zh-CN/plugins/reference/discord)** (`@openclaw/discord`) - npm；ClawHub。OpenClaw Discord 渠道插件，支持频道、私信、命令和应用事件。

- **[exa](/zh-CN/plugins/reference/exa)** (`@openclaw/exa-plugin`) - npm；ClawHub：`clawhub:@openclaw/exa-plugin`。添加 Web 搜索提供商支持。

- **[featherless](/zh-CN/plugins/reference/featherless)** (`@openclaw/featherless-provider`) - npm；ClawHub：`clawhub:@openclaw/featherless-provider`。OpenClaw Featherless AI 提供商插件。

- **[feishu](/zh-CN/plugins/reference/feishu)** (`@openclaw/feishu`) - npm；ClawHub。OpenClaw Feishu/Lark 渠道插件，用于聊天和工作场所工具（由 @m1heng 社区维护）。

- **[firecrawl](/zh-CN/plugins/reference/firecrawl)** (`@openclaw/firecrawl-plugin`) - npm；ClawHub：`clawhub:@openclaw/firecrawl-plugin`。添加智能体可调用的工具。添加网页抓取提供商支持。添加 Web 搜索提供商支持。
...
- **[google-meet](/zh-CN/plugins/reference/google-meet)** (`@openclaw/google-meet`) - npm；ClawHub。OpenClaw Google Meet 参与者插件，用于通过 Chrome 或 Twilio 传输协议加入通话。

- **[googlechat](/zh-CN/plugins/reference/googlechat)** (`@openclaw/googlechat`) - npm；ClawHub。OpenClaw Google Chat 渠道插件，支持聊天室和私信。
...
- **[llama-cpp](/zh-CN/plugins/reference/llama-cpp)** (`@openclaw/llama-cpp-provider`) - npm；ClawHub。通过 node-llama-cpp 进行本地 GGUF 文本推理和嵌入。
...
- **[memory-lancedb](/zh-CN/plugins/reference/memory-lancedb)** (`@openclaw/memory-lancedb`) - npm；ClawHub。由 LanceDB 支持的 OpenClaw 长期记忆插件，具备自动回忆、自动捕获和向量搜索功能。
...
- **[msteams](/zh-CN/plugins/reference/msteams)** (`@openclaw/msteams`) - npm；ClawHub。用于 Bot 对话的 OpenClaw Microsoft Teams 渠道插件。

- **[mxc](/zh-CN/plugins/reference/mxc)** (`@openclaw/mxc-sandbox`) - npm；ClawHub。通过 MXC 在操作系统级沙箱隔离环境中执行工具：使用已配置的 MXC 策略文件，在 Windows ProcessContainer 中运行命令。
...
- **[qwen](/zh-CN/plugins/reference/qwen)** (`@openclaw/qwen-provider`) - npm；ClawHub：`clawhub:@openclaw/qwen-provider`。为 OpenClaw 添加 Qwen、Qwen Cloud、Model Studio、DashScope、Qwen Token Plan、Bailian Token Plan 模型提供商支持。
...
- **[slack](/zh-CN/plugins/reference/slack)** (`@openclaw/slack`) - npm；ClawHub。OpenClaw Slack 渠道插件，支持频道、私信、命令和应用事件。
...
- **[synology-chat](/zh-CN/plugins/reference/synology-chat)** (`@openclaw/synology-chat`) - npm；ClawHub。用于 OpenClaw 渠道和私信的 Synology Chat 渠道插件。
...
- **[voice-call](/zh-CN/plugins/reference/voice-call)** (`@openclaw/voice-call`) - npm；ClawHub。用于 Twilio、Telnyx 和 Plivo 电话呼叫的 OpenClaw 语音通话插件。
...
- **[qa-lab](/zh-CN/plugins/reference/qa-lab)** (`@openclaw/qa-lab`) - 仅限源码检出。OpenClaw QA 实验室插件，提供私有调试器 UI 和场景运行器。
