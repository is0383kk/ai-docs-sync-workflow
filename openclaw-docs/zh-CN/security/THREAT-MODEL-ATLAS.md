...
x-i18n:
    generated_at: "2026-07-26T06:01:36Z"
    model: gpt-5.6
...
本威胁模型记录了针对 OpenClaw AI 智能体平台和 ClawHub 技能市场的对抗性威胁。它是一份由 OpenClaw 社区维护、持续更新的文档。有关如何报告新威胁、提出攻击链或建议缓解措施，请参阅[参与威胁模型贡献](/zh-CN/security/CONTRIBUTING-THREAT-MODEL)。

**关键 ATLAS 资源：** [技术](https://atlas.mitre.org/techniques/) | [战术](https://atlas.mitre.org/tactics/) | [案例研究](https://atlas.mitre.org/studies/) | [ATLAS GitHub](https://github.com/mitre-atlas/atlas-data) | [参与 ATLAS 贡献](https://atlas.mitre.org/resources/contribute)
...
范围外报告和误报模式（暴露于公共互联网、仅涉及提示词注入且未绕过边界的攻击链、互不信任的操作员共享同一 Gateway 网关主机，以及其他模式）列举在 [`SECURITY.md`](https://github.com/openclaw/openclaw/blob/main/SECURITY.md) 中；该文件是当前漏洞报告范围的权威来源，而不是本页面。
...
│  ┌──────────────────────────────────────────────────────────┐   │
│  │                      GATEWAY                              │   │
│  │  • 设备配对（私信配对 TTL 为 1h / 节点配对 TTL 为 5m）   │   │
│  │  • AllowFrom / 允许列表验证                               │   │
│  │  • 令牌 / 密码 / Tailscale 身份验证                      │   │
│  └──────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
...
│  ┌──────────────────────────────────────────────────────────┐   │
│  │                      CLAWHUB                              │   │
│  │  • 技能发布（语义化版本，必须提供 SKILL.md）             │   │
│  │  • 静态模式 + 邻近 AST 的审核扫描                        │   │
│  │  • 基于 LLM 的智能体式风险审查 + VirusTotal 扫描         │   │
│  │  • GitHub 账户年龄验证（14 天）                          │   │
│  └──────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────┘
```
...
| **描述**                | 攻击者在配对窗口期间截获配对码（私信/通用配对为 1h，节点配对为 5m）                                                 |
| **攻击向量**            | 肩窥、网络嗅探、社会工程                                                                                             |
| **受影响的组件**        | 设备配对系统                                                                                                         |
| **当前缓解措施**        | 1h TTL（私信/通用配对）、5m TTL（节点配对）；通过现有渠道发送配对码                                                 |
| **残余风险**            | 中 - 配对窗口可被利用                                                                                                |
| **建议**                | 缩短配对窗口，添加确认步骤                                                                                           |
...
| **描述**                | 攻击者发送精心构造的提示词来操纵智能体行为                                                                                                       |
| **攻击向量**            | 包含对抗性指令的渠道消息                                                                                                                         |
| **受影响的组件**        | 智能体 LLM、所有输入界面                                                                                                                         |
| **当前缓解措施**        | 模式检测、外部内容封装；若不存在边界绕过，则在漏洞报告中视为超出范围（参见 `SECURITY.md`）                                                  |
| **残余风险**            | 严重 - 仅检测而不阻止；复杂攻击可绕过                                                                                                            |
| **建议**                | 在现有检测之上，对敏感操作实施输出验证和用户确认                                                                                                 |
...
| **当前缓解措施**        | 使用随机边界 XML 风格标记封装内容、同形异义字符/特殊令牌规范化以及安全通知                                             |
| **残余风险**            | 高 - LLM 仍可能忽略封装指令                                                                                             |
| **建议**                | 为封装内容使用独立的执行上下文                                                                                          |
...
| **当前缓解措施**        | 允许列表 + 询问模式，并进行命令规范化（解包分派包装器、检测内联求值、分析 shell 命令链）                                                                                                  |
| **残余风险**            | 高 - 规范化缩小了混淆绕过的空间，但无法将其完全消除；Exec 路径之间仅涉及一致性的发现被视为加固问题，而非漏洞（参见 `SECURITY.md`）                                                   |
| **建议**                | 持续扩展命令规范化对新型混淆技术的覆盖                                                                                                                                                    |
...
| **受影响的组件**        | ClawHub、技能加载、智能体执行                                                                                             |
| **当前缓解措施**        | GitHub 账户年龄验证、静态模式/邻近 AST 扫描、基于 LLM 的智能体式风险审查、VirusTotal 扫描                                 |
| **残余风险**            | 高 - 已有多层检测，但技能仍以智能体权限运行，且没有执行沙箱隔离                                                          |
| **建议**                | 技能执行沙箱隔离、扩大社区审查                                                                                            |
...
| **ATLAS ID**            | AML.T0010.001 - 供应链入侵：AI 软件                                     |
| **描述**                | 攻击者入侵热门技能并推送恶意更新                                        |
| **攻击向量**            | 账户入侵、针对技能所有者的社会工程                                      |
| **受影响的组件**        | ClawHub 版本控制、自动更新流程                                           |
| **当前缓解措施**        | 版本指纹识别、新版本重新执行审核/扫描                                   |
| **残余风险**            | 高 - 自动更新可能在审查完成前拉取恶意版本                               |
| **建议**                | 更新签名、回滚能力、版本固定                                             |
...
| **受影响的组件**        | 外部内容包装                                                                                                  |
| **当前缓解措施**        | 随机边界的 XML 风格标记 + 安全提示，以及同形异义字符/空白变体标记伪造检测                                    |
| **剩余风险**            | 中 - 经常会发现新的逃逸方式                                                                                   |
...
| **描述**                | 攻击者指示智能体将数据发送至外部 URL，以此窃取数据                               |
| **攻击向量**            | 提示注入导致智能体向攻击者服务器 POST 数据                                       |
| **受影响的组件**        | `web_fetch` 工具                                                          |
| **当前缓解措施**        | 针对内部/私有网络的 SSRF 阻止机制（DNS 固定 + IP 阻止）                          |
| **剩余风险**            | 高 - 仍允许任意外部 URL                                                          |
| **建议**                | URL 允许列表、数据分类感知                                                       |
...
| **受影响的组件**        | 技能执行环境                                                                                                                                             |
| **当前缓解措施**        | ClawHub 凭据模式扫描（硬编码密钥、凭据环境访问与网络发送组合）；运行时不对技能执行进行沙箱隔离                                                          |
| **剩余风险**            | 严重 - 技能以智能体权限运行                                                                                                                              |
| **建议**                | 技能执行沙箱隔离、凭据隔离                                                                                                                               |
...
| **剩余风险**            | 严重 - 禁用沙箱时可能在主机上执行命令                                                               |
| **建议**                | 改进审批用户体验；关闭沙箱的部署仍是操作员有意作出的选择，并应如实记录在文档中                       |
...
| 控制措施                       | 实现方式                                                                                   | 有效性                                                         |
| ------------------------------ | ------------------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| GitHub 账户注册时长            | `requireGitHubAccountAge()`（最低 14 天）                                                           | 中等 - 提高新攻击者的门槛                                      |
| 路径清理                       | `sanitizePath()`                                                                         | 高 - 防止路径遍历                                              |
| 文件类型验证                   | `isTextFile()`                                                                         | 中等 - 仅扫描文本文件，但仍可被利用                            |
| 大小限制                       | 总包大小 50MB（`MAX_PUBLISH_TOTAL_BYTES`）                                                        | 高 - 防止资源耗尽                                              |
| 必须包含 SKILL.md              | 发布时必须提供自述文件                                                                     | 安全价值低 - 仅供参考                                          |
| 静态 + AST 邻近扫描            | 模式引擎覆盖命令执行、数据外泄、凭据窃取、混淆等行为                                       | 中高 - 覆盖许多已知滥用模式，但仍基于模式                      |
| 基于 LLM 的智能体风险审查      | 发布时依据安全提示词给出判定                                                               | 中高 - 能发现静态模式遗漏的行为                                |
| VirusTotal 扫描                | 已接入技能和软件包版本发布/重新扫描流程，由操作员 API key 控制启用                          | 启用时效果高 - 静态引擎检测                                    |
...
| R-006 | 改进命令执行审批用户体验并继续扩展命令规范化   | T-EXEC-004   |
...
| ID    | 建议                                   | 应对威胁      |
| ----- | -------------------------------------- | ------------- |
| R-008 | 在可行时添加加密渠道验证               | T-ACCESS-002  |
...
| `src/security/external-content.ts`                  | 提示词注入缓解措施         | **严重**   |
