...
`OPENCODE_ZEN_API_KEY`）。OpenClaw 将运行时提供商 ID 分开，以确保
上游按模型路由保持正确，但新手引导和文档将它们视为
同一个 OpenCode 设置。
...
运行 `openclaw models list --provider opencode` 可查看当前完整列表，其中
还包括 `opencode/big-pickle` 和
`opencode/deepseek-v4-flash-free` 等免费层级条目。
...
<AccordionGroup>
  ...
  <Accordion title="共享凭证">
    在设置期间输入一个 OpenCode 密钥，即会为两个运行时
    提供商存储凭证。无需分别为每个目录运行新手引导。
  </Accordion>
  ...
  <Accordion title="Gemini 重放行为">
    由 Gemini 支持的 OpenCode 引用继续使用代理 Gemini 路径，因此 OpenClaw 会在该路径
    保留 Gemini 思维签名清理，而不会启用原生 Gemini
    重放验证或引导重写。
  </Accordion>

  <Accordion title="非 Gemini 重放行为">
    非 Gemini OpenCode 引用保留最精简的 OpenAI 兼容重放策略。
  </Accordion>
</AccordionGroup>
