---
read_when:
    - 你想将 Together AI 与 OpenClaw 配合使用
    - 你需要 API key 环境变量或 CLI 身份验证选项
summary: Together AI 设置（身份验证 + 模型选择）
title: Together AI
x-i18n:
    generated_at: "2026-07-26T06:58:35Z"
    model: gpt-5.6
...
<Steps>
  <Step title="获取 API key">
    在
    [api.together.ai/settings/api-keys](https://api.together.ai/settings/api-keys) 创建 API key。
  </Step>
  <Step title="运行新手引导">
    ```bash
...
<Note>
新手引导会将 `together/meta-llama/Llama-3.3-70B-Instruct-Turbo` 设置为
默认模型。
</Note>
...
| 模型引用                                           | 名称                         | 输入        | 上下文  | 最大输出   | 费用（输入/输出） | 说明             |
| -------------------------------------------------- | ---------------------------- | ----------- | ------- | ---------- | ----------------- | ---------------- |
...
| `together/Qwen/Qwen2.5-7B-Instruct-Turbo`          | Qwen2.5 7B Instruct Turbo    | 文本        | 32,768  | 8,192      | 0.30 / 0.30       | 快速、非推理模型 |
...
| 时长                 | 1-10 秒                                                                                   |
| 支持的参数           | `size`（解析为 `<width>x<height>`）；不会读取 `aspectRatio`/`resolution` |

要将 Together 设置为默认视频提供商：
...
<AccordionGroup>
  <Accordion title="环境说明">
    ...
    `TOGETHER_API_KEY`（例如，将其放在
    `~/.openclaw/.env` 中，或通过 `env.shellEnv` 提供）。

    <Warning>
    仅在交互式 shell 中设置的密钥对守护进程管理的 Gateway 网关
    进程不可见。使用 `~/.openclaw/.env` 或 `env.shellEnv` 配置
    以确保其持续可用。
    </Warning>
  ...
  <Accordion title="故障排除">
    ...
    - 如果模型未显示，请确认 API key 已设置在 Gateway 网关进程的正确
      环境中。
