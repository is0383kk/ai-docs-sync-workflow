...
OpenClaw 内置了用于托管式视频生成的 `runway` 提供商，该提供商默认启用，并根据 `videoGenerationProviders` 合约注册。
...
<Steps>
  ...
  <Step title="生成视频">
    让智能体生成视频。系统将自动使用 Runway。
  </Step>
</Steps>

## 支持的模式和模型

该提供商提供七个 Runway 模型，分为三种模式。同一个模型 ID 可以支持多种模式（例如，`gen4.5` 同时支持文本生成视频和图像生成视频）。
...
<Warning>
  视频生成视频目前要求使用 `runway/gen4_aleph`。其他 Runway 模型 ID 会拒绝视频参考输入。
</Warning>

<Note>
  如果从错误的列中选择 Runway 模型 ID，系统会在 API 请求离开 OpenClaw 之前返回明确错误。该提供商会在 `extensions/runway/video-generation-provider.ts` 中根据相应模式的允许列表（`TEXT_ONLY_MODELS`、`IMAGE_MODELS`、`VIDEO_MODELS`）验证 `model`。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="任务轮询">
    ...
    会轮询 `GET /v1/tasks/{id}`，直到视频准备就绪。轮询行为
    无需额外配置。
  </Accordion>
</AccordionGroup>
