...
read_when:
    - 你需要 OpenCode Go 目录
    - 你需要 Go 托管模型的运行时模型引用
summary: 将 OpenCode Go 目录与共享 OpenCode 设置结合使用
title: OpenCode Go
x-i18n:
    generated_at: "2026-07-26T06:58:44Z"
    model: gpt-5.6
...
`OPENCODE_API_KEY` 凭据，但使用自己的运行时提供商 ID
（`opencode-go`），以确保上游按模型路由保持正确。
...
<Tip>
有关共享新手引导概览以及完整的 Zen + Go 目录参考，请参阅
