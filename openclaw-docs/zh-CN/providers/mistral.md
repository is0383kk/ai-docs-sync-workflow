...
summary: 通过 OpenClaw 使用 Mistral 模型和 Voxtral 转录
title: Mistral
x-i18n:
    generated_at: "2026-07-26T06:57:09Z"
    model: gpt-5.6
...
内置的 `mistral` 插件注册了四项契约：聊天补全、媒体理解（Voxtral 批量转录）、用于语音通话的实时 STT（Voxtral Realtime），以及记忆嵌入（`mistral-embed`）。
...
<Note>
OpenClaw 默认将 Mistral 实时 STT 设置为 8 kHz 的 `pcm_mulaw`，以便语音通话可以直接转发 Twilio 媒体帧。仅当上游流已经是原始 PCM 时，才使用 `encoding: "pcm_s16le"` 和匹配的 `sampleRate`。
</Note>

## 高级配置

<AccordionGroup>
  <Accordion title="可调节推理">
    `mistral/mistral-small-latest`、`mistral/mistral-small-2603` 和 `mistral/mistral-medium-3-5` 通过 `reasoning_effort` 在聊天补全 API 上支持[可调节推理](https://docs.mistral.ai/studio-api/conversations/reasoning/adjustable)（`none` 可尽量减少输出中的额外思考；`high` 会在最终答案前呈现完整的思考轨迹）。
    ...
    <Warning>
    避免将 Medium 3.5 推理模式与 `temperature: 0` 结合使用；据报告，Mistral HTTP API 会拒绝同时包含 `reasoning_effort="high"` 和 `temperature: 0` 的请求，并返回 400 响应。不要设置温度；或者先将思考设为关闭/最少，让 OpenClaw 发送 `reasoning_effort: "none"`，然后再设置较低的温度。
    </Warning>
    ...
    <Note>
    其他内置 Mistral 目录模型不使用此参数。如果需要 Mistral 原生的推理优先行为，请继续使用 `magistral-*` 模型。
    </Note>
