...
结构化媒体字段和 `[[...]]` 标签是交付元数据。`[embed ...]` 是单独的仅限 Web 的富渲染路径；它不是媒体别名。

## 媒体附件

远程附件必须是公开的 `https:` URL。`http:`、loopback、链路本地、私有和内部主机名会被拒绝作为附件指令；除此之外，服务端媒体获取器还会应用自身的网络防护措施。

本地附件接受绝对路径、工作区相对路径或主目录相对的 `~/` 路径。交付前，它们仍需通过智能体文件读取策略和媒体类型检查。
...
为保持兼容性，旧版最终回复文本仍可进行规范化，但这不是通用的插件/工具协议。
</Warning>

默认情况下，纯 Markdown 图片语法（`![alt](url)`）仍作为文本处理。若渠道希望将 Markdown 图片视为媒体回复，可在其出站适配器中选择启用；Telegram 已启用此行为，因此 `![alt](url)` 会成为媒体附件。

启用分块流式传输后，媒体必须通过结构化载荷字段传递。如果同一媒体 URL 既出现在流式传输块中，又再次出现在最终助手载荷中，OpenClaw 只会交付一次，并从最终载荷中移除重复项。
