...
title: 设备型号数据库
x-i18n:
    generated_at: "2026-07-26T06:22:16Z"
    model: gpt-5.6
...
从采用 MIT 许可证的 `kyle-seongwoo-jun/apple-device-identifiers` GitHub 仓库引入。JSON 文件固定到 `NOTICE.md` 中记录的提交 SHA，以确保构建具有确定性。
...
4. 确认 `LICENSE.apple-device-identifiers.txt` 仍与上游一致；如果上游许可证发生变化，请将其替换。
5. 验证 macOS 应用能够顺利构建：
