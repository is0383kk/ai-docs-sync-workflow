...
x-i18n:
    generated_at: "2026-07-26T06:49:19Z"
    model: gpt-5.6
...
<Note>
此内容现已移至[远程访问](/zh-CN/gateway/remote#macos-persistent-ssh-tunnel-via-launchagent)。请参阅该页面获取当前指南；本页面保留作为重定向目标。
</Note>
...
1. 添加包含 `LocalForward 18789 127.0.0.1:18789` 的 SSH 配置条目（完整配置块请参阅[远程访问](/zh-CN/gateway/remote#macos-persistent-ssh-tunnel-via-launchagent)）。
...
若要让隧道在重启后仍然运行并自动重新连接，请使用[远程访问](/zh-CN/gateway/remote#macos-persistent-ssh-tunnel-via-launchagent)页面上的 LaunchAgent 设置，而不是手动运行 `ssh -N`。
...
| `KeepAlive`                          | 隧道崩溃时自动重启（LaunchAgent） |
| `RunAtLoad`                          | LaunchAgent 加载时启动隧道（LaunchAgent）    |

OpenClaw.app 连接客户端上的 `ws://127.0.0.1:18789`。隧道会将该连接转发到运行 Gateway 网关的远程主机上的 18789 端口。
