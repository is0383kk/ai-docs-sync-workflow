...
title: macOS 开发环境设置
x-i18n:
    generated_at: "2026-07-26T06:53:27Z"
    model: gpt-5.6
...
- **Xcode 26.2+**（Swift 6.2 工具链），并使用“软件更新”中可用的最新 macOS。
- **Node.js 24.15+ 和 pnpm**，用于 Gateway 网关、CLI 和打包脚本。Node 22.22.3+ 也可用。
...
输出 `dist/OpenClaw.app`。如果没有 Apple Developer ID 证书，脚本会回退到临时签名。

有关开发运行模式、签名标志和 Team ID 故障排查，请参阅
[apps/macos/README.md](https://github.com/openclaw/openclaw/blob/main/apps/macos/README.md)。
从仓库根目录开始快速开发循环：`scripts/restart-mac.sh`（添加 `--no-sign` 可使用临时签名；使用 `--no-sign` 时，TCC 权限不会保留）。
...
打包的应用内嵌了规范的 `scripts/install-cli.sh` 安装程序。对于新配置文件，请在新手引导期间选择 **This Mac**；应用会先安装匹配的用户空间 CLI 和运行时，然后再启动 Gateway 网关向导。
...
`pnpm add -g openclaw@<version>` 和 `bun add -g openclaw@<version>` 也可用。对于 Gateway 网关本身，Node 仍是推荐的运行时。
