...
title: 重试策略
x-i18n:
    generated_at: "2026-07-26T05:47:05Z"
    model: gpt-5.6
...
- 对于 Anthropic 和 OpenAI 等基于 Stainless 的 SDK，可重试响应（`408`、`409`、`429` 和 `5xx`）可以包含 `retry-after-ms` 或 `retry-after`。当该等待时间超过 60 秒时，OpenClaw 会注入 `x-should-retry: false`，使 SDK 立即返回错误，以便模型故障转移能够切换到另一个身份验证配置文件或回退模型。
- 使用 `OPENCLAW_SDK_RETRY_MAX_WAIT_SECONDS=<seconds>` 覆盖此上限。将其设置为 `0`、`false`、`off`、`none` 或 `disabled`，以允许 SDK 在内部遵循较长的 `Retry-After` 休眠时间。

### Discord

- 遇到速率限制错误（HTTP 429）、请求超时、HTTP 5xx 响应，以及 DNS 查询失败、连接重置、套接字关闭和 fetch 失败等临时传输故障时进行重试。
