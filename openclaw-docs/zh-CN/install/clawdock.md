...
title: ClawDock
x-i18n:
    generated_at: "2026-07-26T06:17:24Z"
    model: gpt-5.6
...
ClawDock 是用于基于 Docker 的 OpenClaw 安装的一层轻量 shell 辅助工具。

它提供 `clawdock-start`、`clawdock-dashboard` 和 `clawdock-fix-token` 等简短命令，免去较长的 `docker compose ...` 调用。

如果尚未设置 Docker，请先参阅 [Docker](/zh-CN/install/docker)。
...
如果之前从 `scripts/shell-helpers/clawdock-helpers.sh` 安装了 ClawDock，请从当前的 `scripts/clawdock/clawdock-helpers.sh` 路径重新安装；旧的 GitHub raw 路径已被移除。

这些辅助工具会在首次使用时自动检测 OpenClaw 检出目录（检查 `~/openclaw`、`~/projects/openclaw` 等常用路径），并将结果缓存到 `~/.clawdock/config` 中。如果检出目录位于其他位置，请自行设置 `CLAWDOCK_DIR`。
...
| `clawdock-logs`    | 实时查看 Gateway 网关日志  |
...
| `clawdock-show-config` | 输出配置文件并隐去敏感值           |
...
- 位于 `docker-compose.yml` 旁的项目 `.env`：包含镜像名称、端口和 `OPENCLAW_GATEWAY_TOKEN` 等 Docker 专用值。`clawdock-token` 从这里读取令牌。
- `~/.openclaw/.env`（挂载到容器中）：OpenClaw 自身管理的由环境变量支持的密钥，以及 `openclaw.json` 和 `agents/<agentId>/agent/auth-profiles.json`。

`clawdock-fix-token` 会将令牌从项目 `.env` 复制到容器的 `gateway.remote.token` 和 `gateway.auth.token` 配置值中，然后重启 Gateway 网关。

使用 `clawdock-show-config` 可快速检查 `openclaw.json` 和两个 `.env` 文件；它会在输出内容中隐去 `.env` 的值。

## 相关内容

<CardGroup cols={2}>
  <Card title="Docker" href="/zh-CN/install/docker" icon="docker">
    OpenClaw 的标准 Docker 安装方式。
  </Card>
  <Card title="Docker VM 运行时" href="/zh-CN/install/docker-vm-runtime" icon="cube">
    由 Docker 管理的虚拟机运行时，用于强化隔离。
  </Card>
