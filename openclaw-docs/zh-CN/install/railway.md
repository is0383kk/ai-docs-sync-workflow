...
title: Railway
x-i18n:
    generated_at: "2026-07-26T06:46:12Z"
    model: gpt-5.6
...
使用一键式模板在 Railway 上部署 OpenClaw，并通过网页 Control UI 访问。这是最简单的“服务器上无需终端”方案：Railway 会为你运行 Gateway 网关。
...
<Step title="启用公共网络">
  ...
  <Step title="连接">
    在 **Railway -> your service -> Settings -> Domains** 中查找公共 URL——可以是生成的域名（通常为 `https://<something>.up.railway.app`），也可以是你附加的自定义域名。

    打开 `https://<your-railway-domain>/openclaw`，使用配置的共享密钥连接。模板默认使用 `OPENCLAW_GATEWAY_TOKEN`；如果将其替换为密码身份验证，请改用该密码。
...
- 通过 Railway Volume（`/data`）实现持久化存储，因此 `openclaw.json`、每个 Agent 的 `auth-profiles.json`、渠道/提供商状态、会话和工作区在重新部署后仍会保留

## 连接渠道

使用 `/openclaw` 上的 Control UI，或通过 Railway 的 shell 运行 `openclaw onboard`，以获取渠道设置说明：
...
导出状态、配置、身份验证配置文件和工作区：

```bash
openclaw backup create
```

此命令会创建一个可移植的备份归档，其中包含 OpenClaw 状态及所有已配置的工作区。有关详细信息，请参阅[备份](/zh-CN/cli/backup)。
