...
summary: 节点的位置命令、平台权限模式和 Linux GeoClue 设置
title: 位置命令
x-i18n:
    generated_at: "2026-07-26T06:50:05Z"
    model: gpt-5.6
...
操作系统的位置权限分为多个级别。精确位置也是一项单独的操作系统授权（iOS 14+ 中的“精确”，Android 中的“精确”与“粗略”）。应用内选择器控制请求的模式，但实际授予的权限仍由操作系统决定。
...
- 在 Android 第三方构建版本中选择 `always` 时，会先请求前台权限，说明后台访问权限，然后打开 Android 应用设置，以单独授予 **Allow all the time** 权限。
- Android Play 构建版本不声明后台位置权限，也不显示 `always`。
- 如果操作系统拒绝请求的权限级别，应用会恢复为已授予的最高级别并显示状态。
...
- 仅当用户选择 `Always` 且 Android 授予后台位置权限时，Android 第三方构建版本才会接受后台 `location.get`。现有的持久节点服务会添加 `location` 服务类型，并在活动期间披露 `Location: Always`。
- Android Play 构建版本和 `While Using` 模式会在应用处于后台时拒绝 `location.get`。
- 其他节点平台可能有所不同。

## Linux 节点主机

内置 Linux Node 插件会将 `location.get` 添加到 CLI `openclaw node` 服务，包括没有 Linux 桌面应用的无头主机。位置功能默认关闭。请在插件条目下启用它，然后重启节点服务：
...
该插件使用 `where-am-i`，而不是依次进行多次 `busctl` 调用。GeoClue 将客户端创建、属性、启动、更新和停止绑定到同一个 D-Bus 客户端连接；该演示程序会将整个生命周期保持在一起，而单独的 `busctl` 子进程则无法做到这一点。不会添加 npm 依赖项。

Linux 将 `coarse`、`balanced` 和 `precise` 映射到 GeoClue 精度级别 `4`、`6` 和 `8`。它会根据返回的时间戳验证 `maxAgeMs`。GeoClue 的演示程序不会公开所选提供商，因此 `source` 为 `unknown`；仅当报告的精度为 100 米或更高时，`isPrecise` 才为 true。
