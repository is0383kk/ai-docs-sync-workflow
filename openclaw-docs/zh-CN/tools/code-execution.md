...
（`https://api.x.ai/v1/responses` 使用的同一端点 `x_search`）。它由内置的 `xai` 插件根据 `tools` 契约注册。
...
可将其用于计算、制表、快速统计和图表式分析，包括分析由 `x_search` 或 `web_search` 返回的数据。它无法访问本地文件、你的 shell、你的仓库或已配对设备，并且不会在调用之间持久保存状态，因此应将每次调用视为临时分析，而不是笔记本会话。对于最新的 X 数据，请先运行 [`x_search`](/zh-CN/tools/web#x_search)，然后将结果传入。
...
<Steps>
  <Step title="提供 xAI 凭据">
    ...
    这三种方式中的任意一种也能为 `x_search` 和 Grok `web_search` 提供支持。

  </Step>

  <Step title="启用并调优 code_execution">
    省略 `enabled` 时，仅当当前模型的提供商为 `xai` 且能够解析 xAI 凭据时，才会公开 `code_execution`。对于提供商已知且不是 xAI 的当前模型，请将 `plugins.entries.xai.config.codeExecution.enabled` 设置为 `true`，以选择启用跨提供商使用。如果当前模型提供商缺失或无法解析，该工具将保持隐藏。将 `enabled` 设置为 `false` 可对所有提供商禁用它。始终需要 xAI 凭据。
      ...
      plugins: {
        entries: {
          xai: {
            config: {
              codeExecution: {
                enabled: true, // 对于提供商已知且不是 xAI 的模型，此项为必需
                model: "grok-4.3", // 覆盖默认的 xAI 代码执行模型
                maxTurns: 2,            // 内部工具轮次的可选上限
  ...
  <Step title="重启 Gateway 网关">
    ...
    xAI 插件重新注册，并且上述提供商、启用状态和身份验证检查均通过后，`code_execution` 将出现在智能体的工具列表中。
...
没有身份验证时，该工具会返回结构化 JSON 错误（而不是抛出的异常），以便智能体自行纠正：
