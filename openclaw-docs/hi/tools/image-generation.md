...
read_when:
    ...
    - image_generate टूल के पैरामीटर समझना
sidebarTitle: Image generation
summary: OpenAI, Google, fal, Microsoft Foundry, MiniMax, ComfyUI, DeepInfra, OpenRouter, LiteLLM, xAI, Vydra में image_generate के माध्यम से छवियाँ जनरेट और संपादित करें
title: छवि निर्माण
x-i18n:
    generated_at: "2026-07-27T18:42:34Z"
    model: gpt-5.6
...
<Steps>
  ...
  <Step title="एजेंट से कहें">
    _"एक मित्रवत रोबोट मैस्कॉट की इमेज बनाएँ।"_
...
| पारदर्शी-बैकग्राउंड वाली OpenAI PNG/WebP इमेज               | `openai/gpt-image-1.5`                             | `OPENAI_API_KEY` या OpenAI Codex OAuth |
...
| Google Gemini इमेज जनरेशन                       | `google/gemini-3.1-flash-image`                    | `GEMINI_API_KEY` या `GOOGLE_API_KEY`   |
...
| ----------------- | --------------------------------------- | ---------------------------------- | ----------------------------------------------------- |
| ComfyUI           | `workflow`                              | हाँ (1 इमेज, वर्कफ़्लो-कॉन्फ़िगर किया गया) | `COMFY_API_KEY` या क्लाउड के लिए `COMFY_CLOUD_API_KEY`    |
...
| Google            | `gemini-3.1-flash-image`                | हाँ (अधिकतम 5 इमेज)               | `GEMINI_API_KEY` या `GOOGLE_API_KEY`                  |
...
<ParamField path="aspectRatio" type="string">
  आस्पेक्ट रेशियो: `1:1`, `2:1`, `20:9`, `19.5:9`, `2:3`, `3:2`, `2.35:1`, `3:4`,
...
उम्मीदवार को स्वचालित रूप से आज़माया जाता है। यदि सभी विफल होते हैं, तो त्रुटि में
...
OpenAI, OpenRouter और Google `images` पैरामीटर के माध्यम से अधिकतम 5
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (और gpt-image-1.5)">
    ...
    `openai.background`, `transparent`, `opaque` या
  ...
  <Accordion title="Microsoft Foundry MAI इमेज मॉडल">
    ...
    - आउटपुट: एक PNG इमेज
    - आकार: डिफ़ॉल्ट `1024x1024`; चौड़ाई और ऊँचाई प्रत्येक कम-से-कम 768 px
      होनी चाहिए और कुल पिक्सेल अधिकतम 1,048,576 होने चाहिए
    - संपादन: एक PNG या JPEG संदर्भ इमेज, जो केवल
      `MAI-Image-2.5-Flash` और `MAI-Image-2.5` डिप्लॉयमेंट द्वारा समर्थित है
    ...
    के लिए [Microsoft Foundry Plugin](/hi/plugins/reference/microsoft-foundry) देखें।
  ...
  <Accordion title="fal Krea 2">
    ...
    फ़ोटोयथार्थवादी तथा टेक्सचरयुक्त रूप के लिए Krea 2 Large चुनें:
    ...
    ```json
    {
      ...
      "prompt": "रिसोग्राफ़ टेक्सचर वाला एक साइबर ज़ीन पोर्ट्रेट",
      "aspectRatio": "9:16",
      "fal": {
        "creativity": "high"
      }
...
```text
/tool image_generate action=generate model=openai/gpt-image-1.5 prompt="पारदर्शी पृष्ठभूमि पर लाल वृत्त का एक साधारण स्टिकर" outputFormat=png background=transparent
```
...
openclaw infer image generate \
  ...
  --prompt "पारदर्शी पृष्ठभूमि पर लाल वृत्त का एक साधारण स्टिकर" \
  --json
```
...
```text
/tool image_generate action=generate model=openai/gpt-image-2 prompt="विषय को बनाए रखें, पृष्ठभूमि को एक उज्ज्वल स्टूडियो सेटअप से बदलें" image=/path/to/reference.png size=1024x1536
```
  </Tab>
...
```text
/tool image_generate action=generate model=fal/krea/v2/medium/text-to-image prompt="इस रंग पैलेट और प्रिंट बनावट का उपयोग करने वाला एक अभिव्यंजक संपादकीय पोर्ट्रेट" images='["/path/to/palette.png","/path/to/texture.jpg"]' aspectRatio=9:16 fal='{"creativity":"high"}'
```
  </Tab>
</Tabs>

समान `--output-format`, `--background`, `--quality`, और
