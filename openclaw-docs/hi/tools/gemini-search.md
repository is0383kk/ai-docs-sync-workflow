...
title: Gemini खोज
x-i18n:
    generated_at: "2026-07-27T20:48:01Z"
    model: gpt-5.6
...
OpenClaw अंतर्निहित
[Google Search ग्राउंडिंग](https://ai.google.dev/gemini-api/docs/grounding) वाले Gemini मॉडल का समर्थन करता है,
जो उद्धरणों सहित लाइव Google Search परिणामों पर आधारित AI-संश्लेषित उत्तर
लौटाता है।
      ...
      google: {
        config: {
          webSearch: {
            ...
            baseUrl: "https://generativelanguage.googleapis.com/v1beta", // वैकल्पिक; उपलब्ध न होने पर models.providers.google.baseUrl का उपयोग करता है
...
**क्रेडेंशियल वरीयता:** Gemini वेब खोज पहले
`plugins.entries.google.config.webSearch.apiKey`, फिर `GEMINI_API_KEY`,
और फिर `models.providers.google.apiKey` का उपयोग करती है। आधार URL के लिए, समर्पित
`plugins.entries.google.config.webSearch.baseUrl` को
`models.providers.google.baseUrl` पर वरीयता मिलती है।
...
Gemini इनलाइन उद्धरणों सहित AI-संश्लेषित उत्तर तैयार करने के लिए Google Search ग्राउंडिंग
का उपयोग करता है। परिणामों में संश्लेषित उत्तर और स्रोत
URL, दोनों शामिल होते हैं।

- Gemini ग्राउंडिंग के उद्धरण URL, OpenClaw के SSRF-सुरक्षित
  फ़ेच पथ के माध्यम से HEAD अनुरोध द्वारा Google रीडायरेक्ट URL से सीधे URL में
  स्वतः हल किए जाते हैं (रीडायरेक्ट का अनुसरण, http/https सत्यापन)।
- रीडायरेक्ट समाधान सख्त SSRF डिफ़ॉल्ट का उपयोग करता है, इसलिए
  निजी/आंतरिक लक्ष्यों पर जाने वाले रीडायरेक्ट अवरुद्ध रहते हैं।
...
`pd`, `pw`, `pm`, और `py` स्वीकार करता है। `day`/`pd`, सख्त 24-घंटे की सीमा के बजाय Gemini
...
डिफ़ॉल्ट मॉडल `gemini-2.5-flash` (तेज़ और किफ़ायती) है। ग्राउंडिंग का समर्थन करने वाले किसी भी Gemini
मॉडल का उपयोग
`plugins.entries.google.config.webSearch.model` के माध्यम से किया जा सकता है।

## आधार URL ओवरराइड

जब Gemini वेब खोज को किसी ऑपरेटर प्रॉक्सी या कस्टम Gemini-संगत एंडपॉइंट से
रूट करना आवश्यक हो, तो `plugins.entries.google.config.webSearch.baseUrl` सेट करें। यदि
