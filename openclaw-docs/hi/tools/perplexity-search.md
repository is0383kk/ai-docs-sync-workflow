...
x-i18n:
    generated_at: "2026-07-27T18:47:26Z"
    model: gpt-5.6
...
OpenClaw, Perplexity Search API को `web_search` प्रदाता के रूप में समर्थन देता है। यह `title`, `url`, और `snippet` फ़ील्ड वाले संरचित परिणाम लौटाता है।
...
यदि आप Perplexity Sonar के लिए पहले से OpenRouter का उपयोग कर रहे थे, तो `provider: "perplexity"` बनाए रखें और Gateway परिवेश में `OPENROUTER_API_KEY` सेट करें, या `plugins.entries.perplexity.config.webSearch.apiKey` में कोई `sk-or-...` कुंजी संग्रहीत करें।
...
**कॉन्फ़िगरेशन के माध्यम से:** `openclaw configure --section web` चलाएँ। यह कुंजी को `plugins.entries.perplexity.config.webSearch.apiKey` के अंतर्गत `~/.openclaw/openclaw.json` में संग्रहीत करता है। यह फ़ील्ड SecretRef ऑब्जेक्ट भी स्वीकार करता है।
...
यदि `provider: "perplexity"` कॉन्फ़िगर किया गया है और Perplexity कुंजी का SecretRef बिना किसी परिवेश फ़ॉलबैक के अनिर्णीत है, तो स्टार्टअप/रीलोड तुरंत विफल हो जाता है।
...
2-अक्षर वाला ISO देश कोड (उदा. `US`, `DE`)।
</ParamField>
...
केवल इस तारीख (`YYYY-MM-DD`) के बाद प्रकाशित परिणाम।
</ParamField>

<ParamField path="date_before" type="string">
केवल इस तारीख (`YYYY-MM-DD`) से पहले प्रकाशित परिणाम।
</ParamField>

<ParamField path="domain_filter" type="string[]">
डोमेन अनुमति-सूची/अस्वीकृति-सूची ऐरे (अधिकतम 20)।
</ParamField>
...
- `count` वहाँ केवल संगतता के लिए है; प्रतिक्रिया फिर भी N-परिणाम सूची के बजाय उद्धरणों वाला एक संश्लेषित उत्तर होती है।
- केवल Search API वाले फ़िल्टर (`country`, `language`, `date_after`, `date_before`, `domain_filter`, `max_tokens`, `max_tokens_per_page`) स्पष्ट त्रुटियाँ लौटाते हैं।
...
// डोमेन फ़िल्टरिंग (अनुमति-सूची)
await web_search({
  query: "जलवायु अनुसंधान",
  domain_filter: ["nature.com", "science.org", ".edu"],
});

// डोमेन फ़िल्टरिंग (अस्वीकृति-सूची - आगे - लगाएँ)
await web_search({
  query: "उत्पाद समीक्षाएँ",
  domain_filter: ["-reddit.com", "-pinterest.com"],
});

// अधिक सामग्री निष्कर्षण
await web_search({
  query: "विस्तृत AI अनुसंधान",
  max_tokens: 50000,
...
- एक ही अनुरोध में अनुमति-सूची और अस्वीकृति-सूची प्रविष्टियाँ मिश्रित नहीं की जा सकतीं।
- अस्वीकृति-सूची प्रविष्टियों के लिए `-` उपसर्ग का उपयोग करें (उदा., `["-reddit.com"]`)।
...
<CardGroup cols={2}>
  ...
  <Card title="Perplexity Search API दस्तावेज़" href="https://docs.perplexity.ai/docs/search/quickstart" icon="arrow-up-right-from-square">
    आधिकारिक Perplexity Search API त्वरित शुरुआत और संदर्भ।
  </Card>
</CardGroup>
