...
summary: '`openclaw voicecall` के लिए CLI संदर्भ (वॉइस-कॉल Plugin कमांड सतह)'
title: वॉइसकॉल
x-i18n:
    generated_at: "2026-07-27T19:30:03Z"
    model: gpt-5.6
...
`voicecall` Plugin द्वारा प्रदान की गई कमांड है। यह केवल तभी दिखाई देती है, जब voice-call
Plugin इंस्टॉल और सक्षम हो।

जब Gateway चल रहा होता है, तब संचालन संबंधी कमांड (`call`, `start`,
`continue`, `speak`, `dtmf`, `end`, `status`) उस Gateway के
voice-call रनटाइम पर भेजी जाती हैं। यदि कोई Gateway उपलब्ध नहीं है, तो वे स्टैंडअलोन
CLI रनटाइम का उपयोग करती हैं।
...
| उपकमांड | विवरण                                                     |
| ---------- | --------------------------------------------------------------- |
| `setup`    | प्रदाता और Webhook की तत्परता जाँच दिखाएँ।                     |
| `smoke`    | तत्परता जाँच चलाएँ; केवल `--yes` के साथ लाइव परीक्षण कॉल करें। |
...
| `end`      | सक्रिय कॉल समाप्त करें।                                         |
| `status`   | सक्रिय कॉल की जाँच करें (या `--call-id` द्वारा किसी एक की)।                   |
| `tail`     | `calls.jsonl` को टेल करें (प्रदाता परीक्षणों के दौरान उपयोगी)।              |
| `latency`  | `calls.jsonl` से टर्न-विलंबता मेट्रिक्स का सारांश बनाएँ।              |
| `expose`   | Webhook एंडपॉइंट के लिए Tailscale serve/funnel को टॉगल करें।         |

## सेटअप और स्मोक परीक्षण

### `setup`

डिफ़ॉल्ट रूप से मानव-पठनीय तत्परता जाँच प्रिंट करता है। स्क्रिप्ट के लिए `--json` दें।
...
वही तत्परता जाँच चलाता है। वास्तविक फ़ोन कॉल केवल तभी करता है, जब
...
<Note>
बाहरी प्रदाताओं (`plivo`, `telnyx`, `twilio`) के लिए, `setup` और `smoke` को `publicUrl`, किसी टनल या Tailscale एक्सपोज़र से सार्वजनिक Webhook URL की आवश्यकता होती है। लूपबैक या निजी serve फ़ॉलबैक अस्वीकार कर दिया जाता है, क्योंकि कैरियर उस तक नहीं पहुँच सकते।
</Note>
...
| `-t, --to <phone>`     | नहीं       | कॉन्फ़िगरेशन `toNumber` | कॉल किया जाने वाला E.164 फ़ोन नंबर।                                                |
| `--mode <mode>`        | नहीं       | `conversation`    | कॉल मोड: `notify` (संदेश के बाद कॉल समाप्त करें) या `conversation` (कॉल जारी रखें)। |
...
| `--message <text>` | हाँ      | बोलने के लिए संदेश। |
...
नई पंक्तियाँ लिखे जाने पर उन्हें स्ट्रीम करता है।
...
`calls.jsonl` से टर्न-विलंबता और सुनने-की-प्रतीक्षा मेट्रिक्स का सारांश बनाएँ। आउटपुट
JSON है, जिसमें `recordsScanned`, `turnLatency`, और `listenWait` सारांश हैं।
...
## Webhook को सार्वजनिक करना

### `expose`

वॉइस Webhook के लिए Tailscale serve/funnel कॉन्फ़िगरेशन को सक्षम, अक्षम या
परिवर्तित करें।

| फ़्लैग                  | डिफ़ॉल्ट                                   | विवरण                                     |
| --------------------- | ----------------------------------------- | ----------------------------------------------- |
| `--mode <mode>`       | `funnel`                                  | `off`, `serve` (tailnet), या `funnel` (सार्वजनिक)। |
| `--path <path>`       | कॉन्फ़िगरेशन `tailscale.path` या `--serve-path` | एक्सपोज़ किया जाने वाला Tailscale पथ।                       |
| `--port <port>`       | कॉन्फ़िगरेशन `serve.port` या `3334`             | स्थानीय Webhook पोर्ट।                             |
| `--serve-path <path>` | कॉन्फ़िगरेशन `serve.path` या `/voice/webhook`   | स्थानीय Webhook पथ।                             |
...
<Warning>
Webhook एंडपॉइंट को केवल उन्हीं नेटवर्क पर सार्वजनिक करें जिन पर आप भरोसा करते हैं। जहाँ संभव हो, Funnel के बजाय Tailscale Serve को प्राथमिकता दें।
</Warning>
