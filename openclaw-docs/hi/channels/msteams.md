---
read_when:
    - Microsoft Teams चैनल सुविधाओं पर कार्य करना
summary: Microsoft Teams बॉट समर्थन की स्थिति, क्षमताएँ और कॉन्फ़िगरेशन
title: Microsoft Teams
x-i18n:
    generated_at: "2026-07-27T20:29:20Z"
    model: gpt-5.6
...
teams login
teams status   # पुष्टि करें कि आप लॉग इन हैं और अपने टेनेंट की जानकारी देखें
```
...
यदि आवश्यक हो, तो devtunnel CLI इंस्टॉल करें और प्रमाणित करें ([आरंभ करने की मार्गदर्शिका](https://learn.microsoft.com/en-us/azure/developer/dev-tunnels/get-started))।
...
<Note>
`--allow-anonymous` आवश्यक है, क्योंकि Teams devtunnels के साथ प्रमाणित नहीं कर सकता। आने वाले प्रत्येक बॉट अनुरोध को फिर भी Teams SDK द्वारा सत्यापित किया जाता है।
</Note>
...
<Note>
समूह चैट डिफ़ॉल्ट रूप से अवरुद्ध हैं (`channels.msteams.groupPolicy: "allowlist"`)। समूह उत्तरों की अनुमति देने के लिए `channels.msteams.groupAllowFrom` सेट करें, या किसी भी सदस्य को अनुमति देने के लिए `groupPolicy: "open"` उपयोग करें (मेंशन आवश्यक रहेगा)।
</Note>
...
- कुंजियों के रूप में Teams लिंक से प्राप्त स्थिर Teams वार्तालाप ID का उपयोग करें, बदल सकने वाले डिस्प्ले नामों का नहीं ([टीम और चैनल ID](#team-and-channel-ids-common-gotcha) देखें)।
...
<Warning>
नए मल्टी-टेनेंट बॉट बनाने की सुविधा 2025-07-31 के बाद अप्रचलित कर दी गई थी। नए बॉट के लिए **Single Tenant** उपयोग करें।
</Warning>
...
जब भी Graph क्रेडेंशियल कॉन्फ़िगर होते हैं, यह कार्रवाई उपलब्ध होती है; इसके लिए कोई अलग `channels.msteams.actions.memberInfo` टॉगल नहीं है।
...
निजी/साझा चैनल और गैर-वर्तमान चैट सदस्य लुकअप के लिए अतिरिक्त रोस्टर अनुमतियाँ आवश्यक हैं
और डिफ़ॉल्ट अनुमति आधाररेखा द्वारा अस्वीकार किए जाते हैं।
...
# सामग्री बदलने पर संस्करण अपने-आप बढ़ जाता है
```
...
2. **`version` फ़ील्ड बढ़ाएँ** (उदाहरण के लिए, `1.0.0` → `1.1.0`)।
...
Teams HTTP Webhook के माध्यम से संदेश भेजता है। OpenClaw उस Webhook लिसनर पर निश्चित HTTP सर्वर
टाइमआउट लागू करता है: निष्क्रियता के लिए 30s, कुल अनुरोध के लिए 30s और हेडर
प्राप्त करने के लिए 15s। वैकल्पिक इनबाउंड मीडिया और संदर्भ संवर्धन का साझा
बजट 10 सेकंड है। रॉ गतिविधि को स्थायी रूप से जोड़ दिए जाने के बाद SDK वापस लौटता है;
एजेंट टर्न स्वतंत्र रूप से समाप्त होता है और सक्रिय रूप से उत्तर भेजता है। यदि अनुरोध
संसाधन या स्थायी स्वीकृति ट्रांसपोर्ट समय-सीमा से चूक जाती है, तो Teams उस
गतिविधि को दोबारा भेज सकता है और इनग्रेस टूम्बस्टोन दोहराई गई इवेंट ID को अस्वीकार कर देता है।
...
Microsoft, Teams की सक्रिय संदेश सेवा के दस्तावेज़ों के [वार्तालाप बनाएँ](https://learn.microsoft.com/en-us/microsoftteams/platform/bots/how-to/conversations/send-proactive-messages?tabs=dotnet#create-the-conversation) अनुभाग में वैश्विक सक्रिय Bot Connector एंडपॉइंट प्रकाशित करता है। उपलब्ध होने पर आने वाली गतिविधि के `serviceUrl` का उपयोग करें; अन्यथा नीचे दी गई Microsoft की तालिका का उपयोग करें।
...
- बुनियादी फ़ॉर्मैटिंग काम करती है: **बोल्ड**, _इटैलिक_, `code`, लिंक।
...
- `channels.msteams.graphMediaFallback`: जब चैनल/समूह HTML में फ़ाइल मार्कर न हों, तब Graph संदेश लुकअप का विकल्प चुनें (डिफ़ॉल्ट `false`; [चैनल/समूह फ़ाइल पुनर्प्राप्ति](#channelgroup-file-recovery-graphmediafallback) देखें)।
...
Graph अनुमतियों के बिना, चित्रों वाले चैनल संदेश केवल टेक्स्ट के रूप में आते हैं (चित्र सामग्री बॉट के लिए सुलभ नहीं होती)।
...
Authorization हेडर केवल `channels.msteams.mediaAuthAllowHosts` में मौजूद होस्ट के लिए जोड़े जाते हैं (डिफ़ॉल्ट रूप से Graph + Bot Framework होस्ट)। इस सूची को सख्त रखें (मल्टी-टेनेंट प्रत्ययों से बचें)।
...
बॉट एक एप्लिकेशन पहचान का उपयोग करते हैं, जबकि Microsoft Graph के `/me` संसाधन के लिए [साइन-इन किया हुआ उपयोगकर्ता आवश्यक है](https://learn.microsoft.com/en-us/graph/api/user-get?view=graph-rest-1.0)। समूह चैट/चैनलों में फ़ाइलें भेजने के लिए, बॉट एक **SharePoint साइट** पर अपलोड करता है और साझाकरण लिंक बनाता है।
...
# प्रदर्शन नाम द्वारा किसी उपयोगकर्ता को भेजें (Graph API लुकअप ट्रिगर करता है)
...
<Note>
`user:` उपसर्ग के बिना, नाम डिफ़ॉल्ट रूप से समूह या टीम रिज़ॉल्यूशन का उपयोग करते हैं। प्रदर्शन नाम द्वारा लोगों को लक्षित करते समय हमेशा `user:` का उपयोग करें।
</Note>
...
https://teams.microsoft.com/l/team/19%3ABk4j...%40thread.tacv2/conversations?groupId=...
                                    └────────────────────────────┘
                                    टीम वार्तालाप ID (इसे URL-डिकोड करें)
```
...
- **अपलोड करते समय "Something went wrong":** इसके बजाय [https://admin.teams.microsoft.com](https://admin.teams.microsoft.com) के माध्यम से अपलोड करें, ब्राउज़र DevTools (F12) → Network tab खोलें और वास्तविक त्रुटि के लिए प्रतिक्रिया बॉडी जाँचें।
...
- [Azure Bot बनाएँ](https://learn.microsoft.com/en-us/azure/bot-service/bot-service-quickstart-registration) - Azure Bot सेटअप गाइड
...
- [RSC अनुमतियों का संदर्भ](https://learn.microsoft.com/en-us/microsoftteams/platform/graph-api/rsc/resource-specific-consent)
