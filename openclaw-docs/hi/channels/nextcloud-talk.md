---
read_when:
    - Nextcloud Talk चैनल की सुविधाओं पर कार्य करना
summary: Nextcloud Talk समर्थन की स्थिति, क्षमताएँ और कॉन्फ़िगरेशन
title: Nextcloud Talk
x-i18n:
    generated_at: "2026-07-27T17:22:01Z"
    model: gpt-5.6
...
वर्तमान आधिकारिक रिलीज़ टैग का अनुसरण करने के लिए केवल पैकेज विनिर्देश का उपयोग करें। सटीक संस्करण केवल तभी पिन करें, जब आपको पुनरुत्पाद्य इंस्टॉलेशन की आवश्यकता हो।
...
इंस्टॉल करने के बाद Gateway पुनः आरंभ करें। विवरण: [Plugins](/hi/tools/plugin)
...
2. अपने Nextcloud सर्वर पर एक बॉट बनाएँ:
   ...
   `--feature response` बनाए रखें: इसके बिना, आउटबाउंड उत्तर 401 के साथ विफल हो जाते हैं। किसी मौजूदा बॉट की मरम्मत `./occ talk:bot:state --feature webhook --feature response --feature reaction <botId> 1` से करें।
...
4. OpenClaw कॉन्फ़िगर करें:
   ...
   CLI सेटअप (`--url`/`--token` स्पष्ट फ़ील्ड के उपनाम हैं; `nc-talk` और `nc` चैनल उपनाम के रूप में काम करते हैं):
...
5. Gateway पुनः आरंभ करें (या सेटअप पूरा करें)।
...
- बॉट प्रत्यक्ष संदेश आरंभ नहीं कर सकते। उपयोगकर्ता को पहले बॉट को संदेश भेजना होगा।
- Webhook URL Nextcloud सर्वर से पहुँच योग्य होना चाहिए; जब Gateway किसी प्रॉक्सी के पीछे हो, तो `webhookPublicUrl` सेट करें। Webhook अनुरोधों पर बॉट सीक्रेट से HMAC-SHA256 हस्ताक्षर किए जाते हैं; अमान्य हस्ताक्षर अस्वीकार किए जाते हैं और उनकी दर सीमित की जाती है।
- बॉट API मीडिया अपलोड का समर्थन नहीं करता; आउटबाउंड मीडिया को `Attachment: <url>` पंक्ति के रूप में जोड़ा जाता है।
- Webhook पेलोड प्रत्यक्ष संदेशों और रूम के बीच अंतर नहीं करता; रूम-प्रकार की खोज सक्षम करने के लिए `apiUser` + `apiPassword` सेट करें (लगभग 5 मिनट तक कैश किया जाता है)। इनके बिना, प्रत्येक वार्तालाप को रूम माना जाता है।
- आउटबाउंड अनुरोध SSRF सुरक्षा से होकर जाते हैं। विश्वसनीय निजी/आंतरिक नेटवर्क पर स्थित Nextcloud होस्ट के लिए `channels.nextcloud-talk.network.dangerouslyAllowPrivateNetwork: true` से अनुमति दें।
...
- प्रति-रूम कुंजियाँ: `requireMention` (डिफ़ॉल्ट true), `enabled` (false रूम को अक्षम करता है), `allowFrom` (प्रति-रूम प्रेषक अनुमति-सूची), `tools` (टूल के अनुमति/अस्वीकृति ओवरराइड), `skills` (लोड किए गए Skills सीमित करें), `systemPrompt`।
...
- `channels.nextcloud-talk.botSecretFile`: सामान्य फ़ाइल वाले सीक्रेट का पथ। प्रतीकात्मक लिंक अस्वीकार किए जाते हैं।
- `channels.nextcloud-talk.apiUser`: रूम की खोज (प्रत्यक्ष संदेश पहचान) और स्थिति जाँच के लिए API उपयोगकर्ता।
- `channels.nextcloud-talk.apiPassword`: रूम की खोज के लिए API/ऐप पासवर्ड।
- `channels.nextcloud-talk.apiPasswordFile`: API पासवर्ड फ़ाइल पथ।
- `channels.nextcloud-talk.webhookPort`: Webhook श्रोता पोर्ट (डिफ़ॉल्ट: 8788)।
...
- `channels.nextcloud-talk.dmPolicy`: `pairing | allowlist | open | disabled` (डिफ़ॉल्ट: पेयरिंग)। `open` के लिए `allowFrom=["*"]` आवश्यक है।
- `channels.nextcloud-talk.allowFrom`: प्रत्यक्ष संदेश अनुमति-सूची (उपयोगकर्ता ID)।
- `channels.nextcloud-talk.groupPolicy`: `allowlist | open | disabled` (डिफ़ॉल्ट: अनुमति-सूची)।
- `channels.nextcloud-talk.groupAllowFrom`: रूम प्रेषक अनुमति-सूची (उपयोगकर्ता ID); सेट न होने पर `allowFrom` का उपयोग करता है।
- `channels.nextcloud-talk.rooms`: प्रति-रूम सेटिंग और अनुमति-सूची (ऊपर देखें)।
- स्थिर प्रेषक अभिगम समूहों को `allowFrom` और `groupAllowFrom` से `accessGroup:<name>` का उपयोग करके संदर्भित किया जा सकता है।
...
- `channels.nextcloud-talk.textChunkLimit`: आउटबाउंड टेक्स्ट खंड का अक्षर आकार (डिफ़ॉल्ट: 4000)।
- `channels.nextcloud-talk.streaming.chunkMode`: लंबाई के अनुसार खंड बनाने से पहले रिक्त पंक्तियों (अनुच्छेद सीमाओं) पर विभाजित करने के लिए `length` (डिफ़ॉल्ट) या `newline`।
- `channels.nextcloud-talk.streaming.block.enabled`: इस चैनल के लिए ब्लॉक स्ट्रीमिंग सक्षम या अक्षम करें।
- `channels.nextcloud-talk.streaming.block.coalesce`: ब्लॉक स्ट्रीमिंग संयोजन का समायोजन।
...
- `channels.nextcloud-talk.network.dangerouslyAllowPrivateNetwork`: निजी/आंतरिक Nextcloud होस्ट को SSRF सुरक्षा से आगे जाने की अनुमति दें।
