...
read_when:
    ...
    - आप Amazon Bedrock Mantle के माध्यम से Claude Opus 5, Sonnet 5 या Mythos 5 का उपयोग करना चाहते हैं
summary: OpenClaw के साथ Amazon Bedrock Mantle के OpenAI-संगत और Claude Messages मॉडल का उपयोग करें
title: Amazon Bedrock Mantle
x-i18n:
    generated_at: "2026-07-27T18:25:35Z"
    model: gpt-5.6
...
Mantle के OpenAI-संगत एंडपॉइंट से कनेक्ट होता है। Mantle, Bedrock अवसंरचना द्वारा समर्थित एक मानक
`/v1/chat/completions` सतह के माध्यम से ओपन-सोर्स और
...
| प्रमाणीकरण     | स्पष्ट `AWS_BEARER_TOKEN_BEDROCK` या IAM क्रेडेंशियल-चेन से बियरर-टोकन जनरेशन    |
...
<Tabs>
  <Tab title="स्पष्ट बियरर टोकन">
    **इनके लिए सर्वोत्तम:** ऐसे परिवेश जहाँ आपके पास पहले से Mantle बियरर टोकन है।

    <Steps>
      <Step title="Gateway होस्ट पर बियरर टोकन सेट करें">
        ```bash
      ...
      <Step title="सत्यापित करें कि मॉडल खोजे गए हैं">
        ...
        आप डिफ़ॉल्ट को ओवरराइड नहीं करना चाहते, किसी अतिरिक्त कॉन्फ़िगरेशन की आवश्यकता नहीं है।
      </Step>
    </Steps>
  ...
  <Tab title="IAM क्रेडेंशियल">
    **इनके लिए सर्वोत्तम:** AWS SDK-संगत क्रेडेंशियल (साझा कॉन्फ़िगरेशन, SSO, वेब पहचान, इंस्टेंस या टास्क भूमिकाएँ) का उपयोग करना।
    ...
    <Steps>
      ...
      <Step title="सत्यापित करें कि मॉडल खोजे गए हैं">
        ...
        OpenClaw क्रेडेंशियल चेन से स्वचालित रूप से Mantle बियरर टोकन जनरेट करता है।
      </Step>
    </Steps>

    <Tip>
    जब `AWS_BEARER_TOKEN_BEDROCK` सेट नहीं होता, तब OpenClaw AWS की डिफ़ॉल्ट क्रेडेंशियल चेन से आपके लिए बियरर टोकन बनाता है, जिसमें साझा क्रेडेंशियल/कॉन्फ़िगरेशन प्रोफ़ाइल, SSO, वेब पहचान और इंस्टेंस या टास्क भूमिकाएँ शामिल हैं।
    </Tip>
...
OpenClaw AWS की डिफ़ॉल्ट क्रेडेंशियल चेन से Mantle बियरर टोकन जनरेट करने का
प्रयास करता है। इसके बाद यह क्षेत्र के `/v1/models` एंडपॉइंट को क्वेरी करके
उपलब्ध Mantle मॉडल खोजता है।

| व्यवहार           | विवरण                                                                                |
| ----------------- | ------------------------------------------------------------------------------------ |
| खोज कैश           | परिणाम प्रत्येक क्षेत्र के लिए 1 घंटे तक कैश किए जाते हैं; फ़ेच विफल होने पर अंतिम कैश किया गया परिणाम लौटता है |
| IAM टोकन रीफ़्रेश | प्रत्येक 2 घंटे में, हर क्षेत्र के लिए कैश किया गया                                 |

Mantle plugin को सक्षम रखते हुए स्वचालित खोज और IAM
बियरर-टोकन जनरेशन को रोकने के लिए, plugin के स्वामित्व वाला खोज टॉगल अक्षम करें:
...
<Note>
यह बियरर टोकन वही `AWS_BEARER_TOKEN_BEDROCK` है जिसका उपयोग मानक [Amazon Bedrock](/hi/providers/bedrock) प्रदाता करता है।
</Note>
...
यदि आप स्वचालित खोज के बजाय स्पष्ट कॉन्फ़िगरेशन पसंद करते हैं:
...
एक स्पष्ट गैर-रिक्त `models` सूची आधिकारिक मानी जाती है और नीचे दी गई
Claude पंक्तियों सहित प्रत्येक खोजी गई पंक्ति को प्रतिस्थापित करती है। स्वचालित Mantle
कैटलॉग बनाए रखने के लिए `models` को छोड़ दें, या उन सभी Claude मॉडल प्रविष्टियों को शामिल करें
...
<AccordionGroup>
  ...
  <Accordion title="एंडपॉइंट की अनुपलब्धता">
    यदि Mantle एंडपॉइंट उपलब्ध नहीं है, कोई मॉडल नहीं लौटाता, या बियरर-टोकन
    समाधान विफल होता है, तो खोज एक रिक्त परिणाम लौटाती है और निहित
    प्रदाता को छोड़ दिया जाता है। OpenClaw कोई त्रुटि नहीं देता; अन्य कॉन्फ़िगर किए गए प्रदाता
    सामान्य रूप से काम करते रहते हैं।
  </Accordion>

  <Accordion title="Anthropic Messages रूट के माध्यम से Claude">
    जब स्वचालित खोज मॉडल सूची की स्वामी होती है, तो सफल लुकअप के बाद OpenClaw पाँच Claude
    मॉडल जोड़ता है, भले ही `/v1/models` कुछ भी लौटाए:
    ...
    Preview)। वे `anthropic-messages` API सतह का उपयोग करते हैं और
    उसी बियरर-प्रमाणीकृत Anthropic-संगत एंडपॉइंट
    (`<mantle-base>/anthropic`) के माध्यम से स्ट्रीम करते हैं, इसलिए AWS बियरर टोकन को
    Anthropic API कुंजी की तरह नहीं माना जाता।

    Claude Opus 5, 1,000,000-टोकन संदर्भ विंडो, 128,000-टोकन
    आउटपुट सीमा, इमेज इनपुट और `$5/$25` इनपुट/आउटपुट मूल्य प्रकाशित करता है। अनुकूली
    चिंतन डिफ़ॉल्ट रूप से `high` होता है; `/think off` चिंतन अक्षम करता है, और
    `/think xhigh|max` मॉडल के मूल प्रयास स्तरों का उपयोग करता है। OpenClaw
    कॉलर द्वारा चुने गए सैंपलिंग पैरामीटर छोड़ देता है।

    Claude Sonnet 5 हमेशा अनुकूली चिंतन का उपयोग करता है और डिफ़ॉल्ट रूप से `high`
    प्रयास का उपयोग करता है। `/think off` और `/think minimal`, `low` से मैप होते हैं क्योंकि Mantle
    रूट चिंतन को अक्षम नहीं कर सकता। OpenClaw, Sonnet 5 अनुरोधों के लिए
    कस्टम तापमान भी छोड़ देता है।

    Claude Mythos 5 की पहुँच सीमित है। यह 1,000,000-टोकन संदर्भ
    विंडो और 128,000-टोकन आउटपुट सीमा प्रकाशित करता है, हमेशा अनुकूली चिंतन का उपयोग करता है,
    `/think off` और `/think minimal` को `low` से मैप करता है, और कॉलर द्वारा चुने गए
    सैंपलिंग पैरामीटर छोड़ देता है।

    जब कोई `/think` स्तर सेट नहीं होता, तो Claude Mythos Preview हमेशा रीज़निंग का अनुरोध करता है और डिफ़ॉल्ट रूप से `high`
    प्रयास का उपयोग करता है (`xhigh`/`max` से घटाकर
    `high`, और `minimal` से बढ़ाकर `low` तक मैप किया जाता है)। Mantle पर Opus 4.7,
    ...
    सूची संपूर्ण खोजे गए कैटलॉग को प्रतिस्थापित करती है। जब आप इन अंतर्निहित Claude पंक्तियों को
    चाहते हैं, तो उस सूची को छोड़ दें।
