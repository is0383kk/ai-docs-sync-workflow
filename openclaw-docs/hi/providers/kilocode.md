...
summary: OpenClaw में कई मॉडल एक्सेस करने के लिए Kilo Gateway की एकीकृत API का उपयोग करें
title: Kilo Gateway
x-i18n:
    generated_at: "2026-07-27T20:26:17Z"
    model: gpt-5.6
...
Kilo Gateway एक ही OpenAI-संगत एंडपॉइंट और API कुंजी के पीछे कई मॉडल के लिए अनुरोध रूट करता है।
...
स्टार्टअप पर OpenClaw `GET https://api.kilo.ai/api/gateway/models` को क्वेरी करता है और खोजे गए मॉडलों को
एक स्थिर फ़ॉलबैक कैटलॉग से पहले मर्ज करता है। स्थिर फ़ॉलबैक में केवल
`kilocode/kilo-auto/balanced` (`Auto Balanced`, `input: ["text", "image"]`, `reasoning: true`,
`contextWindow: 1000000`, `maxTokens: 65536`) शामिल हैं।

Gateway पर किसी भी मॉडल को `kilocode/<upstream-id>` के रूप में संबोधित किया जा सकता है (उदाहरण के लिए
...
<AccordionGroup>
  <Accordion title="ट्रांसपोर्ट और संगतता">
    Kilo Gateway OpenRouter-संगत है, इसलिए यह मूल OpenAI अनुरोध संरचना के बजाय प्रॉक्सी-शैली के OpenAI-संगत अनुरोध
    पथ का उपयोग करता है (कोई `store` नहीं, कोई OpenAI रीजनिंग-एफ़र्ट पेलोड नहीं)।

    - Gemini-समर्थित Kilo रेफ़ प्रॉक्सी-Gemini पथ पर बने रहते हैं: OpenClaw वहाँ Gemini थॉट
      सिग्नेचर को सैनिटाइज़ करता है, लेकिन मूल Gemini रीप्ले सत्यापन या बूटस्ट्रैप रीराइट सक्षम नहीं करता।
    - अनुरोध आपकी API कुंजी से निर्मित Bearer टोकन का उपयोग करते हैं।

  </Accordion>

  <Accordion title="स्ट्रीम रैपर और रीजनिंग">
    Kilo स्ट्रीम रैपर एक `X-KILOCODE-FEATURE` अनुरोध हेडर जोड़ता है (डिफ़ॉल्ट `openclaw`,
    `KILOCODE_FEATURE` एनवायरनमेंट वेरिएबल से ओवरराइड करें) और इसका समर्थन करने वाले
    मॉडलों के लिए रीजनिंग-एफ़र्ट पेलोड को सामान्यीकृत करता है।

    <Warning>
    `kilocode/kilo-auto/balanced` और `x-ai/*` रेफ़ रीजनिंग-एफ़र्ट इंजेक्शन छोड़ देते हैं। यदि आपको रीजनिंग समर्थन चाहिए,
    तो `kilocode/anthropic/claude-sonnet-4` जैसे किसी ठोस मॉडल रेफ़ का उपयोग करें।
    </Warning>

  </Accordion>

  <Accordion title="समस्या निवारण">
    - यदि स्टार्टअप पर मॉडल खोज विफल होती है, तो OpenClaw `kilocode/kilo-auto/balanced` वाले स्थिर कैटलॉग पर वापस चला जाता है।
    - पुष्टि करें कि आपकी API कुंजी मान्य है और आपके Kilo खाते में वांछित मॉडल सक्षम हैं।
    - जब Gateway डेमन के रूप में चलता है, तो सुनिश्चित करें कि `KILOCODE_API_KEY` उस प्रक्रिया को उपलब्ध हो (उदाहरण के लिए `~/.openclaw/.env` में या `env.shellEnv` के माध्यम से)।
