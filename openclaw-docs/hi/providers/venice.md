...
read_when:
    ...
    - आप Venice AI सेटअप संबंधी मार्गदर्शन चाहते हैं
summary: OpenClaw में Venice AI के गोपनीयता-केंद्रित मॉडल का उपयोग करें
title: Venice AI
x-i18n:
    generated_at: "2026-07-27T20:29:06Z"
    model: gpt-5.6
...
[Venice AI](https://venice.ai) गोपनीयता-केंद्रित इन्फ़रेंस प्रदान करता है: खुले मॉडल
...
<Warning>
अनामित मॉडल पूरी तरह निजी नहीं हैं। Venice अग्रेषित करने से पहले मेटाडेटा हटा देता है, लेकिन अंतर्निहित प्रदाता (OpenAI, Anthropic, Google, xAI) फिर भी अनुरोध को संसाधित करता है। जब पूर्ण गोपनीयता आवश्यक हो, तब निजी मॉडल का उपयोग करें।
</Warning>
  ...
  <Step title="OpenClaw कॉन्फ़िगर करें">
    <Tabs>
      <Tab title="इंटरैक्टिव (अनुशंसित)">
        ...
        API कुंजी के लिए संकेत देता है (या मौजूदा `VENICE_API_KEY` का पुनः उपयोग करता है), उपलब्ध Venice मॉडल सूचीबद्ध करता है और आपका डिफ़ॉल्ट मॉडल सेट करता है।
      </Tab>
...
| --------------------- | -------------------------------------------- | -------------------------------------- |
| सामान्य चैट (डिफ़ॉल्ट) | `kimi-k2-5`                                  | शक्तिशाली निजी रीजनिंग और विज़न   |
...
| तेज़ + किफ़ायती           | `llama-3.2-3b`                               | कॉम्पैक्ट निजी मॉडल                  |
| जटिल निजी कार्य  | `deepseek-v3.2`                              | शक्तिशाली रीजनिंग; टूल कॉलिंग अक्षम |
| बिना सेंसर वाला             | `venice-uncensored-1-2`                      | वर्तमान बिना सेंसर वाला Venice मॉडल        |
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="अनामित मॉडल (10) — Venice प्रॉक्सी के माध्यम से">
    | मॉडल ID                        | नाम                           | कॉन्टेक्स्ट | टिप्पणियाँ                      |
    | -------------------------------- | -------------------------------- | ------- | ---------------------------- |
...
ऊपर दिया गया बंडल कैटलॉग एक मैनिफ़ेस्ट-समर्थित प्रारंभिक सूची है। रनटाइम पर OpenClaw
इसे Venice `/models` API से रीफ़्रेश करता है और यदि
API तक पहुँचा नहीं जा सकता, तो प्रारंभिक सूची का उपयोग करता है। `/models` एंडपॉइंट सार्वजनिक है (सूचीबद्ध करने के लिए प्रमाणीकरण आवश्यक नहीं),
लेकिन इन्फ़रेंस के लिए एक मान्य API कुंजी आवश्यक है।

Venice प्रदाता-स्वामित्व वाले उपनामों के रूप में सेवानिवृत्त मॉडल ID स्वीकार करना जारी रख सकता है।
OpenClaw कैटलॉग केवल `/models` द्वारा लौटाए गए कैननिकल मॉडल ID प्रदर्शित करता है।

## DeepSeek V4 रीप्ले व्यवहार

यदि Venice `deepseek-v4-pro` या
`deepseek-v4-flash` जैसे DeepSeek V4 मॉडल उपलब्ध कराता है, तो Venice द्वारा इसे छोड़े जाने पर OpenClaw सहायक संदेशों में आवश्यक `reasoning_content` रीप्ले
...
| विज़न/छवियाँ    | ऊपर "Vision" चिह्नित मॉडल                      |
| JSON मोड        | `response_format` के माध्यम से                             |

## मूल्य निर्धारण

Venice क्रेडिट-आधारित प्रणाली का उपयोग करता है। अनामित मॉडल की लागत लगभग
प्रत्यक्ष API मूल्य के बराबर होती है, जिसमें Venice का एक छोटा शुल्क जुड़ता है। वर्तमान दरों के लिए
...
openclaw agent --model venice/venice-uncensored-1-2 --message "विकल्पों का मसौदा बनाएँ"

# छवि वाला विज़न मॉडल
openclaw agent --model venice/qwen3-vl-235b-a22b --message "संलग्न छवि की समीक्षा करें"

# कोडिंग मॉडल
openclaw agent --model venice/qwen3-coder-480b-a35b-instruct-turbo --message "इस फ़ंक्शन को रिफ़ैक्टर करें"
```
...
<AccordionGroup>
  ...
  <Accordion title="मॉडल उपलब्ध नहीं है">
    ...
    Venice द्वारा मॉडल जोड़ने या सेवानिवृत्त करने पर कैटलॉग बदलता रहता है।
  </Accordion>

  <Accordion title="कनेक्शन संबंधी समस्याएँ">
    Venice API `https://api.venice.ai/api/v1` पर है। पुष्टि करें कि आपका नेटवर्क उस होस्ट पर HTTPS की अनुमति देता है।
  </Accordion>
</AccordionGroup>
