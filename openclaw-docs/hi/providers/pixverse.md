---
read_when:
    - आप OpenClaw में PixVerse वीडियो जेनरेशन का उपयोग करना चाहते हैं
...
title: PixVerse
x-i18n:
    generated_at: "2026-07-27T18:55:29Z"
    model: gpt-5.6
...
OpenClaw होस्ट किए गए PixVerse वीडियो निर्माण के लिए आधिकारिक बाहरी plugin के रूप में `pixverse` प्रदान करता है। यह plugin, `videoGenerationProviders` अनुबंध के साथ `pixverse` प्रदाता को पंजीकृत करता है।
...
<Steps>
  ...
  <Step title="API कुंजी सेट करें">
    ...
    प्रदाता कॉन्फ़िगरेशन में `region` और `baseUrl` लिखने से पहले विज़ार्ड अंतरराष्ट्रीय या CN एंडपॉइंट (नीचे API क्षेत्र देखें) पूछता है।
    गैर-इंटरैक्टिव रन (कुंजी `--pixverse-api-key` या `PIXVERSE_API_KEY` से)
    डिफ़ॉल्ट रूप से अंतरराष्ट्रीय का उपयोग करते हैं।

    यदि अभी तक कोई डिफ़ॉल्ट वीडियो मॉडल कॉन्फ़िगर नहीं है, तो ऑनबोर्डिंग
    `agents.defaults.mediaModels.video.primary` को `pixverse/v6` पर भी सेट करती है।
...
इमेज-से-वीडियो अनुरोध से पहले स्थानीय इमेज संदर्भ PixVerse पर अपलोड किए जाते हैं। रिमोट इमेज URL, PixVerse इमेज अपलोड एंडपॉइंट के माध्यम से `image_url` के रूप में भेजे जाते हैं।
...
<Note>
PixVerse इमेज टेम्पलेट निर्माण अभी `image_generate` के माध्यम से उपलब्ध नहीं है। वह API टेम्पलेट आईडी द्वारा संचालित है, जबकि OpenClaw के साझा इमेज-निर्माण अनुबंध में वर्तमान में PixVerse-विशिष्ट टाइप किया गया विकल्प समूह नहीं है।
</Note>

## प्रदाता विकल्प

वीडियो प्रदाता निम्न वैकल्पिक प्रदाता-विशिष्ट कुंजियाँ स्वीकार करता है:

| विकल्प                              | प्रकार  | प्रभाव                                         |
| ----------------------------------- | ------- | ---------------------------------------------- |
| `seed`                  | संख्या  | नियतात्मक सीड, 0 से 2147483647                 |
| `negativePrompt` / `negative_prompt` | स्ट्रिंग | नकारात्मक प्रॉम्प्ट                        |
| `quality`                  | स्ट्रिंग | PixVerse गुणवत्ता, जैसे `720p`    |
...
<AccordionGroup>
  <Accordion title="API क्षेत्र">
    ...
    कोई क्षेत्र चुनने के लिए `openclaw onboard --auth-choice pixverse-api-key` चलाएँ:
  ...
  <Accordion title="कार्य पोलिंग">
    PixVerse निर्माण अनुरोध से `video_id` लौटाता है। OpenClaw कार्य के सफल होने, विफल होने या समय-सीमा तक पहुँचने तक हर 5 सेकंड में
    `/openapi/v2/video/result/{video_id}` को पोल करता है (डिफ़ॉल्ट 5 मिनट; इसे
    `agents.defaults.mediaModels.video.timeoutMs` से ओवरराइड करें)।
  </Accordion>
</AccordionGroup>
