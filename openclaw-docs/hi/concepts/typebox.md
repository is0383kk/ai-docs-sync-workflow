...
title: TypeBox
x-i18n:
    generated_at: "2026-07-27T19:16:20Z"
    model: gpt-5.6
...
क्लाइंट                   Gateway
  |---- अनुरोध:कनेक्ट ------->|
  |<---- प्रतिक्रिया:हेलो-ओके --|
  |<---- इवेंट:टिक ------------|
  |---- अनुरोध:हेल्थ --------->|
  |<---- प्रतिक्रिया:हेल्थ -----|
```

सामान्य मेथड और इवेंट:

| श्रेणी      | उदाहरण                                                     | टिप्पणियाँ                                             |
| ----------- | ----------------------------------------------------------- | ------------------------------------------------------ |
| कोर         | `connect`, `health`, `status`                              | `connect` पहले होना चाहिए                     |
| मैसेजिंग    | `send`, `agent`, `agent.wait`, `system-event`, `logs.tail` | दुष्प्रभाव वाले मेथड को `idempotencyKey` चाहिए |
| चैट         | `chat.history`, `chat.send`, `chat.abort`                  | WebChat इनका उपयोग करता है                             |
| सेशन        | `sessions.list`, `sessions.patch`, `sessions.delete`       | सेशन प्रशासन                                           |
| ऑटोमेशन     | `wake`, `cron.list`, `cron.run`, `cron.runs`               | वेक और Cron नियंत्रण                                   |
| Node        | `node.list`, `node.invoke`, `node.pair.*`                  | Gateway WS के साथ Node क्रियाएँ                        |
