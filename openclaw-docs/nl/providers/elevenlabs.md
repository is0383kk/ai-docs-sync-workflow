...
summary: Gebruik spraak van ElevenLabs, Scribe STT en realtime transcriptie met OpenClaw
title: ElevenLabs
x-i18n:
    generated_at: "2026-07-27T06:31:06Z"
    model: gpt-5.6
...
v2 en streaming-STT met Scribe v2 Realtime. De Plugin is gebundeld en
standaard ingeschakeld; er is geen `plugins install`-stap nodig.

| Mogelijkheid                   | OpenClaw-oppervlak                                                   | Standaard                |
| ------------------------------ | -------------------------------------------------------------------- | ------------------------ |
| Tekst-naar-spraak              | `tts` / `talk`                              | `eleven_multilingual_v2`       |
...
de geselecteerde `voice.tts`/`tts`-provider is: het afspelen begint vanuit de
geretourneerde audiostream, in plaats van te wachten totdat OpenClaw eerst het volledige
audiobestand heeft gedownload. `latencyTier` wordt gekoppeld aan de `optimize_streaming_latency`-queryparameter
van ElevenLabs voor modellen die deze accepteren; OpenClaw laat die parameter weg voor
`eleven_v3`, omdat dit model de parameter weigert.
...
OpenClaw verzendt multipart-audio naar ElevenLabs `/v1/speech-to-text` met
`model_id: "scribe_v2"`. Taalaanwijzingen worden, indien aanwezig, gekoppeld aan `language_code`.

## Streaming-STT

De gebundelde `elevenlabs`-Plugin registreert Scribe v2 Realtime voor streamingtranscriptie
in Voice Call en de agentmodus van Google Meet.
...
| Samplesnelheid   | `...elevenlabs.sampleRate`                                                       | `8000`                                |
...
<Note>
Voice Call ontvangt Twilio-media als 8 kHz G.711 u-law. De realtimeprovider
van ElevenLabs gebruikt standaard `ulaw_8000`, zodat telefonieframes zonder
