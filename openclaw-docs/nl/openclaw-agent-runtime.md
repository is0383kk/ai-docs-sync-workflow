...
read_when:
    ...
    - Lint-, typecheck- en live testflows voor de agentruntime uitvoeren
summary: 'Ontwikkelaarsworkflow voor de OpenClaw-agentruntime: bouwen, testen en livevalideren'
title: Workflow van de OpenClaw-agentruntime
x-i18n:
    generated_at: "2026-07-27T05:54:41Z"
    model: gpt-5.6
...
Ontwikkelaarsworkflow voor de agentruntime (`src/agents/`) in de OpenClaw-repo.
...
De eerste glob omvat ook de suites `agent-tools*`, `agent-settings` en
...
- Gebruik de TUI voor interactieve foutopsporing: `pnpm tui`

Vraag voor het gedrag van toolaanroepen om een actie met `read` of `exec`, zodat je
toolstreaming en payloadverwerking kunt observeren.

## Volledige reset

De status bevindt zich in de OpenClaw-statusmap: standaard `~/.openclaw`, of
`$OPENCLAW_STATE_DIR` wanneer dit is ingesteld. Paden relatief ten opzichte van die map:

| Pad                                            | Bevat                                                              |
| ---------------------------------------------- | ------------------------------------------------------------------ |
| `openclaw.json`                                | Configuratie                                                        |
| `state/openclaw.sqlite`                        | Gedeelde database voor runtimestatus                                |
...
| `agents/<agentId>/sessions/`                   | Transcriptgeschiedenis en migratiebronnen voor verouderde sessies   |
...
Verwijder deze paden voor een volledige reset. Gerichtere resets:
