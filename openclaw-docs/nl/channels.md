...
read_when:
    ...
    - Je hebt een snel overzicht van ondersteunde berichtenplatforms nodig
summary: Berichtenplatforms waarmee OpenClaw verbinding kan maken
title: Chatkanalen
x-i18n:
    generated_at: "2026-07-27T05:33:34Z"
    model: gpt-5.6
...
iMessage, Telegram en de WebChat-UI worden meegeleverd met de kerninstallatie. Kanalen die zijn gemarkeerd als
...
opnieuw worden gestart. Kanalen met een "externe plugin" worden buiten de OpenClaw-repository onderhouden.
...
- [iMessage](/nl/channels/imessage) - Inbegrepen in de kern. Native macOS-integratie via de `imsg`-bridge op een aangemelde Mac (of een SSH-wrapper wanneer de Gateway elders draait), inclusief privé-API-acties voor antwoorden, tapbacks, effecten, bijlagen en groepsbeheer.
- [IRC](/nl/channels/irc) - Klassieke IRC-servers; kanalen + DM's met besturingselementen voor koppeling/toelatingslijsten (officiële plugin).
...
- [QQ Bot](/nl/channels/qqbot) - QQ Bot API; privéchat, groepschat en rijke media (officiële plugin).
- [Reef](/channels/reef) - Beveiligde, end-to-end versleutelde claw-naar-claw-berichtenuitwisseling tussen OpenClaw-agenten van verschillende personen (meegeleverde plugin).
- [Raft](/nl/channels/raft) - Raft CLI-wekbridge voor samenwerking tussen mensen en agenten (officiële plugin).
...
- [Telegram](/nl/channels/telegram) - Inbegrepen in de kern. Bot API via grammY; ondersteunt groepen.
- [Tlon](/nl/channels/tlon) - Op Urbit gebaseerde messenger (officiële plugin).
...
- [WebChat](/nl/web/webchat) - Inbegrepen in de kern. Gateway WebChat-UI via WebSocket.
...
- [Zalo](/nl/channels/zalo) - Zalo Bot API; populaire messenger in Vietnam (officiële plugin).
- [Zalo ClawBot](/nl/channels/zaloclawbot) - Persoonlijke Zalo-assistent via QR-aanmelding; gebonden aan de eigenaar (externe plugin).
...
- Telegram-antwoorden die Markdown-afbeeldingssyntaxis bevatten, zoals `![alt](url)`,
  worden waar mogelijk in het uiteindelijke uitgaande traject omgezet in media-antwoorden.
- DM's met meerdere personen in Slack worden als groepschats gerouteerd, waardoor het groepsbeleid, het gedrag
  rond vermeldingen en de regels voor groepssessies van toepassing zijn op MPIM-gesprekken.
- De installatie van WhatsApp gebeurt naar behoefte: tijdens de onboarding kan de configuratiestroom worden weergegeven voordat
...
- Kanalen die door bots geschreven inkomende berichten accepteren, kunnen gedeelde
  [beveiliging tegen botlussen](/nl/channels/bot-loop-protection) gebruiken om te voorkomen dat botparen
  elkaar onbeperkt blijven beantwoorden.
- Ondersteunde permanent actieve ruimtes kunnen [omgevingsgebeurtenissen voor ruimtes](/nl/channels/ambient-room-events)
  gebruiken, zodat gesprekken in de ruimte zonder vermelding stille context worden, tenzij de agent berichten verzendt met
  de tool `message`.
