...
read_when:
    ...
    - De parameters van de tool image_generate begrijpen
...
x-i18n:
    generated_at: "2026-07-27T05:19:06Z"
    model: gpt-5.6
...
beschikbaar is. Als je `image_generate` niet ziet tussen de tools van je agent,
...
<Steps>
  ...
  <Step title="Een standaardmodel kiezen (optioneel)">
    ...
    ChatGPT/Codex OAuth gebruikt dezelfde modelverwijzing `openai/gpt-image-2`. Wanneer een
...
<Warning>
Behoud voor OpenAI-compatibele LAN-eindpunten zoals LocalAI de aangepaste
`models.providers.openai.baseUrl` en meld je expliciet aan met
...
Gebruik `action: "status"` om de actieve taak voor het genereren van afbeeldingen voor de
...
<ParamField path="resolution" type='"1K" | "2K" | "4K"'>Resolutieaanwijzing.</ParamField>
...
<ParamField path="background" type='"transparent" | "opaque" | "auto"'>
  Achtergrondaanwijzing wanneer de provider deze ondersteunt. Gebruik `transparent` met
...
instellingen; `details.normalization` legt elke omzetting van aangevraagd naar toegepast
vast.
</Note>
...
Als een provider mislukt (authenticatiefout, snelheidslimiet enzovoort), wordt de volgende geconfigureerde
kandidaat automatisch geprobeerd. Als alle kandidaten mislukken, bevat de fout details
van elke poging.
...
<AccordionGroup>
  ...
  <Accordion title="Automatische detectie houdt rekening met authenticatie">
...
OpenAI, OpenRouter en Google ondersteunen maximaal 5 referentieafbeeldingen via de
...
<AccordionGroup>
  ...
  <Accordion title="Microsoft Foundry MAI-afbeeldingsmodellen">
  ...
  <Accordion title="fal Krea 2">
    ...
    voor tragere, gedetailleerdere fotorealistische en gestructureerde resultaten:
    ...
    ```json
    {
      ...
      "prompt": "Een cyberzineportret met risograaftextuur",
      "aspectRatio": "9:16",
      "fal": {
        "creativity": "high"
      }
  ...
  <Accordion title="Dubbele authenticatie van MiniMax">
...
openclaw infer image generate \
  ...
  --prompt "Een eenvoudige rode cirkelsticker op een transparante achtergrond" \
  --json
```

  </Tab>
  <Tab title="Genereren (lage OpenAI-kwaliteit)">
```text
...
openclaw infer image generate \
  ...
  --prompt "Voordelige conceptposter voor een rustige productiviteitsapp" \
  --json
```
...
- [ComfyUI](/nl/providers/comfy) - workflowconfiguratie voor lokale ComfyUI en Comfy Cloud
