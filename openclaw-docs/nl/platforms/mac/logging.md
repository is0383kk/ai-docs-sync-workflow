---
read_when:
    - macOS-logboeken vastleggen of de registratie van privégegevens onderzoeken
...
x-i18n:
    generated_at: "2026-07-27T05:21:30Z"
    model: gpt-5.6
...
## Roterend diagnostisch bestandslogboek (paneel Debug)
...
- Rotatie: roteert bij 5 MB; maximaal 5 back-ups met de achtervoegsels `.1`...`.5` (de oudste wordt verwijderd).
- Wissen: **Debug pane -> Logs -> App logging -> "Clear"** verwijdert het actieve bestand en alle back-ups.

Behandel het bestand als gevoelig; deel het niet zonder het te controleren.

## Privégegevens in uniforme logboekregistratie op macOS

Uniforme logboekregistratie redigeert de meeste payloads, tenzij een subsysteem zich aanmeldt voor `privacy -off`. Dit wordt geregeld door een plist in `/Library/Preferences/Logging/Subsystems/`, met de subsysteemnaam als sleutel. Alleen nieuwe logboekvermeldingen nemen de vlag over, dus schakel deze in voordat je een probleem reproduceert. Achtergrond: [eigenaardigheden rond privacy bij macOS-logboekregistratie](https://steipete.me/posts/2025/logging-privacy-shenanigans).
...
Opnieuw opstarten is niet nodig; logd neemt het bestand snel over, maar alleen nieuwe logboekregels bevatten privépayloads. Bekijk de uitgebreidere uitvoer met `./scripts/clawlog.sh --category WebChat --last 5m` (`--last`/`-l` stelt het tijdsbereik in, standaard `5m`; `--category`/`-c` filtert op categorie).
...
- Voer eventueel `sudo log config --reload` uit om logd de overschrijving onmiddellijk te laten verwijderen.
- Dit oppervlak kan telefoonnummers en berichtinhoud bevatten; laat de plist alleen staan zolang deze actief nodig is.
