...
title: Ansible
x-i18n:
    generated_at: "2026-07-27T05:02:14Z"
    model: gpt-5.6
...
<Info>
De [openclaw-ansible](https://github.com/openclaw/openclaw-ansible)-repository is de gezaghebbende bron voor implementatie met Ansible. Deze pagina biedt een kort overzicht.
</Info>
...
<Note>
De Gateway draait rechtstreeks op de host en niet in Docker. Agentsandboxing is
...
<Steps>
  <Step title="Overschakelen naar de openclaw-gebruiker">
    ```bash
    sudo -i -u openclaw
    ```
  </Step>
...
# Liveregelboeken bekijken
sudo journalctl -u openclaw -f

# Gateway opnieuw starten
sudo systemctl restart openclaw

# Aanmelden bij kanaal (uitvoeren als openclaw-gebruiker)
sudo -i -u openclaw
openclaw channels login --channel <name>
```
...
Alleen poort 22 (SSH) hoort open te zijn. Gateway en Docker blijven afgeschermd.

Docker wordt geïnstalleerd voor agentsandboxes (geïsoleerde uitvoering van tools), niet om de Gateway uit te voeren. Zie [Multi-agentsandbox en tools](/nl/tools/multi-agent-sandbox-tools) voor de sandboxconfiguratie.
...
<AccordionGroup>
  ...
  <Accordion title="Problemen met de Docker-sandbox">
    ...
    # De sandboximage bouwen als deze ontbreekt (vereist een broncodecheckout)
    cd /opt/openclaw/openclaw
    sudo -u openclaw ./scripts/sandbox-setup.sh
    # Zie voor npm-installaties zonder broncodecheckout:
...
- [Docker](/nl/install/docker): configuratie van een Gateway in een container
- [Sandboxing](/nl/gateway/sandboxing): configuratie van agentsandboxes
- [Multi-agentsandbox en tools](/nl/tools/multi-agent-sandbox-tools): isolatie per agent
