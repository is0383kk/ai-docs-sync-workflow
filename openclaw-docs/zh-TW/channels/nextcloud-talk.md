...
x-i18n:
    generated_at: "2026-07-26T07:08:45Z"
    model: gpt-5.6
...
Nextcloud Talk 是可下載的頻道外掛（`@openclaw/nextcloud-talk`），透過 Talk 網路鉤子機器人將 OpenClaw 連接至自行託管的 Nextcloud 執行個體。支援私訊、聊天室、回應和 Markdown 訊息；媒體會以 URL 形式傳送。
...
使用不含版本的套件規格，以跟隨目前的官方發行標籤。只有在需要可重現的安裝時，才固定確切版本。
...
2. 在你的 Nextcloud 伺服器上建立機器人：
   ...
   保留 `--feature response`：若沒有它，外送回覆會因 401 而失敗。使用 `./occ talk:bot:state --feature webhook --feature response --feature reaction <botId> 1` 修復現有的機器人。
...
- Nextcloud 伺服器必須能連線至網路鉤子 URL；當閘道位於 Proxy 後方時，請設定 `webhookPublicUrl`。網路鉤子要求會使用機器人密鑰進行 HMAC-SHA256 簽署；無效的簽章會遭到拒絕並受到速率限制。
- 機器人 API 不支援媒體上傳；外送媒體會附加為一行 `Attachment: <url>`。
- 網路鉤子承載資料不會區分私訊與聊天室；設定 `apiUser` + `apiPassword` 以啟用聊天室類型查詢（快取約 5 分鐘）。若未設定，每個對話都會被視為聊天室。
- 外送要求會通過 SSRF 防護。若 Nextcloud 主機位於受信任的私人／內部網路，請使用 `channels.nextcloud-talk.network.dangerouslyAllowPrivateNetwork: true` 選擇允許。
...
- 預設值：`channels.nextcloud-talk.groupPolicy = "allowlist"`（須提及才會觸發）。
- 使用 `channels.nextcloud-talk.rooms` 允許聊天室，並以聊天室權杖為鍵；`"*"` 設定萬用字元預設值：
...
- 各聊天室的鍵：`requireMention`（預設為 true）、`enabled`（設為 false 會停用聊天室）、`allowFrom`（各聊天室的傳送者允許清單）、`tools`（允許／拒絕工具覆寫）、`skills`（限制載入的 Skills）、`systemPrompt`。
- 若要不允許任何聊天室，請讓允許清單保持空白，或設定 `channels.nextcloud-talk.groupPolicy="disabled"`。
...
- `channels.nextcloud-talk.botSecretFile`：一般檔案的密鑰路徑。符號連結會遭到拒絕。
...
- `channels.nextcloud-talk.groupAllowFrom`：聊天室傳送者允許清單（使用者 ID）；未設定時會回退至 `allowFrom`。
- `channels.nextcloud-talk.rooms`：各聊天室設定和允許清單（見上文）。
...
- `channels.nextcloud-talk.dms`：以使用者 ID 為鍵的各私訊覆寫（`historyLimit`）。
- `channels.nextcloud-talk.textChunkLimit`：外送文字分塊的字元數（預設值：4000）。
- `channels.nextcloud-talk.streaming.chunkMode`：`length`（預設值），或使用 `newline` 在依長度分塊前，先按空白行（段落邊界）分割。
- `channels.nextcloud-talk.streaming.block.enabled`：啟用或停用此頻道的區塊串流。
- `channels.nextcloud-talk.streaming.block.coalesce`：區塊串流合併調整。
- `channels.nextcloud-talk.responsePrefix`：外送回覆前綴。
...
- `channels.nextcloud-talk.accounts.<id>`：各帳號覆寫（相同的鍵）；`defaultAccount` 選擇預設帳號。環境變數 `NEXTCLOUD_TALK_BOT_SECRET` / `NEXTCLOUD_TALK_API_PASSWORD` 僅套用於預設帳號。

## 相關內容

- [頻道概覽](/zh-TW/channels) — 所有支援的頻道
- [配對](/zh-TW/channels/pairing) — 私訊驗證和配對流程
- [群組](/zh-TW/channels/groups) — 群組聊天行為和提及觸發機制
