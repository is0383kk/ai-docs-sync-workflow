...
title: MiniMax
x-i18n:
    generated_at: "2026-07-26T08:03:17Z"
    model: gpt-5.6
...
<Tip>
MiniMax Coding Plan 推薦連結（享 9 折優惠）：[MiniMax Coding Plan](https://platform.minimax.io/subscribe/coding-plan?code=DbXJTRClnb&source=link)
</Tip>
...
| `image-01`               | 圖片生成 | 文字轉圖片及圖片轉圖片編輯 |
| `music-2.6`              | 音樂生成 | 預設音樂模型                      |
| `MiniMax-Hailuo-2.3`     | 影片生成 | 文字轉影片及圖片轉影片流程   |
...
<Tabs>
  ...
  <Tab title="API 金鑰">
    **最適合：**搭配 Anthropic 相容 API 的託管 MiniMax。
    ...
    <Warning>
    MiniMax-M2.x 的 Anthropic 相容串流端點會在 OpenAI 樣式的差異區塊中發出 `reasoning_content`，而非原生 Anthropic 思考區塊；若思考功能維持隱含啟用，內部推理便會洩漏至可見輸出。除非你自行明確設定 `thinking`，否則 OpenClaw 預設會停用 M2.x 的思考功能。MiniMax-M3（以及向前相容的 M3.x）不受此限制：M3 會發出正確的 Anthropic 思考區塊，且必須啟用思考功能才能產生可見內容，因此 OpenClaw 會讓 M3 繼續使用提供者的自適應思考路徑。請參閱下方「進階設定」中的「思考功能預設值」一節。
    </Warning>
...
- 文字轉圖片生成及圖片轉圖片編輯（主體參照），兩者皆支援長寬比控制
- 每個要求最多輸出 9 張圖片，每個編輯要求可使用 1 張參照圖片
...
圖片生成一律使用 MiniMax 的專用圖片端點（`/v1/image_generation`），並忽略 `models.providers.minimax.baseUrl`，因為該欄位設定的是聊天／Anthropic 相容的基底 URL。若要透過中國版端點路由圖片生成，請設定 `MINIMAX_API_HOST=https://api.minimaxi.com`；預設的全球端點為 `https://api.minimax.io`。
...
- 一般音訊附件會維持 MP3 格式。語音訊息目標（Feishu、Telegram，以及要求語音訊息相容附件的其他頻道）會使用 `ffmpeg` 將 MiniMax MP3 轉碼為 48kHz Opus，因為例如 Feishu/Lark 檔案 API 的原生音訊訊息僅接受 `file_type: "opus"`
- MiniMax T2A 的 `speed` 和 `vol` 接受小數，但 `pitch` 會以整數傳送；OpenClaw 會在 API 要求前截去 `pitch` 值的小數部分
...
- 輸出格式：`mp3`
- 以工作階段為基礎的執行會透過共用工作／狀態流程卸離，包括 `action: "status"`
...
- 模式：文字轉影片和單一圖片參考流程
- 支援 `resolution`（Hailuo 2.3/02 模型使用 `768P` 或 `1080P`）；不支援 `aspectRatio`，系統會將其忽略
...
因此，即使隨附的文字供應商目錄也包含支援圖片的 M3 聊天參照，自動媒體路由仍可使用 MiniMax 圖片理解功能。PDF 理解僅使用 `MiniMax-M2.7` 擷取文字；MiniMax 並未註冊 PDF 轉圖片的轉換路徑。
...
- 區域重用順序：`plugins.entries.minimax.config.webSearch.region`，接著是 `MINIMAX_API_HOST`，最後是 MiniMax 供應商基礎 URL
- 搜尋維持使用供應商 ID `minimax`；OAuth 中國／全球設定可透過 `models.providers.minimax-portal.baseUrl` 間接控制區域，並可透過 `MINIMAX_OAUTH_TOKEN` 提供 Bearer 驗證

設定位於 `plugins.entries.minimax.config.webSearch.*` 下。

<Note>
完整的網頁搜尋設定與使用方式請參閱 [MiniMax 搜尋](/zh-TW/tools/minimax-search)。
</Note>
...
<AccordionGroup>
  <Accordion title="設定選項">
    | 選項 | 說明 |
    | --- | --- |
    | `models.providers.minimax.baseUrl` | 優先使用 `https://api.minimax.io/anthropic`（與 Anthropic 相容）；`https://api.minimax.io/v1` 可選用於與 OpenAI 相容的承載資料 |
    | `models.providers.minimax.api` | 優先使用 `anthropic-messages`；`openai-completions` 可選用於與 OpenAI 相容的承載資料 |
  ...
  <Accordion title="思考預設值">
    在 `api: "anthropic-messages"` 上，除非先前的包裝器已在承載資料中設定 `thinking` 欄位，否則 OpenClaw 會為 MiniMax M2.x 模型注入 `thinking: { type: "disabled" }`。這可避免 M2.x 的串流端點在 OpenAI 樣式的差異區塊中輸出 `reasoning_content`，以免內部推理洩漏至可見輸出。

    MiniMax-M3（以及 M3.x）不受此限制：停用思考時，M3 會傳回包含 `stop_reason: "end_turn"` 的空白 `content` 陣列，因此 OpenClaw 會移除 M3 隱含的停用預設值，並在設定思考等級時改為強制使用 `thinking: { type: "adaptive" }`。
  ...
  <Accordion title="備援範例">
    **最適合：**將最強的最新世代模型保留為主要模型，並在失敗時改用 MiniMax M2.7。下方範例以 Opus 作為具體的主要模型；可替換成你偏好的最新世代主要模型。
  ...
  <Accordion title="Coding Plan 使用詳情">
    ...
    - 設定後，用量輪詢會從 `models.providers.minimax-portal.baseUrl` 或 `models.providers.minimax.baseUrl` 推導主機，因此使用 `https://api.minimax.io/anthropic` 的全球設定會輪詢 `api.minimax.io`。為維持相容性，基礎 URL 缺失或格式錯誤時會保留中國區備援。
    - OpenClaw 會將 MiniMax Coding Plan 用量正規化為其他供應商所使用的相同 `% left` 顯示格式。MiniMax 的原始 `usage_percent`／`usagePercent` 欄位表示剩餘配額，而非已用配額，因此 OpenClaw 會將其反轉。若存在以數量為基礎的欄位，則優先使用這些欄位。
    - 當 API 傳回 `model_remains` 時，OpenClaw 會優先使用聊天模型項目，並在需要時從 `start_time`／`end_time` 推導時間窗標籤，且會在方案標籤中包含所選模型名稱，以便更容易區分 Coding Plan 時間窗。
    - 用量快照會將 `minimax`、`minimax-cn`、`minimax-portal` 和 `minimax-portal-cn` 視為相同的 MiniMax 配額介面，並優先使用已儲存的 MiniMax OAuth，之後才回退至 Coding Plan 金鑰環境變數。
...
- 初始設定和直接 API 金鑰設定會寫入 M3 與兩種 M2.7 變體的模型定義
...
<AccordionGroup>
  <Accordion title='"未知模型：minimax/MiniMax-M3"'>
    這通常表示**尚未設定 MiniMax 供應商**（找不到相符的供應商項目，也找不到 MiniMax 驗證設定檔／環境金鑰）。修正方式如下：

    - 執行 `openclaw configure` 並選擇 **MiniMax** 驗證選項，或
    - 手動加入相符的 `models.providers.minimax` 或 `models.providers.minimax-portal` 區塊，或
    - 設定 `MINIMAX_API_KEY`、`MINIMAX_OAUTH_TOKEN` 或 MiniMax 驗證設定檔，讓系統可注入相符的供應商。
