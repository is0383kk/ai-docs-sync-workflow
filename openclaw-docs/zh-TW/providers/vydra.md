...
summary: 在 OpenClaw 中使用 Vydra 圖片、影片與語音功能
title: Vydra
x-i18n:
    generated_at: "2026-07-26T08:11:15Z"
    model: gpt-5.6
...
<Warning>
請使用 `https://www.vydra.ai/api/v1` 作為基礎 URL。Vydra 的根網域主機（`https://vydra.ai/api/v1`）目前會重新導向至 `www`。部分 HTTP 用戶端會在這個跨主機重新導向時捨棄 `Authorization`，導致有效的 API 金鑰被誤判為驗證失敗。為避免此問題，隨附的外掛會將任何已設定的 `vydra.ai` 基礎 URL 正規化為 `www.vydra.ai`。
</Warning>
...
<AccordionGroup>
  <Accordion title="圖片產生">
    ...
    隨附支援僅提供文字轉圖片功能，每個請求最多產生一張圖片。Vydra 託管的編輯路由需要遠端圖片 URL，而隨附的外掛並未新增 Vydra 專用的上傳橋接功能。
  ...
  <Accordion title="影片產生">
    ...
    - `vydra/kling` 用於圖片轉影片（恰好需要一個遠端圖片 URL）
    ...
    - `vydra/kling` 會在一開始就拒絕本機檔案上傳；只有遠端圖片 URL 參照可用。
    - Vydra 的 `kling` HTTP 路由對於需要 `image_url` 還是 `video_url` 的要求一直不一致；隨附的提供者會在這兩個欄位中傳送相同的遠端圖片 URL。
    - 隨附的外掛採取保守做法，不會轉送未記載於文件中的樣式調整參數，例如長寬比、解析度、浮水印或產生的音訊。
