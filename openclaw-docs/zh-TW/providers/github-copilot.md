...
read_when:
    ...
    - 你正在內建 Copilot 提供者、Copilot SDK 控制介面與 Copilot Proxy 之間做選擇
summary: 使用裝置流程或非互動式權杖匯入，從 OpenClaw 登入 GitHub Copilot
title: GitHub Copilot
x-i18n:
    generated_at: "2026-07-26T08:03:12Z"
    model: gpt-5.6
...
GitHub Copilot 是 GitHub 的 AI 程式設計助理。它可讓你的 GitHub 帳戶依據方案存取 Copilot
模型。OpenClaw 能以三種不同方式，將 Copilot 用作模型
供應商或代理程式執行階段。
...
<Tabs>
  ...
  <Tab title="Copilot SDK 控制框架外掛 (copilot)">
    若要讓 GitHub 的 Copilot 命令列介面與 SDK 為所選的
    `github-copilot/*` 模型管理底層代理程式迴圈，請安裝外部
    `@openclaw/copilot` 外掛。
    ...
    若要讓這些代理程式回合使用原生 Copilot 命令列介面工作階段、由 SDK 管理的執行緒
    狀態，以及由 Copilot 管理的壓縮，請選擇此方式。若未明確選擇使用
    `agentRuntime`，`github-copilot/*` 模型會繼續使用
    內建供應商。完整執行階段合約請參閱 [Copilot SDK 控制框架](/zh-TW/plugins/copilot)。
...
`your-org.ghe.com` 這類 `*.ghe.com` 主機），Copilot 會位於租用戶本機的
端點，而非公用的 `github.com`。OpenClaw 將此項目提供為
一級驗證選項，因此你不必手動編輯 URL。

<Steps>
  <Step title="選擇 Enterprise 驗證選項">
    在初始設定或 `openclaw models auth` 中，選擇
    **GitHub Copilot (Enterprise / data residency)**。系統會提示你輸入
    Enterprise 網域（例如 `your-org.ghe.com`），接著針對該租用戶
    執行裝置登入。

    請只輸入租用戶根網域（`your-org.ghe.com`）。不接受
    `api.your-org.ghe.com` 或 `copilot-api.your-org.ghe.com` 等衍生服務主機；
    OpenClaw 會自動從租用戶根網域衍生這些端點。
  ...
  <Step title="網域會持久儲存至設定">
    所選主機會儲存在供應商參數下，讓後續的權杖重新整理
    與補全自動以該租用戶為目標：
...
租用戶戳記且不含 Proxy 提示，因此補全基底 URL 會改用
租用戶的 Copilot 主機，而非公用端點。

<Note>
切換網域時一律會重新執行裝置登入。如果你已有儲存的
Copilot 權杖，並選擇不同網域（公用 `github.com` ↔ `*.ghe.com`
租用戶，或從一個租用戶切換到另一個租用戶），OpenClaw 不會重複使用現有權杖，
而是強制重新登入，確保權杖的範圍符合要寫入
設定的網域。針對*相同*網域重新執行登入時，仍會提供重複使用目前
權杖的選項。切換回公用 `github.com` 會清除持久儲存的
`githubDomain`，使設定回復為預設值。
</Note>

<Note>
`COPILOT_GITHUB_DOMAIN` 環境變數會覆寫所有會解析網域的 Copilot 路徑所解析出的網域，
包括 Enterprise 裝置登入
（`--method device-enterprise`）、獨立的
`openclaw models auth login-github-copilot` 捷徑、權杖重新整理、嵌入，
以及補全。對於完全無頭或 CI 設定，請將其設為你的 `*.ghe.com` 主機。
若要使用公用 `github.com`，請勿設定此變數（且不要加入設定參數）。
登入會持久儲存其簽發權杖所用的網域（針對公用 `github.com` 登入時則會清除），
因此即使之後取消設定環境變數，路由仍會維持正確。
</Note>
...
裝置登入流程需要互動式 TTY。對於無頭設定，請使用 `openclaw onboard --non-interactive`
匯入現有 GitHub OAuth 存取權杖：
...
GitHub Copilot 供應商驗證選項。如果省略該旗標，初始設定會依序退回使用
`COPILOT_GITHUB_TOKEN`、`GH_TOKEN`，再使用 `GITHUB_TOKEN`。設定
`COPILOT_GITHUB_TOKEN` 後搭配 `--secret-input-mode ref`，可儲存以環境變數為後端的
`tokenRef`，而不是在 `auth-profiles.json` 中儲存純文字。

<AccordionGroup>
  <Accordion title="需要互動式 TTY">
    裝置登入流程需要互動式 TTY。請直接在
    終端機中執行，不要在非互動式指令碼或 CI 流水線中執行。
  </Accordion>

  <Accordion title="模型可用性取決於你的方案">
    Copilot 模型可用性取決於你的 GitHub 方案。如果模型
  ...
  <Accordion title="從 Copilot API 即時重新整理目錄">
    ...
    （與 VS Code Copilot 使用的端點相同）重新整理模型目錄，讓執行階段能追蹤
    每個帳戶的權益與準確的上下文視窗，而不必頻繁變更資訊清單。
  ...
  <Accordion title="傳輸方式選擇">
    ...
    模型則繼續使用 OpenAI Responses 傳輸方式。OpenClaw 會根據模型參照
    選擇正確的傳輸方式。
  </Accordion>
  ...
  <Accordion title="請求相容性">
    ...
    將工具結果的後續回合標記為由代理程式起始，並在回合包含圖片輸入時設定 Copilot
    視覺標頭。
  </Accordion>

  <Accordion title="環境變數解析順序">
    OpenClaw 會依下列優先順序，從環境變數解析 Copilot 驗證：
    ...
    其權杖儲存在驗證設定檔儲存區中，且優先於所有環境
    變數。

  </Accordion>

  <Accordion title="權杖儲存">
    登入會將 GitHub 權杖儲存在驗證設定檔儲存區中（設定檔 ID
...
如果你訂閱了 Copilot 且已登入，OpenClaw 無需另外的 API 金鑰
即可使用它來產生嵌入。

### 設定

明確將 `memory.search.provider` 設為使用 GitHub Copilot 嵌入。如果有可用的
GitHub 權杖，OpenClaw 會從 Copilot API 探索可用的嵌入模型，
並自動選擇最佳模型。
