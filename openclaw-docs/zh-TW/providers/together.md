...
title: Together AI
x-i18n:
    generated_at: "2026-07-26T08:34:50Z"
    model: gpt-5.6
...
[Together AI](https://together.ai) 透過統一 API，提供 Llama、DeepSeek、Kimi 等領先的開放原始碼
模型。OpenClaw 將其內建為 `together` 提供者。
...
<Note>
初始設定會將 `together/meta-llama/Llama-3.3-70B-Instruct-Turbo` 設為
預設模型。
</Note>
...
| 模式                 | 文字轉影片；只有 `Wan-AI/Wan2.2-I2V-A14B` 支援圖片轉影片（單張參考圖片） |
| 時長                 | 1-10 秒                                                                              |
...
<Tip>
共用工具參數、提供者選擇及容錯移轉行為，請參閱[影片生成](/zh-TW/tools/video-generation)。
</Tip>

<AccordionGroup>
  <Accordion title="環境注意事項">
    如果閘道以常駐程式（launchd/systemd）執行，請確保該程序可使用
    `TOGETHER_API_KEY`（例如放在 `~/.openclaw/.env`
    中，或透過 `env.shellEnv` 提供）。

    <Warning>
    僅在互動式殼層中設定的金鑰，對由常駐程式管理的
  ...
  <Accordion title="疑難排解">
    - 驗證你的金鑰是否有效：`openclaw models list --provider together`
