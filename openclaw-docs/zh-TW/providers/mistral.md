...
title: Mistral
x-i18n:
    generated_at: "2026-07-26T08:34:01Z"
    model: gpt-5.6
...
隨附的 `mistral` 外掛會註冊四項合約：聊天補全、媒體理解（Voxtral 批次轉錄）、用於 Voice Call 的即時 STT（Voxtral Realtime），以及記憶嵌入（`mistral-embed`）。
...
| `mistral/mistral-small-latest`               | 文字、影像   | 262,144 | 16,384     | 最新版 Mistral Small 4；可調整 `reasoning_effort`     |
| `mistral/mistral-small-2603`               | 文字、影像   | 262,144 | 16,384     | 固定版本 Mistral Small 4；可調整 `reasoning_effort`   |
...
openclaw infer model run --local \
  ...
  --prompt "請只回覆：mistral-ok" \
  --json
```

## 音訊轉錄（Voxtral）

透過媒體理解流水線使用 Voxtral 進行批次音訊轉錄：
...
<AccordionGroup>
  <Accordion title="可調整推理">
    `mistral/mistral-small-latest`、`mistral/mistral-small-2603` 和 `mistral/mistral-medium-3-5` 可透過 `reasoning_effort` 在 Chat Completions API 上支援[可調整推理](https://docs.mistral.ai/studio-api/conversations/reasoning/adjustable)（`none` 會將輸出中的額外思考降至最低；`high` 會在最終答案前顯示完整的思考軌跡）。

    OpenClaw 會將工作階段的 **思考** 層級對應至 Mistral API：

    | OpenClaw 思考層級                                               | Mistral `reasoning_effort` |
    | ---------------------------------------------------------------- | --------------------------- |
    | **關閉** / **最低**                                              | `none`          |
    | **低** / **中** / **高** / **超高** / **自適應** / **最高**     | `high`          |

    <Warning>
    避免將 Medium 3.5 推理模式與 `temperature: 0` 搭配使用；據報 Mistral HTTP API 會以 400 回應拒絕 `reasoning_effort="high"` 加上 `temperature: 0`。請不要設定溫度，或將思考設為關閉／最低，讓 OpenClaw 在你設定低溫度之前先傳送 `reasoning_effort: "none"`。
    </Warning>
    ...
    <Note>
    其他隨附的 Mistral 目錄模型不使用此參數。若要使用 Mistral 原生的推理優先行為，請繼續使用 `magistral-*` 模型。
    </Note>
