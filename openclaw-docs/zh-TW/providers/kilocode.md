---
read_when:
    - 你想用單一 API 金鑰存取多個 LLM
    - 你想要在 OpenClaw 中透過 Kilo 閘道執行模型
summary: 使用 Kilo Gateway 的統一 API，在 OpenClaw 中存取多種模型
title: Kilo 閘道
x-i18n:
    generated_at: "2026-07-26T08:40:17Z"
    model: gpt-5.6
...
Kilo Gateway 透過單一 OpenAI 相容端點和 API 金鑰，將請求路由至多個模型。
...
<AccordionGroup>
  <Accordion title="傳輸與相容性">
    ...
    路徑，而非原生 OpenAI 請求塑形（無 `store`，也無 OpenAI 推理強度承載資料）。

    - 由 Gemini 支援的 Kilo 參照會維持使用代理 Gemini 路徑：OpenClaw 會在該處清理 Gemini 思考
      簽章，但不會啟用原生 Gemini 重播驗證或啟動程序改寫。
    - 請求使用以你的 API 金鑰建立的 Bearer 權杖。

  </Accordion>

  <Accordion title="串流包裝器與推理">
    Kilo 串流包裝器會加入 `X-KILOCODE-FEATURE` 請求標頭（預設為 `openclaw`，
    可使用 `KILOCODE_FEATURE` 環境變數覆寫），並為支援此功能的模型
    正規化推理強度承載資料。

    <Warning>
    `kilocode/kilo-auto/balanced` 和 `x-ai/*` 參照會略過推理強度注入。如需推理支援，
  ...
  <Accordion title="疑難排解">
    ...
    - 當閘道以常駐程式執行時，請確保該程序可取得 `KILOCODE_API_KEY`（例如放在 `~/.openclaw/.env` 中，或透過 `env.shellEnv` 提供）。
