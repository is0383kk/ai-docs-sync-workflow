...
summary: 透過 OpenClaw 使用 NovitaAI 的 OpenAI 相容 API
title: NovitaAI
x-i18n:
    generated_at: "2026-07-26T07:32:28Z"
    model: gpt-5.6
...
- 透過單一供應商帳戶使用 DeepSeek、Kimi、MiniMax、GLM 或 Qwen 系列路由。
- 除了 DeepInfra、GMI、OpenRouter 或供應商直接 API 之外的另一條託管式備援路徑。
- 使用供應商端的模型託管，而不必維護 LM Studio、Ollama、
  SGLang 或 vLLM 基礎架構。
...
- `401`/`403`：在 Novita 的金鑰管理頁面確認金鑰；如果儲存的設定檔
  已過期，請重新執行 `openclaw onboard --auth-choice novita-api-key`。
- 未知模型錯誤：使用 `openclaw models list --provider novita` 傳回的確切
  `novita/<route-id>`。
- 路由緩慢或失敗：嘗試其他 Novita 模型路由，或針對能容許供應商特定差異的
  工作負載，將 Novita 設為備援供應商。
