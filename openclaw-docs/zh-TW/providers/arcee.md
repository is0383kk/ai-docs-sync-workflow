...
title: Arcee AI
x-i18n:
    generated_at: "2026-07-26T08:32:28Z"
    model: gpt-5.6
...
[Arcee AI](https://arcee.ai) 透過 OpenAI 相容 API 提供 Trinity 混合專家模型系列。所有 Trinity 模型皆採用 Apache 2.0 授權。Arcee 是官方 OpenClaw 外掛，未與核心綁定，因此必須先安裝才能進行初始設定。
...
| `arcee/trinity-large-preview`  | Trinity Large Preview  | 文字  | 128K    | 16K        | $0.25 / $1.00        | 是   | 通用；400B 個參數，13B 個啟用參數  |
...
OpenRouter 初始設定會公開 `arcee/trinity-large-preview` 和 `arcee/trinity-large-thinking`。OpenClaw 會在設定中保留這些包含提供者限定詞的模型參照，並傳送 OpenRouter 的標準 `arcee-ai/*` 執行階段 ID。OpenRouter 已不再提供 Trinity Mini；該模型請使用 Arcee 直接 API。
...
<AccordionGroup>
  ...
  <Accordion title="OpenRouter 路由">
    ...
    OpenClaw 會使用標準 `arcee-ai/trinity-large-thinking`
    OpenRouter 執行階段 ID 進行路由。OpenRouter 專屬設定的詳細資訊請參閱
