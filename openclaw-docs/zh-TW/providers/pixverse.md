...
title: PixVerse
x-i18n:
    generated_at: "2026-07-26T08:10:47Z"
    model: gpt-5.6
...
OpenClaw 提供 `pixverse` 作為官方外部外掛，用於託管式 PixVerse 影片生成。此外掛會依據 `videoGenerationProviders` 合約註冊 `pixverse` 提供者。
...
<Steps>
  ...
  <Step title="設定 API 金鑰">
    ...
    精靈會先提示選擇國際或中國端點（請參閱下方的 API 區域），再將 `region` 和 `baseUrl` 寫入提供者設定。
    非互動式執行（從 `--pixverse-api-key` 或 `PIXVERSE_API_KEY` 取得金鑰）
    預設使用國際端點。

    如果尚未設定預設影片模型，初始設定也會將 `agents.defaults.mediaModels.video.primary` 設為
    `pixverse/v6`。
...
本機圖片參考會在圖片轉影片請求前上傳至 PixVerse。遠端圖片 URL 則會以 `image_url` 傳送至 PixVerse 圖片上傳端點。
...
<Note>
PixVerse 圖片範本生成功能目前尚未透過 `image_generate` 提供。該 API 由範本 ID 驅動，而 OpenClaw 的共用圖片生成合約目前沒有 PixVerse 專用的具型別選項集合。
</Note>
...
| `motionMode` / `motion_mode` | 字串 | 圖片轉影片的動態模式（預設 `normal`） |
| `cameraMovement` / `camera_movement` | 字串 | PixVerse 攝影機移動預設集                        |
...
<AccordionGroup>
  <Accordion title="API 區域">
    ...
    當你的金鑰屬於特定的 PixVerse 平台區域時，請手動設定 `models.providers.pixverse.region`，或執行
    `openclaw onboard --auth-choice pixverse-api-key`，以便在設定精靈中選擇區域：
  ...
  <Accordion title="工作輪詢">
    ...
    `/openapi/v2/video/result/{video_id}`，直到工作成功、失敗或逾時（預設 5 分鐘；可使用
    `agents.defaults.mediaModels.video.timeoutMs` 覆寫）。
  </Accordion>
</AccordionGroup>
