...
title: Qwen
x-i18n:
    generated_at: "2026-07-26T08:34:32Z"
    model: gpt-5.6
...
Qwen Cloud 是官方的外部 OpenClaw 提供者外掛，標準 id 為 `qwen`。它適用於 Qwen Cloud / Alibaba DashScope Standard 與 Coding Plan 端點，將 Token Plan 公開為 `qwen-token-plan`，保留 `modelstudio` 作為相容性別名，並獨立擁有 Alibaba 文件所述的 `bailian-token-plan` 自訂提供者 id。

| 屬性                   | 值                                         |
| ---------------------- | ------------------------------------------ |
| 提供者                 | `qwen`                         |
| Token Plan 提供者      | `qwen-token-plan`                         |
| 慣用環境變數           | `QWEN_API_KEY`                         |
...
`qwen` 以官方外部外掛形式提供，不隨核心綁定。請安裝此外掛並重新啟動閘道：
...
<Tabs>
  <Tab title="Coding Plan（訂閱制）">
    **最適合：**透過 Qwen Coding Plan 取得訂閱制存取權。
    ...
    <Note>
    舊版 `modelstudio-*` 認證選擇 id 與 `modelstudio/...` 模型參照仍可
    作為相容性別名使用，但新的設定流程應優先使用標準的
    `qwen-*` 認證選擇 id 與 `qwen/...` 模型參照。如果你定義了完全相符的
    自訂 `models.providers.modelstudio` 項目，且其 `api` 值不同，該
    自訂提供者將擁有 `modelstudio/...` 參照，而非 Qwen 相容性
    別名。
    </Note>
  ...
  <Tab title="Standard（隨用隨付）">
    **最適合：**透過 Standard Model Studio 端點取得隨用隨付存取權，包括 Coding Plan 不提供的 `qwen3.7-max` 與 `qwen3.6-flash`。
  ...
  <Tab title="Token Plan（團隊版）">
    **最適合：**透過 Alibaba Cloud Model Studio，以點數制團隊訂閱方式存取 Qwen 與支援的第三方模型。
    ...
    <Steps>
      <Step title="取得專用金鑰">
        指派一個 Token Plan 席次，並建立其專用的 `sk-sp-...` 金鑰。Token Plan、Coding Plan 與隨用隨付金鑰無法互換使用。請參閱[全球 Token Plan 概覽](https://www.alibabacloud.com/help/en/model-studio/token-plan-overview)或[中國 Token Plan 概覽](https://help.aliyun.com/zh/model-studio/token-plan-overview)。
      </Step>
      <Step title="執行初始設定">
        若使用位於新加坡的**全球／國際**端點：
    ...
    提供者。此前掛會將該 id 註冊為相容性擁有者，但新的
    設定應使用 `qwen-token-plan`。完全相符的自訂
    `models.providers.bailian-token-plan` 項目會保有其已設定傳輸方式與目錄的
    擁有權；它絕不會合併至標準 OpenAI 目錄。
    </Note>

    <Warning>
    Token Plan 僅可用於互動式 OpenClaw 工作階段。請勿將它用於
    排程工作、無人值守指令碼或應用程式後端。Alibaba 表示，
    非互動式使用可能會導致訂閱遭停用，或其 API 金鑰遭撤銷。
    </Warning>
...
提供者會根據你的認證選擇自動選取端點。標準
選項使用 `qwen-*` 系列；`modelstudio-*` 僅保留供相容性使用。
可在設定中透過自訂 `baseUrl` 覆寫。
...
OpenClaw 提供以下 Qwen 靜態目錄。此目錄可辨識端點：Coding
Plan 設定會省略僅適用於 Standard 端點的模型。

| 模型參照                    | 輸入        | 上下文    | 備註                    |
| --------------------------- | ----------- | --------- | ----------------------- |
| `qwen/qwen3.5-plus`          | 文字、影像  | 1,000,000 | 預設模型                |
| `qwen/qwen3.6-flash`          | 文字、影像  | 1,000,000 | 僅限 Standard 端點      |
| `qwen/qwen3.6-plus`          | 文字、影像  | 1,000,000 | Coding Plan + Standard  |
| `qwen/qwen3.7-max`          | 文字        | 1,000,000 | 僅限 Standard 端點      |
| `qwen/qwen3.7-plus`          | 文字、影像  | 1,000,000 | Coding Plan + Standard  |
...
<Note>
即使模型列於靜態目錄中，其可用性仍可能因端點與計費方案而異。
</Note>

### Token Plan 目錄

Token Plan 使用獨立的完全字串允許清單。此處不包含僅供影像生成的方案
模型，因為它們使用不同的 API。

| 模型參照                            | 輸入        | 上下文    |
| ----------------------------------- | ----------- | --------- |
| `qwen-token-plan/qwen3.7-max`                  | 文字        | 1,000,000 |
| `qwen-token-plan/qwen3.7-plus`                  | 文字、影像  | 1,000,000 |
| `qwen-token-plan/qwen3.6-plus`                  | 文字、影像  | 1,000,000 |
| `qwen-token-plan/qwen3.6-flash`                  | 文字、影像  | 1,000,000 |
...
| `qwen-token-plan/kimi-k2.7-code`                  | 文字、影像  | 262,144   |
| `qwen-token-plan/kimi-k2.6`                  | 文字、影像  | 262,144   |
| `qwen-token-plan/kimi-k2.5`                  | 文字、影像  | 262,144   |
...
`qwen3.7-max`、`qwen3.7-plus`、`qwen3.6-flash` 與 `qwen3.6-plus` 在
內建目錄中已啟用推理。對於 `qwen` 系列的推理模型，
提供者會將 OpenClaw 的思考層級對應至 DashScope 頂層的
`enable_thinking` 請求旗標：停用思考時傳送 `enable_thinking: false`，
任何其他層級則傳送 `enable_thinking: true`。自訂模型可在模型項目中設定
`compat.thinkingFormat: "qwen-chat-template"`，以選用替代的聊天範本思考承載內容。

Token Plan 模型也標示為具備推理能力。`kimi-k2.7-code` 與
`MiniMax-M2.5` 僅支援思考，因此即使工作階段要求
`/think off`，OpenClaw 仍會保持啟用思考。DeepSeek V4 會將 `minimal` 到 `high`
對應至服務的 `high` 強度，並將 `xhigh` 或 `max` 對應至 `max`。GLM 5.2 接受
從 `minimal` 到 `max` 的完整範圍；GLM 5.1 與 GLM 5 接受最高至
...
端點上提供多模態功能，不支援 Coding Plan 端點：

- **影像與影片理解**，透過 `qwen3.6-plus`
- **Wan 影片生成**，透過 `wan2.6-t2v`（預設）、`wan2.6-i2v`、`wan2.6-r2v`、`wan2.6-r2v-flash`、`wan2.7-r2v`

系統會根據已設定的 Qwen 認證自動解析媒體理解功能；不需要額外
設定。若要使用媒體理解功能，請確認使用的是 Standard（隨用隨付）
端點。
...
影片生成限制：每個請求可輸出 1 部影片、最多可輸入 1 張圖片
（圖片轉影片）、最多可輸入 4 部影片（影片轉影片），最長
10 秒。支援 `size`、`aspectRatio`、`resolution`、`audio` 和
`watermark`。參考圖片／影片輸入必須使用遠端 http(s) URL；系統會預先拒絕本機
檔案路徑，因為 DashScope 影片端點不接受為這些參考資料上傳的本機緩衝區。

<Note>
如需共用工具參數、供應商選擇和容錯移轉行為，請參閱[影片生成](/zh-TW/tools/video-generation)。
</Note>
...
<AccordionGroup>
  <Accordion title="Qwen 3.6 和 3.7 的可用性">
    `qwen3.7-plus` 和 `qwen3.6-plus` 可用於 Coding Plan 和 Standard 端點。`qwen3.7-max` 和 `qwen3.6-flash` 僅適用於 Standard。Standard（隨用隨付）端點如下：
    ...
    如果 Coding Plan 端點對其中任一模型傳回「不支援的模型」錯誤，
    請切換至相符的 Standard 端點和金鑰。
  ...
  <Accordion title="串流用量相容性">
    原生 Qwen 端點會在共用的
    `openai-completions` 傳輸上宣告串流用量相容性，因此以相同原生主機為目標的
    DashScope 相容自訂供應商 ID，無須特別使用內建的
    `qwen` 供應商 ID，即可繼承相同行為。這適用於 Coding Plan、
  ...
  <Accordion title="功能規劃">
    `qwen` 外掛的定位是完整 Qwen
    Cloud 介面的供應商歸屬位置，而不僅限於程式設計／文字模型。
    ...
    - **影片生成：**可透過此外掛及共用影片生成功能使用

  </Accordion>

  <Accordion title="環境與常駐程式設定">
    如果閘道以常駐程式（launchd/systemd）執行，請確保該程序可取得 `QWEN_API_KEY`
    或 `QWEN_TOKEN_PLAN_API_KEY`（例如放在
...
<CardGroup cols={2}>
  ...
  <Card title="Alibaba Model Studio" href="/zh-TW/providers/alibaba" icon="cloud">
    位於相同 DashScope 平台上的內建 Wan 影片生成供應商。
  </Card>
