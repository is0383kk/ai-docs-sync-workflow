...
title: Groq
x-i18n:
    generated_at: "2026-07-26T07:31:58Z"
    model: gpt-5.6
...
[Groq](https://groq.com) 使用自訂 LPU 硬體，為開放權重模型（Llama、Gemma、Kimi、Qwen、GPT OSS 等）提供超高速推論。Groq 外掛會同時註冊一個與 OpenAI 相容的聊天提供者，以及一個音訊媒體理解提供者。
...
OpenClaw 隨附以資訊清單為基礎的 Groq 目錄，其中包含推理與非推理項目。執行 `openclaw models list --provider groq` 可查看已安裝版本的靜態項目，或查看 [console.groq.com/docs/models](https://console.groq.com/docs/models) 以取得 Groq 的權威清單。
...
<Tip>
  此目錄會隨每個 OpenClaw 版本演進。`openclaw models list --provider groq` 會顯示已安裝版本已知的項目；請與 [console.groq.com/docs/models](https://console.groq.com/docs/models) 交叉核對，以確認新增或已棄用的模型。
</Tip>

## 推理模型

Groq 推理模型（上表中的 `reasoning: true`）會將 OpenClaw 共用的 `/think` 等級對應至 `reasoning_effort` 的 `low`、`medium` 或 `high` 值。`/think off` 或 `/think none` 會從請求中省略 `reasoning_effort`，而不是傳送停用值。

請參閱[思考模式](/zh-TW/tools/thinking)，了解共用的 `/think` 等級，以及 OpenClaw 如何針對各提供者轉換這些等級。
...
<AccordionGroup>
  <Accordion title="常駐程式的環境可用性">
    如果閘道以受管理的服務執行（launchd、systemd、Docker），該程序必須能看見 `GROQ_API_KEY`，而不能只讓你的互動式 shell 看見。

    <Warning>
      除非也將該環境匯入 launchd 或 systemd，否則僅在互動式 shell 中匯出的金鑰無法供其常駐程式使用。請在 `~/.openclaw/.env` 中或透過 `env.shellEnv` 設定金鑰，讓閘道程序可以讀取。
    </Warning>

  </Accordion>

  <Accordion title="自訂 Groq 模型 ID">
    OpenClaw 在執行階段接受任何 Groq 模型 ID。請使用 Groq 顯示的確切 ID，並在前面加上 `groq/`。靜態目錄涵蓋常見情況；未列入目錄的 ID 會改用預設的 OpenAI 相容範本。
