...
title: 遠端閘道設定
x-i18n:
    generated_at: "2026-07-26T08:34:15Z"
    model: gpt-5.6
...
若要讓通道在重新啟動後持續運作並自動重新連線，請使用[遠端存取](/zh-TW/gateway/remote#macos-persistent-ssh-tunnel-via-launchagent)頁面上的 LaunchAgent 設定，而不是手動執行 `ssh -N`。
...
| `RunAtLoad`                          | 載入 LaunchAgent 時啟動通道（LaunchAgent）    |
