...
read_when:
    ...
    - 將閘道作為子程序進行監督
    - 無須擷取日誌即可處理閘道就緒、重新啟動、關閉或無效設定的情況
summary: 從 Electron 或其他主機應用程式將 OpenClaw 閘道作為子程序監督執行
title: 嵌入 OpenClaw
x-i18n:
    generated_at: "2026-07-26T07:19:29Z"
    model: gpt-5.6
...
請使用實際的 `node_modules` 安裝並啟動套件可執行檔。對於自行管理
...
請依照範例透過已安裝的套件解析 OpenClaw；不要假設宿主程序的
...
| 設定                             | 嵌入效果                                                                                                                                                                                         |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `OPENCLAW_DISABLE_BONJOUR=1`               | 當宿主自行管理探索時，停用由閘道管理的區域網路多播公告。                                                                                                                                         |
| `OPENCLAW_NO_RESPAWN=1`               | 在未受管理的嵌入子程序中，防止 OpenClaw 將更新重新啟動交給已卸離的子程序。例行重新啟動仍在程序內進行，因此宿主會保有受追蹤 PID 的所有權。                                                         |
| `OPENCLAW_EXEC_SHELL_SNAPSHOT=0`               | 停用宿主執行命令的登入 shell 快照擷取。                                                                                                                                                          |
| `OPENCLAW_SKIP_CHANNELS=1`               | 略過頻道啟動與重新載入。僅當嵌入應用程式需要純控制平面或僅限 WebChat 的閘道時才設定此值。                                                                                                         |
...
Shell 快照擷取會從登入 shell 執行 `process.execPath -e <script>`。在
...
請使用 WebSocket 訊號，而不是比對記錄子字串：

1. 開啟閘道 WebSocket。
2. 等待 `connect.challenge` 事件。此事件證明接聽程式已接受
   WebSocket，且挑戰交握可以開始。
3. 傳送 `connect`，並附上與挑戰綁定的裝置簽章。
4. 將 `hello-ok` 視為已驗證 RPC 的應用程式就緒訊號。
...
`config.get` 會先遮蔽敏感值與 SecretRef 識別碼，再傳回
...
- [建置閘道用戶端](https://docs.openclaw.ai/gateway/clients)
- [閘道協定](https://docs.openclaw.ai/gateway/protocol)
