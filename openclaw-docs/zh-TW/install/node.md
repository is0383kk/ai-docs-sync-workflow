...
title: Node.js
x-i18n:
    generated_at: "2026-07-26T07:23:35Z"
    model: gpt-5.6
...
OpenClaw 需要 **Node 22.22.3+、Node 24.15+ 或 Node 25.9+**。對於安裝、CI 與發布工作流程，**Node 24 是預設且建議使用的執行環境**；Node 22 仍透過有效的 LTS 系列獲得支援。不支援 Node 23。[安裝程式指令碼](/zh-TW/install#alternative-install-methods)會自動偵測並安裝 Node——如果你想自行設定 Node（版本、PATH、全域安裝），請參閱此頁面。
...
建議預設使用 `v24.15.0` 或更新的 24.x 版本。`v22.22.3` 或更新的 22.x 版本是受支援的 Node 22 LTS 路徑；Node `v25.9.0+` 也受到支援。不支援 Node 23。如果尚未安裝 Node，或版本不在支援範圍內，請選擇下方的安裝方式。
...
<Accordion title="使用版本管理工具（nvm、fnm、mise、asdf）">
  版本管理工具可讓你輕鬆切換不同的 Node 版本。常用選項包括：
...
```
  ...
  <Warning>
  請在 shell 啟動檔案（`~/.zshrc` 或 `~/.bashrc`）中初始化版本管理工具。如果略過此步驟，因為 PATH 不會包含 Node 的 bin 目錄，新開啟的終端機工作階段可能會找不到 `openclaw`。
  </Warning>
</Accordion>
...
<Steps>
  <Step title="尋找你的全域 npm 前置路徑">
    ```bash
    npm prefix -g
    ```
  </Step>
  <Step title="檢查它是否位於你的 PATH 中">
    ```bash
    echo "$PATH"
    ```
...
如果出現 `EACCES` 錯誤，請將 npm 的全域前置路徑切換至使用者可寫入的目錄：
...
將 `export PATH=...` 這一行加入 `~/.bashrc` 或 `~/.zshrc`，使設定永久生效。
