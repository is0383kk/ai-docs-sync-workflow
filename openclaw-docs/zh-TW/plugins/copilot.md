...
x-i18n:
    generated_at: "2026-07-26T08:40:08Z"
    model: gpt-5.6
...
| `permissionPolicy`       | SDK 的 `onPermissionRequest` 處理常式選用覆寫，用於內建 SDK 工具種類（`shell`、`write`、`read`、`url`、`mcp`、`memory`、`hook`）。預設為 `rejectAllPolicy`，作為安全防護；請參閱[權限與 ask_user](#permissions-and-ask_user)，瞭解為何它實際上絕不會觸發。 |
