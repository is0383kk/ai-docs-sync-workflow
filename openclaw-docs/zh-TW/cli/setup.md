---
read_when:
    - 你想與 OpenClaw 對話以進行設定或修復
    - 你正在使用入門精靈進行首次執行設定
    - 你想要設定預設工作區路徑
    - 你需要供指令碼使用的僅基準設定旗標
summary: '`openclaw setup` 的命令列介面參考（系統代理程式聊天，含新手引導備援）'
title: 設定
x-i18n:
    generated_at: "2026-07-26T07:47:45Z"
    model: gpt-5.6
...
`openclaw setup` 會開啟互動式 OpenClaw 聊天。在全新的系統上，它會
轉而進入引導式初始設定。使用 `-m`/`--message` 執行單一要求，或使用
`--baseline` 在不啟動精靈的情況下初始化設定／工作區資料夾。
...
3. 若未提供路由選項，已設定的互動式系統會開啟 OpenClaw。全新的
   系統則會執行初始設定。在已設定的系統上，即使沒有 TTY，`--json` 仍會輸出
   系統概覽；若提供初始設定選項，則會保留初始設定的
   JSON 摘要。

在引導模式中，`--workspace <dir>` 是向 OpenClaw 提議的工作區；
只有在你核准該提議後才會保存。基準、傳統和
非互動式設定會在全新安裝時，透過各自的正常流程保存所提供的工作區。
當現有的代理程式名冊將被重新對應時，
...
系統會在命令列介面和 API 金鑰候選項目之後，自動測試偵測到的本機執行環境；
若有多個本機模型可用，OpenClaw 會優先選擇
工具呼叫能力最強的指令微調模型系列。所選候選項目必須能回應一次
真實的補全要求，系統才會儲存其供應商和模型設定。
如果已安裝的 Gemini、Antigravity、Pi 和 OpenCode 命令列介面
無法作為引導式設定可重複使用的推論路由，系統也會回報它們。
...
`openclaw onboard --tui` 相同的終端替代入口。完整旗標參考與
非互動式範例請參閱[初始設定](/zh-TW/cli/onboard)和
[命令列介面自動化](/zh-TW/start/wizard-cli-automation)。`openclaw onboard --modern` 仍是同一個
受推論能力檢查限制之 OpenClaw 助理的相容性進入點。

<Note>
`openclaw setup` 適用於設定可變更的安裝。在 Nix 模式（`OPENCLAW_NIX_MODE=1`）中，OpenClaw 會拒絕寫入設定，因為設定檔由 Nix 管理。請使用第一方的 [nix-openclaw 快速入門](https://github.com/openclaw/nix-openclaw#quick-start)，或其他 Nix 套件的等效原始碼設定。
</Note>
...
| 旗標                       | 說明                                                                                          |
| -------------------------- | ---------------------------------------------------------------------------------------------------- |
| `-m, --message <text>`     | 執行一個 OpenClaw 要求。                                                                            |
| `--yes`                    | 核准一次 `--message` 要求所需的永久設定寫入。                                        |
| `--workspace <dir>`        | 工作區提議；現有代理程式群需要傳統模式確認，非互動式執行時則予以保留。 |
| `--baseline`               | 在不執行初始設定的情況下，建立基準設定／工作區／工作階段資料夾。                                 |
| `--wizard`                 | 強制執行互動式初始設定。                                                                        |
| `--tui`                    | 使用終端替代入口，而非瀏覽器轉交流程。                                               |
| `--non-interactive`        | 在不顯示提示的情況下執行初始設定。                                                                      |
| `--accept-risk`            | 確認完整系統代理程式存取的風險；與 `--non-interactive` 一起使用時為必要選項。                        |
...
| `--import-from <provider>` | 初始設定期間要執行的遷移供應商。                                                         |
...
顯示提示的精靈，而非互動式設定則使用自動化路徑。
在互動式初始設定中，`--remote-url` 和 `--remote-token` 會預先填入
遠端閘道步驟，且在該次執行中優先於已儲存的遠端值。
除非你也傳入權杖，否則變更 URL 不會重複使用已儲存的認證資訊。
權杖會持續以遮蔽方式顯示，並使用精靈所選的純文字或 SecretRef
儲存模式。

### 基準模式

`openclaw setup --baseline` 會保留舊版僅限基準的行為：它會
建立設定、工作區和工作階段目錄，然後直接結束而不
執行初始設定。它接受 `--workspace` 和不影響行為的輸出控制選項，但會
拒絕明確指定的初始設定、閘道、驗證、重設或常駐程式選項，而不是
...
- 完成基準設定後，請執行 `openclaw onboard` 以進行完整的引導流程、執行 `openclaw configure` 以進行特定變更，或執行 `openclaw channels add` 以新增頻道帳號。
- 如果偵測到 Hermes 狀態，互動式初始設定可以自動提供遷移選項。匯入式初始設定需要全新的設定；若要在初始設定之外取得試執行計畫、備份和覆寫模式，請使用[遷移](/zh-TW/cli/migrate)。
