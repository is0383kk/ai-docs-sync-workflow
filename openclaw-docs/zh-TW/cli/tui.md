...
summary: '`openclaw tui` 的命令列介面參考（由閘道支援或本機內嵌終端使用者介面）'
title: 終端介面
x-i18n:
    generated_at: "2026-07-26T07:39:07Z"
    model: gpt-5.6
...
| `--token <token>`            | （無）                                    | 閘道需要權杖時所使用的權杖。                                                         |
| `--password <pass>`          | （無）                                    | 閘道需要密碼時所使用的密碼。                                                      |
| `--tls-fingerprint <sha256>` | `gateway.remote.tlsFingerprint`           | 固定 `wss://` 閘道的預期 TLS 憑證指紋。                |
...
別名：`openclaw chat` 和 `openclaw terminal` 會叫用此命令，並隱含
`--local`。

## 注意事項

- `--local` 無法與 `--url`、`--token`、`--password` 或 `--tls-fingerprint` 搭配使用。
- 可能的情況下，`tui` 會解析已設定的閘道驗證 SecretRef，以供權杖／密碼驗證使用
  （`env`/`file`/`exec` 提供者）。
- 未明確指定 URL 或連接埠時，`tui` 會採用執行中閘道所記錄的作用中本機閘道連接埠。
  明確指定的 `--url`、`OPENCLAW_GATEWAY_URL`、
  `OPENCLAW_GATEWAY_PORT` 和遠端閘道設定仍具有較高優先順序。
- 從已設定的代理程式工作區目錄內啟動時，終端介面會自動選取
  該代理程式作為工作階段金鑰的預設值（除非明確將 `--session`
  設為 `agent:<id>:...`）。
- 本機模式會直接使用嵌入式代理程式執行階段。大多數本機工具都能運作，
  但無法使用僅限閘道的功能。
- 本機模式會將 `/auth [provider]` 新增至終端介面的命令介面。
- 外掛核准閘門在本機模式下仍然適用：需要核准的工具
...
openclaw chat --message "比較我的設定與文件，並告訴我該修正哪些地方"
