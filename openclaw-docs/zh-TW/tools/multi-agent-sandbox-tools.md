...
title: 多代理沙箱與工具
x-i18n:
    generated_at: "2026-07-26T08:01:21Z"
    model: gpt-5.6
...
<CardGroup cols={3}>
  <Card title="沙箱隔離" href="/zh-TW/gateway/sandboxing">
    後端與模式 — 完整的沙箱參考資料。
  </Card>
...
<Warning>
驗證以代理為範圍：每個代理在 `~/.openclaw/agents/<agentId>/agent/openclaw-agent.sqlite` 中都有自己的 `agentDir` 驗證儲存區。絕不可跨代理重複使用 `agentDir`。代理沒有本機設定檔時，可以讀取預設／主要代理的驗證設定檔，但 OAuth 重新整理權杖不會複製到次要代理儲存區。如果手動複製認證資訊，請僅複製可攜式靜態 `api_key` 或 `token` 設定檔。
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="範例 2b：全域程式設計設定檔 + 僅限傳訊的代理">
    ```json
    {
...
工具原則支援可展開為多個工具的 `group:*` 簡寫。完整清單請參閱[工具群組](/zh-TW/gateway/sandbox-vs-tool-policy-vs-elevated#tool-groups-shorthands)。
...
<Note>
舊版 `agents.defaults.*`/`agents.entries.*.*` 設定鍵（例如 `sandbox.perSession`、`agentRuntime`、`embeddedPi`）會由 `openclaw doctor` 遷移；之後請優先使用 `agents.defaults` + `agents.entries`。
</Note>
...
<Tabs>
  ...
  <Tab title="停用檔案系統工具的 Shell 執行">
    ...
    <Warning>
    此原則會停用 OpenClaw 檔案系統工具，但 `exec` 仍是 Shell，且可以在所選主機或沙箱檔案系統允許的任何位置寫入檔案。若要建立唯讀代理，請拒絕 `exec` 與 `process`，或將 Shell 存取權與 `agents.defaults.sandbox.workspaceAccess: "ro"` 或 `"none"` 等沙箱檔案系統控制搭配使用。
    </Warning>
  ...
  <Tab title="僅限通訊">
    ...
    此設定檔中的 `sessions_history` 仍會傳回有界限且經過清理的回憶檢視，而不是原始逐字稿傾印。代理助理回憶會先移除思考標籤、`<relevant-memories>` 鷹架、純文字工具呼叫 XML 承載資料（包括 `<tool_call>...</tool_call>`、`<function_call>...</function_call>`、`<tool_calls>...</tool_calls>`、`<function_calls>...</function_calls>` 及遭截斷的工具呼叫區塊）、降級的工具呼叫鷹架、洩漏的 ASCII／全形模型控制權杖，以及格式錯誤的 MiniMax 工具呼叫 XML，再進行遮蔽／截斷。
...
<AccordionGroup>
  <Accordion title="即使設定 `mode: 'all'`，代理仍未套用沙箱">
  ...
  <Accordion title="即使有拒絕清單仍可使用的工具">
    ...
    - 每一層只能進一步限制，無法重新授予權限。
    - 請參閱[沙箱、工具政策與提升權限的比較](/zh-TW/gateway/sandbox-vs-tool-policy-vs-elevated)，以進行逐步偵錯。

  </Accordion>
  <Accordion title="容器未依每個代理程式隔離">
    - 預設的 `scope` 是 `"agent"`（每個代理程式 ID 使用一個容器）。
