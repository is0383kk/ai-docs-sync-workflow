...
title: 遠端控制
x-i18n:
    generated_at: "2026-07-26T08:38:57Z"
    model: gpt-5.6
...
此流程可讓 macOS App 作為完整的遠端控制工具，用於控制在另一台主機（桌上型電腦／伺服器）上執行的 OpenClaw 閘道。App 可直接連線至受信任的 LAN／Tailnet 閘道 URL；若遠端閘道僅限回送介面，則可管理 SSH 通道。健康狀態檢查、語音喚醒轉送和 Web Chat 會重複使用 _Settings -> General_ 中的相同遠端設定。
...
App 會針對自身的 SSH 程序停用 SSH 連線多工和驗證後背景化，以便監控並重新啟動該確切程序，即使所選別名已啟用 `ControlMaster` 或 `ForkAfterAuthentication` 亦然。
...
遠端模式中的瀏覽器自動化由命令列介面的節點主機擁有，而非原生 macOS App 節點。App 會在可行時啟動已安裝的節點主機服務；若要從該 Mac 啟用瀏覽器控制，請使用 `openclaw node install ...` 和 `openclaw node start` 安裝／啟動服務（或在前景執行 `openclaw node run ...`），然後指定該支援瀏覽器功能的節點。
...
`openclaw-mac connect`、`wizard` 和 `configure-remote` 會依下列順序解析使用中的設定：`OPENCLAW_CONFIG_PATH`，接著是 `$OPENCLAW_STATE_DIR/openclaw.json`，最後是 `~/.openclaw/openclaw.json`。兩種設定形式都會寫入該使用中檔案、將新手引導標記為已完成，並讓 App 在下次啟動時管理所選傳輸方式。`--local-port`／`--remote-port` 的預設值為 `18789`。其他旗標：`--password`、`--identity <path>`、`--ssh-host-key-policy <strict|openssh>`、`--project-root <path>`、`--cli-path <path>`、`--json`。執行 `openclaw-mac configure-remote --help` 以查看完整參考資訊。
...
2. 在 **OpenClaw runs** 下選擇 **Remote**，並設定：
   ...
   - **SSH target**：`user@host`（可選的 `:port`）。如果閘道位於同一個 LAN 並透過 Bonjour 公告，請從探索到的清單中選取，以自動填入此欄位。
