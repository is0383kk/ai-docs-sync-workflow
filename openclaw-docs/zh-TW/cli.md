...
title: 命令列介面參考指南
x-i18n:
    generated_at: "2026-07-26T08:27:58Z"
    model: gpt-5.6
...
參考頁面，或與其別名所對應的命令一併記錄；此索引列出
適用於整個命令列介面的命令、全域旗標與輸出樣式規則。

依用途分類的設定命令：

- `openclaw setup` 和 `openclaw onboard` 會先驗證推論，然後啟動 OpenClaw，以設定閘道、工作區、頻道、Skills 與健康狀態。
- `openclaw setup --baseline` 會建立基準設定與工作區，而不進入引導式初始設定流程。
- `openclaw configure` 會變更現有設定的特定部分：模型驗證、閘道、頻道、外掛或 Skills。
- `openclaw channels add` 會在基準設定建立後設定頻道帳號；若只選擇頻道，則使用引導式設定，而帳號、認證資訊或頻道設定旗標則會使用供指令碼使用的直接路徑。
...
- 在支援的環境中，OSC-8 超連結會呈現為可點擊的連結；否則
  命令列介面會改用純文字 URL。
- `--json`（以及在支援處使用的 `--plain`）會停用樣式，以產生乾淨的輸出。
...
色彩配置的唯一權威來源：`packages/terminal-core/src/palette.ts`。
...
當 OAuth/API 認證資訊可用時，`openclaw status --usage` 和控制介面會顯示供應商使用量／配額。資料直接來自供應商的使用量端點，並正規化為 `X% left`。目前提供使用量期間資料的供應商包括：Anthropic、Gemini CLI、GitHub Copilot、MiniMax、OpenAI Codex、Xiaomi 與 z.ai。
