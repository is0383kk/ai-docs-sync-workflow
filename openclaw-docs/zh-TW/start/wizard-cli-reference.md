...
summary: OpenClaw 引導設定的逐步行為：每個步驟的作用、寫入的設定及其內部機制
title: 命令列介面設定參考資料
x-i18n:
    generated_at: "2026-07-26T08:39:40Z"
    model: gpt-5.6
...
如需操作說明，請參閱[新手引導（命令列介面）](/zh-TW/start/wizard)。如需完整的命令列介面旗標
...
- 網頁搜尋供應商（選用）
- 常駐程式安裝（LaunchAgent、systemd 使用者單元，或原生 Windows 排定的工作，並以啟動資料夾作為備援）
- 健康狀態檢查
- Skills 設定

遠端模式會設定此機器連線至其他位置的閘道。此模式
不會在遠端主機上安裝或修改任何內容。
...
<Steps>
  ...
  <Step title="工作區">
    ...
    - 建立首次執行啟動程序所需的工作區檔案。
    - 重新執行時，現有的代理程式名冊會保留其整個機群共用的工作區，除非
      你明確確認移動。非互動式重新執行會顯示警告並保留
      目前的值。
  ...
  <Step title="閘道">
    ...
    - 建議：即使使用迴路介面，也請保持權杖驗證啟用，讓本機 WS 用戶端必須通過驗證。
    - 在權杖模式中，互動式設定提供：
      - **產生／儲存純文字權杖**（預設）
      - **使用 SecretRef**（選擇性啟用）
  ...
  <Step title="頻道">
    ...
    - [iMessage](/zh-TW/channels/imessage)：`imsg` 命令列介面路徑 + Messages 資料庫存取權；閘道不在 Mac 上執行時，請使用 SSH 包裝函式
  ...
  <Step title="安裝常駐程式">
    ...
    - Linux 與透過 WSL2 執行的 Windows：systemd 使用者單元
      - 精靈會嘗試執行 `loginctl enable-linger <user>`，讓閘道在登出後仍保持執行。
      - 可能會提示使用 sudo（寫入 `/var/lib/systemd/linger`）；精靈會先嘗試不使用 sudo。
    - 原生 Windows：優先使用排定的工作
      - 如果建立工作遭到拒絕，OpenClaw 會改用每位使用者的啟動資料夾登入項目，並立即啟動閘道。
      - 排定的工作仍是首選，因為它能提供更完善的監督程式狀態。
  ...
  <Step title="Skills">
    ...
    - 當所需的安裝程式可用時，為受信任的隨附 Skills
      安裝選用相依套件。
    - 略過無法使用的 Homebrew、uv 與 Go 安裝程式，接著將受影響的
...
<Note>
如果未偵測到圖形使用者介面，精靈會列印控制介面的 SSH 連接埠轉送指示，而不會開啟瀏覽器。
如果缺少控制介面資產，精靈會嘗試建置；備援方式為 `pnpm ui:build`（自動安裝使用者介面相依套件）。
</Note>
...
<Steps>
  <Step title="探索（選用）">
    如果 `dns-sd`（macOS）或 `avahi-browse`（Linux）可用，新手引導
  ...
  <Step title="連線方式">
    ...
    - **直接連線**：透過 `wss://` 連線，並提示你信任探索到的
      TLS 指紋（首次使用時信任並固定；只有在你接受後才會固定）。
    - **SSH 通道**：列印要先執行的 `ssh -N -L 18789:127.0.0.1:18789 <user>@<host>`
      命令，然後連線至本機通道端點。
  </Step>
  <Step title="驗證">
    選擇權杖（建議）、密碼或無驗證，然後可選擇將其儲存
    為 SecretRef，而不是純文字。
  </Step>
</Steps>

<Note>
如果閘道僅限迴路介面且無法探索，請手動使用 SSH 通道或 tailnet。
純文字 `ws://` 可用於迴路介面、私有 IP 常值、`.local` 和 Tailnet `*.ts.net` URL；其他私有 DNS 名稱需要 `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`。
</Note>

## 驗證與模型選項

如果供應商設定步驟在互動式新手引導中失敗（例如在本機未登入時選擇重複使用命令列介面），
精靈會顯示錯誤並返回供應商選擇器，
而不是結束。明確的 `--auth-choice` 執行仍會快速失敗，以供自動化使用。

<AccordionGroup>
  <Accordion title="Anthropic API 金鑰">
    如果 `ANTHROPIC_API_KEY` 存在則使用它，否則提示輸入金鑰，然後儲存以供常駐程式使用。
  </Accordion>
  <Accordion title="Anthropic Claude 命令列介面">
    互動式新手引導／設定中的首選本機路徑；若有現有的 Claude 命令列介面登入，便會重複使用。
  </Accordion>
  <Accordion title="OpenAI Code 訂閱（OAuth）">
    瀏覽器流程；貼上 `code#state`。

    在沒有主要模型的全新設定中，透過 Codex 執行階段將 `agents.defaults.model`
  ...
  <Accordion title="OpenCode">
    提示輸入 `OPENCODE_API_KEY`（或 `OPENCODE_ZEN_API_KEY`），並讓你選擇 Zen 或 Go 目錄（一組 API 金鑰可同時涵蓋兩者）。
  ...
  <Accordion title="MiniMax">
    系統會自動寫入設定。託管服務預設值為 `MiniMax-M3`；API 金鑰設定使用
    `minimax/...`，OAuth 設定則使用 `minimax-portal/...`。
    更多詳細資訊：[MiniMax](/zh-TW/providers/minimax)。
  </Accordion>
  <Accordion title="StepFun">
    系統會針對中國或全球端點上的 StepFun standard 或 Step Plan 自動寫入設定。
  ...
  <Accordion title="自訂供應商">
    適用於 OpenAI 相容、OpenAI Responses 相容及 Anthropic 相容端點。

    互動式初始設定支援與其他供應商 API 金鑰流程相同的 API 金鑰儲存選項：
    - **立即貼上 API 金鑰**（純文字）
    - **使用密鑰參照**（環境變數參照或已設定的供應商參照，並執行預檢驗證）

    初始設定會推斷常見視覺模型 ID（GPT-4o/4.1/5.x、Claude 3/4、Gemini、Qwen-VL、LLaVA、Pixtral 及類似模型）的影像支援能力，只有在模型名稱未知時才會詢問。
...
- 在非互動式模式中，`--secret-input-mode ref` 僅支援環境變數。
  - 請在初始設定程序的環境中設定供應商環境變數。
  - 行內金鑰旗標（例如 `--openai-api-key`）要求必須設定該環境變數；否則初始設定會立即失敗。
...
- 閘道驗證認證資訊在互動式設定中支援純文字和 SecretRef 選項：
  ...
  - 密碼模式：純文字或 SecretRef。
- 非互動式權杖 SecretRef 路徑：`--gateway-token-ref-env <ENV_VAR>`。
- 現有純文字設定會繼續運作，不受任何變更影響。

<Note>
無介面與伺服器提示：請在具有瀏覽器的電腦上完成 OAuth，然後將
...
- `tools.profile`（未設定時，本機初始設定預設為 `"coding"`；現有明確值會保留）
- `gateway.*`（模式、繫結、驗證、Tailscale）
- `session.dmScope`（初始設定會保留明確值，否則維持未設定，因此 `main` 預設值會在代理程式的滾動主要工作階段中保留所有頻道的直接訊息——這是個人代理程式的預設值。對於共用或多使用者收件匣，請使用 `per-channel-peer`；當 `openclaw security audit` 偵測到多使用者私訊流量時，會建議進行隔離）
...
WhatsApp 認證資訊會放在 `~/.openclaw/credentials/whatsapp/<accountId>/` 下。
使用中的工作階段和逐字稿儲存在
...
<Note>
部分頻道以外掛形式提供。在設定期間選取這些頻道時，精靈會在進行頻道設定前
提示安裝外掛（npm 或本機路徑）。
</Note>

### 已安裝應用程式建議

模型存取檢查成功後，macOS 上的傳統互動式初始設定會掃描應用程式名稱和套件 ID，而不要求 macOS 隱私權限。它會搜尋官方外掛目錄和 ClawHub，接著要求已設定的模型排除錯誤的名稱比對，並建議相關的外掛或 Skills。建議的比對結果預設為選取；選用的比對結果需要明確選取。

結果畫面會列出偵測到的應用程式，並顯示：“使用你設定的模型和 ClawHub 搜尋來比對應用程式名稱。”將 `wizard.appRecommendations` 設為 `false`，即可同時停用此初始設定步驟，以及閘道對節點應用程式清單的存取。快速入門或非 macOS 初始設定不會使用此掃描。

## 非互動式設定

`--non-interactive` 需要 `--accept-risk`（確認代理程式功能強大，且完整系統存取具有風險）：
...
完整旗標參考資料與供應商專屬範例：[`openclaw onboard`](/zh-TW/cli/onboard)、[命令列介面自動化](/zh-TW/start/wizard-cli-automation)。
...
用戶端（macOS 應用程式和控制介面）可以呈現步驟，而不需重新實作初始設定邏輯。

## Signal 設定行為

- 從官方 `signal-cli` GitHub 發行版本下載適當的發行資產（原生組建，僅限 Linux x86-64）
- 在其他平台（macOS、非 x64 Linux）上，改為透過 Homebrew 安裝
- 將發行資產安裝儲存在 `~/.openclaw/tools/signal-cli/<version>/` 下
- 在設定中寫入 `channels.signal.transport.cliPath`，其值為 `kind: "managed-native"`
