...
read_when:
    - 執行本機個人代理程式可靠性檢查
    - 擴充由儲存庫支援的 QA 情境目錄
    - 驗證提醒、回覆、記憶、遮蔽、安全工具後續操作、任務狀態、可安全分享的診斷資訊、有證據支持的完成聲明，以及失敗復原
summary: 用於隱私保護個人助理工作流程檢查的本機 qa-channel 情境。
title: 個人代理程式基準測試套件
x-i18n:
    generated_at: "2026-07-26T07:40:13Z"
    model: gpt-5.6
...
不需要新的執行器：它會重複使用私有 QA 堆疊（[QA 概覽](/zh-TW/concepts/qa-e2e-automation)）、
...
`--pack` 可透過重複使用 `--scenario` 旗標累加。明確指定的情境會
...
情境只會使用虛假使用者、虛假偏好設定、虛假機密，以及
套件建立的暫時 QA 閘道工作區。它們不得讀取或
寫入真實 OpenClaw 使用者的記憶、工作階段、認證資訊、啟動代理程式、全域
設定或即時閘道狀態。

成品會保留在現有的 QA 套件成品目錄下，並視為
測試輸出。遮蔽檢查會使用虛假標記，因此可安全地
檢查失敗並將其提交至議題。
...
良好的後續候選項目：已遮蔽的軌跡匯出檢查、僅限本機的
外掛工作流程檢查。

在情境目錄擁有足夠穩定的案例，足以證明新增介面的合理性之前，
