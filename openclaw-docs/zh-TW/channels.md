...
title: 聊天頻道
x-i18n:
    generated_at: "2026-07-26T08:06:56Z"
    model: gpt-5.6
...
OpenClaw 可以在你已使用的任何聊天應用程式上與你交談。每個頻道都透過閘道連線。
所有頻道都支援文字；媒體與回應功能則依頻道而異。

iMessage、Telegram 與 WebChat 使用者介面隨核心安裝一併提供。標示為
“官方外掛”的頻道可使用一個命令安裝（`openclaw plugins install @openclaw/<id>`）
或在 `openclaw onboard` / `openclaw channels add` 期間視需要安裝，之後必須重新啟動閘道。
“外部外掛”頻道則由 OpenClaw 儲存庫之外的團隊維護。
...
- [iMessage](/zh-TW/channels/imessage) - 包含於核心中。透過已登入的 Mac 上的 `imsg` 橋接器進行原生 macOS 整合（若閘道在其他位置執行，則使用 SSH 包裝器），包括回覆、點按回應、特效、附件與群組管理等私有 API 操作。
...
- [QQ Bot](/zh-TW/channels/qqbot) - QQ Bot API；支援私人聊天、群組聊天與多媒體內容（官方外掛）。
- [Reef](/zh-TW/channels/reef) - 在不同使用者的 OpenClaw 代理程式之間，提供受防護且端對端加密的 claw 對 claw 訊息傳遞（內附外掛）。
...
- [Tlon](/zh-TW/channels/tlon) - 以 Urbit 為基礎的通訊工具（官方外掛）。
...
- [WebChat](/zh-TW/web/webchat) - 包含於核心中。透過 WebSocket 使用閘道 WebChat 使用者介面。
- [微信](/zh-TW/channels/wechat) - 透過 QR 登入使用騰訊 iLink 機器人；僅支援私人聊天（外部外掛）。
...
- [Zalo ClawBot](/zh-TW/channels/zaloclawbot) - 透過 QR 登入使用個人 Zalo 助理；與擁有者綁定（外部外掛）。
- [Zalo Personal](/zh-TW/channels/zalouser) - 透過 QR 登入使用 Zalo 個人帳號（官方外掛）。

## 傳送注意事項

- Telegram 回覆若包含 Markdown 圖片語法（例如 `![alt](url)`），
  在可行時，會於最後的傳出路徑上轉換為媒體回覆。
- Slack 多人私訊會以群組聊天方式路由，因此群組政策、提及
  行為與群組工作階段規則也適用於 MPIM 對話。
- WhatsApp 設定採視需要安裝：外掛套件安裝前，新手引導即可顯示設定流程，
  且只有在頻道實際啟用時，閘道才會載入外部
  ClawHub/npm 外掛。
- 接受機器人撰寫之傳入訊息的頻道可以使用共用的
  [機器人迴圈防護](/zh-TW/channels/bot-loop-protection)，避免機器人配對彼此無限回覆。
- 支援的常駐聊天室可以使用[環境聊天室事件](/zh-TW/channels/ambient-room-events)，
  讓未提及代理程式的聊天室對話成為靜默內容，除非代理程式使用
  `message` 工具傳送訊息。

## 注意事項

- 頻道可同時執行；設定多個頻道後，OpenClaw 會依聊天路由。
- 最快的設定方式通常是 **Telegram**（只需簡單的機器人權杖，無須安裝外掛）。WhatsApp
  需要 QR 配對，並會在磁碟上儲存較多狀態。
- 群組行為依頻道而異；請參閱[群組](/zh-TW/channels/groups)。
- 為了安全，系統會強制執行私訊配對與允許清單；請參閱[安全性](/zh-TW/gateway/security)。
- 疑難排解：[頻道疑難排解](/zh-TW/channels/troubleshooting)。
- 模型提供者另有說明文件；請參閱[模型提供者](/zh-TW/providers/models)。
