...
read_when:
    ...
    - Ви хочете розгортання в хмарі в один клік із браузерним Control UI
summary: Розгорніть OpenClaw на Northflank за допомогою шаблону one-click
title: Northflank
x-i18n:
...
Розгорніть OpenClaw на Northflank за допомогою шаблону one-click і отримайте доступ через вебінтерфейс Control UI.
...
- Постійне сховище через Northflank Volume (`/data`), тож `openclaw.json`,
  `auth-profiles.json` для кожного агента, стан каналу/провайдера, сесії та
  робочий простір зберігаються після повторних розгортань
