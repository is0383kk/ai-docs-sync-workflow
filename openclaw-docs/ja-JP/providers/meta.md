...
title: Meta
x-i18n:
    generated_at: "2026-07-26T09:42:12Z"
    model: gpt-5.6
...
**Meta API** は、`muse-spark-1.1` 推論モデルに OpenAI 互換の **Responses API**（`POST /v1/responses`）を使用します。このプロバイダーは、OpenClaw に同梱される Plugin として提供されます。
...
```
  ...
  <Step title="モデルが利用可能であることを確認する">
    ...
    静的な `muse-spark-1.1` カタログエントリを一覧表示します。`MODEL_API_KEY` を解決できない場合、
    `openclaw models status --json` は不足している認証情報を
    `auth.unusableProfiles` の下に報告します。
...
<Warning>
`muse-spark-1.1` は `reasoning.effort: "none"` を受け付けません。OpenClaw は、このプロバイダーでは
`--thinking off` を `minimal` にマッピングします。
</Warning>
