---
read_when:
    - fal Plugin のインストール、設定、または監査を行っている場合
summary: OpenClaw に fal モデルプロバイダーのサポートを追加します。
title: fal Plugin
x-i18n:
    generated_at: "2026-07-26T10:24:43Z"
    model: gpt-5.6
    postprocess_version: locale-links-v1
    prompt_version: 32
    provider: openai
    source_hash: 5341c85c74911d5d49849cfbee4d31a7a8bdf38a24ca68566118f4a0ced81d2e
    source_path: plugins/reference/fal.md
    workflow: 16
---

# fal Plugin

OpenClaw に fal モデルプロバイダーのサポートを追加します。

## 配布

- パッケージ: `@openclaw/fal-provider`
- インストール経路: OpenClaw に同梱

## サーフェス

プロバイダー: `fal`; コントラクト: `imageGenerationProviders`, `musicGenerationProviders`, `videoGenerationProviders`

## 関連ドキュメント

- [fal](/ja-JP/providers/fal)
