---
read_when:
    - モデルまたは埋め込みプロバイダーが選択された場合にのみ、OpenClaw がローカルモデルサーバーを起動するようにしたい場合
...
title: ローカルモデルサービス
x-i18n:
    generated_at: "2026-07-26T10:13:46Z"
    model: gpt-5.6
...
`command` を、OpenClaw を実行しているマシン上での `which inferrs` の結果に置き換えます。Inferrs の完全なセットアップ: [Inferrs](/ja-JP/providers/inferrs)。
...
完全なセットアップ、コンテキストサイズの設定、および検証コマンドについては、[ds4](/ja-JP/providers/ds4) を参照してください。
