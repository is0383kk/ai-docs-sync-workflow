...
ここに短く明確な起動手順を追加します。バンドルされている `boot-md` フックは、このファイルが存在し、空白以外の内容が含まれている場合、Gateway が起動するたびに、エージェントワークスペースごとにこのファイルを1回実行します。複数のエージェントがワークスペースを共有している場合も、実行は1回だけです。
...
チェックリスト項目でメッセージを送信する場合は、メッセージツールを使用してから、正確なサイレントトークン `NO_REPLY` を返信します（大文字と小文字は区別されません）。
