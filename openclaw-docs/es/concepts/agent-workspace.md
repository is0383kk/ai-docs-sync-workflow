---
read_when:
    - Debe explicar el espacio de trabajo del agente o su estructura de archivos
    - Se desea realizar una copia de seguridad o migrar un espacio de trabajo de agente
...
x-i18n:
    generated_at: "2026-07-26T05:10:04Z"
    model: gpt-5.6
...
<Warning>
El espacio de trabajo es el **cwd predeterminado**, no un entorno aislado estricto. Las herramientas resuelven las rutas relativas con respecto al espacio de trabajo, pero las rutas absolutas aún pueden acceder a otras ubicaciones del host, salvo que el aislamiento esté habilitado. Si necesita aislamiento, use [`agents.defaults.sandbox`](/es/gateway/sandboxing) (o la configuración de aislamiento por agente).

Cuando el aislamiento está habilitado y `workspaceAccess` no es `"rw"`, las herramientas funcionan dentro de un espacio de trabajo aislado en `~/.openclaw/sandboxes`, no en el espacio de trabajo del host.
</Warning>
...
- `OPENCLAW_WORKSPACE_DIR` sustituye a ambos valores anteriores cuando está establecido.
...
Sustitución por agente: `agents.entries.*.workspace`.

`openclaw onboard`, `openclaw configure` o `openclaw setup` crean el espacio de trabajo y generan los archivos de inicialización si faltan.
...
Si ya gestiona por su cuenta los archivos del espacio de trabajo, deshabilite la creación de archivos de inicialización:
...
Es posible que instalaciones anteriores hayan creado `~/openclaw`. Mantener varios directorios de espacio de trabajo puede provocar confusiones en la autenticación o divergencias de estado, ya que solo puede haber un espacio de trabajo activo a la vez.

<Note>
**Recomendación:** mantenga un único espacio de trabajo activo. Si ya no utiliza las carpetas adicionales, archívelas o muévalas a la papelera (por ejemplo, `trash ~/openclaw`). Si mantiene intencionadamente varios espacios de trabajo, asegúrese de que `agents.defaults.workspace` (o la clave por agente `workspace`) apunte al espacio activo.
</Note>
...
<AccordionGroup>
  <Accordion title="AGENTS.md: instrucciones de funcionamiento">
    Instrucciones de funcionamiento para el agente y sobre cómo debe utilizar la memoria. Se carga al inicio de cada sesión. Es un buen lugar para las reglas, las prioridades y los detalles sobre cómo comportarse.
  </Accordion>
  ...
  <Accordion title="HEARTBEAT.md: lista de comprobación de Heartbeat">
    Pequeña lista de comprobación opcional para las ejecuciones de Heartbeat. Manténgala breve para evitar consumir tokens.
  </Accordion>
  <Accordion title="BOOT.md: lista de comprobación de inicio">
    Lista de comprobación de inicio opcional que se ejecuta automáticamente al reiniciar el Gateway (cuando están habilitados los [hooks internos](/es/automation/hooks)). Manténgala breve; utilice la herramienta de mensajes para los envíos salientes.
  </Accordion>
  ...
  <Accordion title="MEMORY.md: memoria a largo plazo seleccionada (opcional)">
    Memoria a largo plazo seleccionada: hechos duraderos, preferencias, decisiones y resúmenes breves. Mantenga los registros detallados en `memory/YYYY-MM-DD.md` para que las herramientas de memoria puedan recuperarlos cuando sea necesario sin insertarlos en cada prompt. Cargue `MEMORY.md` únicamente en la sesión principal privada (no en contextos compartidos o grupales). Consulte [Memoria](/es/concepts/memory) para conocer el flujo de trabajo y el volcado automático de memoria.
  </Accordion>
  <Accordion title="skills/: Skills del espacio de trabajo (opcional)">
    Skills específicas del espacio de trabajo. Es la ubicación de Skills con mayor prioridad para ese espacio de trabajo, por delante de las Skills del agente del proyecto, las Skills personales del agente, las Skills administradas, las Skills incluidas y `skills.load.extraDirs` cuando coinciden los nombres.
  </Accordion>
  <Accordion title="canvas/: archivos de la interfaz Canvas (opcional)">
    Archivos de la interfaz Canvas para las pantallas de los Node (por ejemplo, `canvas/index.html`).
  </Accordion>
</AccordionGroup>

<Note>
Si falta un archivo de inicialización, OpenClaw inserta un marcador de «archivo ausente» en la sesión y continúa. Los archivos de inicialización grandes se truncan al insertarse; ajuste los límites con `agents.defaults.bootstrapMaxChars` (valor predeterminado: `20000`) y `agents.defaults.bootstrapTotalMaxChars` (valor predeterminado: `60000`). `openclaw setup` puede volver a crear los archivos predeterminados que falten sin sobrescribir los existentes.
</Note>
...
- `~/.openclaw/state/openclaw.sqlite` (estado y certificaciones de la configuración del espacio de trabajo compartido)
- `~/.openclaw/agents/<agentId>/agent/auth-profiles.json` (perfiles de autenticación de modelos: OAuth y claves de API)
- `~/.openclaw/agents/<agentId>/agent/openclaw-agent.sqlite` (filas de sesiones, transcripciones y estado de ejecución por agente)
- `~/.openclaw/agents/<agentId>/agent/codex-home/` (cuenta de ejecución de Codex por agente, configuración, Skills, plugins y estado nativo de los hilos)
- `~/.openclaw/credentials/` (estado del canal o proveedor, además de datos heredados de importación de OAuth)
...
sobre uno de estos archivos, ejecute `openclaw doctor --fix`; Doctor importa el estado
...
Trate el espacio de trabajo como memoria privada. Guárdelo en un repositorio Git **privado** para disponer de una copia de seguridad y poder recuperarlo.

Ejecute estos pasos en la máquina donde se ejecuta el Gateway (allí es donde reside el espacio de trabajo).

<Steps>
  <Step title="Inicializar el repositorio">
    Si Git está instalado, los espacios de trabajo completamente nuevos se inicializan automáticamente. Si este espacio de trabajo todavía no es un repositorio, ejecute:
...
Si debe almacenar referencias confidenciales, utilice marcadores de posición y mantenga el secreto real en otro lugar (un gestor de contraseñas, variables de entorno o `~/.openclaw/`).
</Warning>
...
<Steps>
  <Step title="Clonar el repositorio">
    Clone el repositorio en la ruta deseada (de forma predeterminada, `~/.openclaw/workspace`).
  </Step>
...
- El enrutamiento multiagente puede utilizar distintos espacios de trabajo por agente mediante `agents.entries.*.workspace`. Consulte [Enrutamiento de canales](/es/channels/channel-routing) para conocer la configuración del enrutamiento.
- Si `agents.defaults.sandbox` está habilitado, las sesiones que no sean la principal pueden utilizar espacios de trabajo aislados por sesión dentro de `agents.defaults.sandbox.workspaceRoot`.
