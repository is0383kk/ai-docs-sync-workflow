...
read_when:
    - Quieres comprender qué herramientas proporciona OpenClaw
...
x-i18n:
    generated_at: "2026-07-26T04:55:08Z"
    model: gpt-5.6
...
Use esta página para elegir la superficie de capacidades adecuada. Las **herramientas** son
...
Esta es una página de descripción general y orientación. Para consultar de forma exhaustiva las políticas de herramientas, los valores predeterminados,
la pertenencia a grupos, las restricciones de proveedores y los campos de configuración, consulte
[Herramientas y proveedores personalizados](/es/gateway/config-tools).

## Empiece aquí

Para la mayoría de los agentes, empiece con las categorías de herramientas integradas y, después, ajuste la política
solo cuando el agente deba ver menos herramientas o necesite acceso explícito al host.

| Si necesita...                                          | Use esto primero                                      | Después, consulte                                                                                                                                                        |
| ------------------------------------------------------- | ----------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Coordinar varios agentes o entornos de ejecución         | [Subagentes](/es/tools/subagents)                        | [Agentes ACP](/es/tools/acp-agents) y [Envío de agentes](/es/tools/agent-send)                                                                                                |
| Orquestar agentes simultáneos desde código               | [Swarm](/es/tools/swarm)                                 | [Modo de código](/es/tools/code-mode) y [Subagentes](/es/tools/subagents)                                                                                                     |
...
<Steps>
  <Step title="Use una herramienta cuando el agente necesite actuar">
  ...
  <Step title="Use una Skill cuando el agente necesite instrucciones">
    Una Skill es un paquete de instrucciones `SKILL.md` que se carga en el prompt del agente. Use
    ...
    Skills de OpenClaw o un paquete de plugin.
  ...
  <Step title="Use un plugin cuando OpenClaw necesite una nueva capacidad">
    ...
    capacidades de tiempo de ejecución. Use un plugin cuando la capacidad incluya código,
    credenciales, hooks de ciclo de vida, metadatos de manifiesto o
    empaquetado instalable. Los plugins existentes se pueden instalar desde ClawHub, npm, git,
...
permisos y denegaciones, consulte [Herramientas y proveedores personalizados](/es/gateway/config-tools).

| Categoría                    | Úsela cuando el agente necesite...                                                                      | Herramientas representativas                                                                                          | Consulte a continuación                                                                                                          |
| ---------------------------- | ------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| Tiempo de ejecución          | Ejecutar comandos, gestionar procesos o usar análisis de Python respaldado por un proveedor             | `exec`, `process`, `terminal`, `code_execution`                                                                     | [Exec](/es/tools/exec), [Terminal de la interfaz de control](/es/web/control-ui#operator-terminal), [Ejecución de código](/es/tools/code-execution) |
...
| Interfaz del operador        | Organizar paneles, vistas y navegación conectados de la interfaz de control                            | `screen`                                                                                                            | [Pantalla](/es/tools/screen)                                                                                                        |
| Mensajería y canales         | Enviar respuestas o acciones de canal                                                                  | `message`                                                                                                           | [Envío de agentes](/es/tools/agent-send)                                                                                            |
| Sesiones y agentes           | Inspeccionar sesiones, delegar trabajo, orquestar recopiladores, dirigir otra ejecución o informar del estado | `sessions_*`, `agents_wait`, `subagents`, `agents_list`, `session_status`, `get_goal`, `create_goal`, `update_goal` | [Objetivo](/es/tools/goal), [Swarm](/es/tools/swarm), [Subagentes](/es/tools/subagents), [Herramienta de sesión](/es/concepts/session-tool)     |
...
<Note>
El Modo de código y la Búsqueda de herramientas son superficies experimentales para agentes de OpenClaw. Las ejecuciones del entorno
Codex usan el modo de código nativo de Codex, la búsqueda de herramientas nativa, herramientas dinámicas
...
`api.registerTool(...)` y el campo `contracts.tools` del manifiesto; consulte
[SDK de plugins](/es/plugins/sdk-overview) y [Manifiesto de plugins](/es/plugins/manifest)
para obtener los detalles del contrato.
...
- [Mostrar widget](/es/tools/show-widget) para SVG y HTML integrados y autocontenidos en clientes de chat compatibles
...
- [Tokenjuice](/es/tools/tokenjuice) para compactar la salida ruidosa de las herramientas
  `exec` y `bash`
...
- [Canvas](/es/plugins/reference/canvas) para controlar Canvas de Node y renderizar
  A2UI
...
debido a la configuración global, la configuración por agente, la política del canal, las restricciones
...
5. Para las ejecuciones delegadas, compruebe las restricciones por agente en
   [Restricciones de sandbox y herramientas por agente](/es/tools/multi-agent-sandbox-tools).
6. Para catálogos grandes de OpenClaw, confirme si la ejecución usa la exposición directa de herramientas,
   el [modo de código](/es/tools/code-mode) o la [búsqueda de herramientas](/es/tools/tool-search).
...
- [Skills](/es/tools/skills) para el orden de carga, las condiciones de activación y la configuración de las Skills
- [Taller de Skills](/es/tools/skill-workshop) para crear Skills generadas y revisadas
- [Búsqueda de herramientas](/es/tools/tool-search) para descubrir de forma compacta el catálogo de herramientas
  de OpenClaw
...
- [Enjambre](/es/tools/swarm) para la distribución y recopilación estructuradas desde el modo de código
