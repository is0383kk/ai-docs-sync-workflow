---
read_when:
    - Quiere un diagnóstico rápido del estado de los canales y de los destinatarios de las sesiones recientes
    - Se necesita un estado «all» que se pueda pegar para la depuración
summary: Referencia de la CLI para `openclaw status` (diagnósticos, sondeos, instantáneas de uso)
title: openclaw status
x-i18n:
    generated_at: "2026-07-26T05:07:13Z"
    model: gpt-5.6
...
Diagnósticos de canales y sesiones.
...
| Opción                  | Descripción                                                                                                                  |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `--all`                 | Diagnóstico completo (de solo lectura y apto para copiar y pegar). Incluye auditoría de seguridad, compatibilidad de plugins y comprobaciones de vectores de memoria. |
| `--deep`                | Ejecuta comprobaciones en vivo (WhatsApp Web + Telegram + Discord + Slack + Signal). También activa la auditoría de seguridad. |
...
`not checked` en lugar de no disponible cuando omite la inspección de memoria. La auditoría
de seguridad exhaustiva, la compatibilidad de plugins y las comprobaciones de vectores de memoria se reservan para
...
- La salida del estado de la sesión distingue `Execution:` de `Runtime:`. `Execution`
  ...
  [Entornos de ejecución de agentes](/es/concepts/agent-runtimes) para conocer la distinción entre proveedor, modelo y entorno
  de ejecución.
- Cuando la instantánea de la sesión actual contiene pocos datos, `/status` puede completar los contadores
  de tokens y caché a partir del registro de uso de la transcripción más reciente. Los valores
  en vivo distintos de cero siguen teniendo prioridad sobre los valores alternativos de la transcripción.
- La alternativa basada en la transcripción también puede recuperar la etiqueta del modelo del entorno de ejecución activo cuando
  no aparece en la entrada de la sesión en vivo. Si ese modelo de la transcripción difiere
  del modelo seleccionado, el estado resuelve la ventana de contexto según el
  modelo de entorno de ejecución recuperado en lugar del seleccionado.
- Para contabilizar el tamaño del prompt, la alternativa basada en la transcripción prioriza el total
  orientado al prompt que sea mayor cuando faltan metadatos de sesión o su valor es inferior, de modo que
...
- Los campos sin procesar `usage_percent` / `usagePercent` de MiniMax representan la cuota restante,
  ...
  están presentes. Las respuestas de `model_remains` priorizan la entrada del modelo de chat, derivan la
...
- El resumen incluye el estado de instalación y ejecución del servicio de host del Gateway y el nodo cuando
...
- Cuando el Gateway en ejecución tiene algún propietario de SecretRef aislado durante el inicio, la recarga o una escritura de configuración, el estado incluye `degradedSecretOwners` en JSON y una fila de resumen **Secretos degradados** en la salida para personas. Cada entrada indica el propietario, el estado de degradación (`cold` o `stale`), las rutas de configuración y el motivo censurado. Los propietarios inactivos no están disponibles; los propietarios obsoletos continúan con los últimos valores válidos conocidos.
...
- Si se configura una SecretRef de canal compatible, pero no está disponible en la
  ...
  `secretDiagnostics`.
- Cuando la resolución local de SecretRef del comando se realiza correctamente, el estado prioriza la
  instantánea resuelta y elimina de la salida final los marcadores transitorios del canal
  que indican «secreto no disponible».
...
`status --json --all` informa de los detalles de la memoria desde el entorno de ejecución del plugin de memoria activo
seleccionado por `plugins.slots.memory`. Los plugins de memoria personalizados pueden mantener
desactivado el componente integrado `memory.search.enabled` y aun así informar
