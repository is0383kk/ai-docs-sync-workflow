...
read_when:
    ...
    - Primero se quiere una ejecución de prueba
...
elimina; desinstálela por separado mediante npm/pnpm.
...
Si no se especifican indicadores de alcance, una selección múltiple interactiva pregunta qué componentes
se deben eliminar (de forma predeterminada, el servicio, el estado y el espacio de trabajo aparecen preseleccionados).
...
- Ejecute primero `openclaw backup create` para crear una instantánea restaurable antes de eliminar
  el estado o los espacios de trabajo.
- `--state` conserva los directorios del espacio de trabajo configurados, salvo que también se
