---
read_when:
    - Trabajar en el código o las pruebas del entorno de ejecución de agentes de OpenClaw
    - Ejecución de los flujos de lint, comprobación de tipos y pruebas en vivo de agent-runtime
...
x-i18n:
    generated_at: "2026-07-26T05:17:23Z"
    model: gpt-5.6
...
- Comprobación local predeterminada: `pnpm check` (comprobación de tipos, linting, controles de políticas)
- Comprobación de compilación: `pnpm build` cuando el cambio puede afectar al resultado de la compilación, el empaquetado o los límites de carga diferida/módulos
- Comprobación completa previa al envío: `pnpm build && pnpm check && pnpm check:test-types && pnpm test`

## Ejecución de pruebas del entorno de ejecución del agente
...
El primer patrón glob también incluye los conjuntos `agent-tools*`, `agent-settings` y
...
envoltorio de pruebas en vivo (establece `OPENCLAW_LIVE_TEST=1` y requiere credenciales del proveedor):
...
Para comprobar el comportamiento de las llamadas a herramientas, solicite una acción `read` o `exec` para poder observar
la transmisión de las herramientas y el procesamiento de la carga útil.

## Restablecimiento desde cero

El estado se almacena en el directorio de estado de OpenClaw: `~/.openclaw` de forma predeterminada, o
...
| `agents/<agentId>/agent/openclaw-agent.sqlite` | Perfiles de autenticación de modelos por agente (claves de API + OAuth) y estado del entorno de ejecución |
...
- Solo sesiones: no elimine `agents/<agentId>/agent/openclaw-agent.sqlite`; las filas de las sesiones se almacenan allí junto con otros estados por agente. Use `/new` o `/reset` para iniciar una sesión nueva para un chat y `openclaw sessions cleanup` para el mantenimiento de las sesiones.
- Conservar la autenticación: mantenga `agents/<agentId>/agent/openclaw-agent.sqlite` y `credentials/` en su lugar.
...
- [Arquitectura del entorno de ejecución de agentes de OpenClaw](/es/agent-runtime-architecture)
