---
read_when:
    - Está instalando, configurando o auditando el plugin featherless
summary: Plugin del proveedor Featherless AI para OpenClaw.
title: Plugin Featherless
x-i18n:
    generated_at: "2026-07-26T04:48:03Z"
    model: gpt-5.6
    postprocess_version: locale-links-v1
    prompt_version: 32
    provider: openai
    source_hash: cc85911d6fa5ee416b1abd11bd08466d1e8d245ad3edb03e315fb555b4ed38b3
    source_path: plugins/reference/featherless.md
    workflow: 16
---

# Plugin Featherless

Plugin del proveedor Featherless AI para OpenClaw.

## Distribución

- Paquete: `@openclaw/featherless-provider`
- Método de instalación: npm; ClawHub: `clawhub:@openclaw/featherless-provider`

## Superficie

proveedores: `featherless`

## Documentación relacionada

- [featherless](/es/providers/featherless)
