...
title: Venice AI
x-i18n:
    generated_at: "2026-07-26T05:27:52Z"
    model: gpt-5.6
...
| --------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------- |
| **Privado**     | Los prompts y las respuestas nunca se almacenan ni registran. Efímero. | Llama, Qwen, DeepSeek, Kimi, MiniMax, Venice Uncensored, etc. |
| **Anonimizado** | Se envían mediante Venice, que elimina los metadatos antes de reenviarlos. | Claude, GPT, Gemini, Grok                                  |

<Warning>
Los modelos anonimizados no son completamente privados. Venice elimina los metadatos antes de reenviar la solicitud, pero el proveedor subyacente (OpenAI, Anthropic, Google, xAI) sigue procesándola. Utilice modelos privados cuando se requiera privacidad total.
</Warning>
  ...
  <Step title="Configurar OpenClaw">
    <Tabs>
      <Tab title="Interactivo (recomendado)">
        ...
        Solicita la clave de API (o reutiliza un `VENICE_API_KEY` existente), enumera los modelos de Venice disponibles y establece el modelo predeterminado.
      </Tab>
...
También se puede ejecutar `openclaw configure` y seleccionar **Proveedor de modelo/autenticación > Venice AI**.
...
<AccordionGroup>
  <Accordion title="Modelos privados (20) — totalmente privados, sin registros">
...
compatibilidad de esquema de herramientas que el proveedor xAI nativo, ya que comparten el mismo formato
ascendente de llamadas a herramientas.

## Detección de modelos

El catálogo incluido anteriormente es una lista inicial respaldada por un manifiesto. Durante la ejecución, OpenClaw
lo actualiza desde la API `/models` de Venice y recurre a la lista inicial si
la API no está disponible. El endpoint `/models` es público (no se necesita autenticación para
enumerar), pero la inferencia requiere una clave de API válida.

Venice puede seguir aceptando ID de modelos retirados como alias gestionados por el proveedor. El
catálogo de OpenClaw anuncia únicamente los ID de modelo canónicos devueltos por `/models`.

## Comportamiento de reproducción de DeepSeek V4

Si Venice expone modelos DeepSeek V4 como `deepseek-v4-pro` o
`deepseek-v4-flash`, OpenClaw completa el campo de reproducción obligatorio
...
| Llamadas a funciones | La mayoría de los modelos; deshabilitadas por modelo cuando se indica anteriormente |
...
los precios directos de la API, más una pequeña tarifa de Venice. Consulte
...
<AccordionGroup>
  ...
  <Accordion title="El modelo no está disponible">
    Ejecute `openclaw models list --all --provider venice` para consultar los modelos
  ...
  <Accordion title="Problemas de conexión">
    La API de Venice se encuentra en `https://api.venice.ai/api/v1`. Confirme que la red permite conexiones HTTPS con ese host.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Precios" href="https://venice.ai/pricing" icon="credit-card">
    Tarifas de crédito y planes actuales de Venice.
  </Card>
</CardGroup>
