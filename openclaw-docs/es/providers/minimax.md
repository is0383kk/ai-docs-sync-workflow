...
title: MiniMax
x-i18n:
    generated_at: "2026-07-26T05:19:09Z"
    model: gpt-5.6
...
| `image-01`               | Generación de imágenes | Generación de texto a imagen y edición de imagen a imagen |
...
<Tabs>
  ...
  <Tab title="Clave de API">
    **Recomendado para:** MiniMax alojado con una API compatible con Anthropic.
    ...
    <Warning>
    El endpoint de streaming compatible con Anthropic de MiniMax-M2.x emite `reasoning_content` en fragmentos delta al estilo de OpenAI en lugar de bloques de pensamiento nativos de Anthropic, lo que expone el razonamiento interno en la salida visible si el pensamiento queda habilitado implícitamente. OpenClaw deshabilita de forma predeterminada el pensamiento de M2.x, salvo que se establezca explícitamente `thinking`. MiniMax-M3 (y M3.x con compatibilidad futura) está exento: M3 emite bloques de pensamiento de Anthropic adecuados y requiere que el pensamiento esté activo para producir contenido visible, por lo que OpenClaw mantiene M3 en la ruta de pensamiento adaptativo del proveedor. Consulte la sección Valores predeterminados del pensamiento en Configuración avanzada, más adelante.
    </Warning>
...
El plugin MiniMax registra el modelo `image-01` para la herramienta `image_generate` tanto en `minimax` como en `minimax-portal`, reutilizando la misma autenticación `MINIMAX_API_KEY` o OAuth que los modelos de texto.
...
La generación de imágenes siempre usa el endpoint dedicado de imágenes de MiniMax (`/v1/image_generation`) e ignora `models.providers.minimax.baseUrl`, ya que ese campo configura la URL base de chat compatible con Anthropic. Establezca `MINIMAX_API_HOST=https://api.minimaxi.com` para dirigir la generación de imágenes mediante el endpoint de China; el endpoint global predeterminado es `https://api.minimax.io`.
...
- ID de modelos incluidos: `speech-2.8-hd`, `speech-2.8-turbo`, `speech-2.6-hd`, `speech-2.6-turbo`, `speech-02-hd`, `speech-02-turbo`, `speech-01-hd`, `speech-01-turbo`
- Orden de resolución de autenticación: `tts.providers.minimax.apiKey`, después los perfiles de autenticación OAuth/token de `minimax-portal`, luego las claves de entorno de Token Plan (`MINIMAX_OAUTH_TOKEN`, `MINIMAX_CODE_PLAN_KEY`, `MINIMAX_CODING_API_KEY`) y, por último, `MINIMAX_API_KEY`
- Si no se configura ningún host TTS, OpenClaw reutiliza el host OAuth configurado de `minimax-portal` y elimina los sufijos de ruta compatibles con Anthropic, como `/anthropic`
- Los archivos adjuntos de audio normales se mantienen en MP3. Los destinos de notas de voz (Feishu, Telegram y otros canales que solicitan un archivo adjunto compatible con notas de voz) se transcodifican de MP3 de MiniMax a Opus de 48kHz con `ffmpeg`, porque, por ejemplo, la API de archivos de Feishu/Lark solo acepta `file_type: "opus"` para mensajes de audio nativos
...
| `tts.providers.minimax.model`   | `MINIMAX_TTS_MODEL`    | `speech-2.8-hd`               | ID del modelo TTS.                               |
| `tts.providers.minimax.voiceId` | `MINIMAX_TTS_VOICE_ID` | `English_expressive_narrator` | ID de voz utilizado para la salida de voz.       |
| `tts.providers.minimax.speed`   |                        | `1.0`                         | Velocidad de reproducción, `0.5..2.0`.   |
| `tts.providers.minimax.vol`     |                        | `1.0`                         | Volumen, `(0, 10]`.                     |
| `tts.providers.minimax.pitch`   |                        | `0`                           | Desplazamiento entero de tono, `-12..12`. |
...
Por eso, el enrutamiento automático de contenido multimedia puede usar la comprensión de imágenes de MiniMax incluso cuando el catálogo incluido de proveedores de texto también contiene referencias de chat M3 con capacidad para imágenes. La comprensión de PDF usa `MiniMax-M2.7` únicamente para extraer texto; MiniMax no registra una ruta de conversión de PDF a imagen.
...
<AccordionGroup>
  <Accordion title="Opciones de configuración">
    ...
    | `agents.defaults.modelPolicy.allow` | Lista explícita opcional de modelos permitidos |
  ...
  <Accordion title="Ejemplo de alternativa">
    **Ideal para:** mantener como principal el modelo de última generación más potente y recurrir a MiniMax M2.7 en caso de error. El siguiente ejemplo usa Opus como modelo principal concreto; sustitúyalo por el modelo principal de última generación que prefiera.
...
- Use `openclaw models list` para confirmar el ID de proveedor actual y, a continuación, cambie con `openclaw models set minimax/MiniMax-M3` o `openclaw models set minimax-portal/MiniMax-M3`
...
<AccordionGroup>
  <Accordion title='"Modelo desconocido: minimax/MiniMax-M3"'>
    Normalmente, esto significa que el **proveedor MiniMax no está configurado** (no se encontró ninguna entrada de proveedor coincidente ni ningún perfil de autenticación o clave de entorno de MiniMax). Para solucionarlo:
    ...
    - Establezca `MINIMAX_API_KEY`, `MINIMAX_OAUTH_TOKEN` o un perfil de autenticación de MiniMax para que se pueda insertar el proveedor correspondiente.
