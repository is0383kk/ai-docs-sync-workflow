...
El proveedor Volcengine da acceso a modelos Doubao y a modelos de terceros alojados en Volcano Engine, con endpoints separados para cargas de trabajo generales y de programación. El mismo plugin incluido también registra Volcengine Speech como proveedor de TTS.
...
<Tip>
Para una configuración no interactiva (CI, scripts), pase la clave directamente:
...
Campos disponibles en `tts.providers.volcengine`: `apiKey`, `voice`, `speedRatio` (0.2-3.0), `emotion`, `cluster`, `resourceId`, `appKey` y `baseUrl`. `!emotion=<value>` también funciona como directiva de voz en línea cuando se permiten las anulaciones de la configuración de voz.

Para destinos de notas de voz, OpenClaw solicita `ogg_opus` nativo del proveedor. Para archivos adjuntos de audio normales, solicita `mp3`. Los alias de proveedor `bytedance` y `doubao` también se resuelven en este proveedor de voz.
...
Otras variables de entorno opcionales de TTS: `VOLCENGINE_TTS_VOICE`, `VOLCENGINE_TTS_APP_KEY` y `VOLCENGINE_TTS_BASE_URL` sustituyen los campos de configuración `tts.providers.volcengine` correspondientes cuando están establecidas.
...
<Warning>
Al ejecutar OpenClaw como servicio en segundo plano, las variables de entorno establecidas en el shell interactivo no se heredan automáticamente. Consulte la nota sobre el daemon anterior.
</Warning>
