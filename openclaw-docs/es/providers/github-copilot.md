...
read_when:
    ...
    - Está eligiendo entre el proveedor Copilot integrado, el arnés del SDK de Copilot y Copilot Proxy
...
x-i18n:
    generated_at: "2026-07-26T05:18:25Z"
    model: gpt-5.6
...
modelos o entorno de ejecución de agentes de tres maneras diferentes.

## Tres maneras de usar Copilot en OpenClaw

<Tabs>
  <Tab title="Proveedor integrado (github-copilot)">
    Use el flujo nativo de inicio de sesión mediante dispositivo para obtener un token de GitHub y, después, intercambiarlo por
  ...
  <Tab title="Plugin de arnés del SDK de Copilot (copilot)">
    ...
    Elija esta opción cuando quiera sesiones nativas de la CLI de Copilot, estado de hilos
    gestionado por el SDK y Compaction controlada por Copilot para esos turnos del agente. Sin la
    habilitación explícita `agentRuntime`, los modelos `github-copilot/*` siguen usando el
    proveedor integrado. Consulte [Arnés del SDK de Copilot](/es/plugins/copilot) para ver el contrato
  ...
  <Tab title="Plugin Copilot Proxy (copilot-proxy)">
    ...
    El plugin `copilot-proxy` se distribuye con OpenClaw y está habilitado de forma predeterminada.
...
Si su organización usa un inquilino de GitHub Enterprise con residencia de datos (un
host `*.ghe.com` como `your-org.ghe.com`), Copilot se aloja en puntos de conexión
locales del inquilino en lugar de en el `github.com` público. OpenClaw ofrece esta opción
de autenticación directamente para que no tenga que editar las URL manualmente.

<Steps>
  <Step title="Elegir la opción de autenticación Enterprise">
    ...
    **GitHub Copilot (Enterprise / data residency)**. Se le solicitará
    ...
    mediante dispositivo se ejecutará en ese inquilino.

    Introduzca únicamente la raíz del inquilino (`your-org.ghe.com`). No se aceptan hosts de servicio
    derivados, como `api.your-org.ghe.com` o `copilot-api.your-org.ghe.com`;
    OpenClaw obtiene automáticamente esos puntos de conexión a partir de la raíz del inquilino.
  ...
  <Step title="El dominio se conserva en la configuración">
    El host elegido se almacena en los parámetros del proveedor para que las posteriores renovaciones
    de tokens y solicitudes de completado se dirijan automáticamente al inquilino:
...
El flujo mediante dispositivo, el intercambio de tokens y las solicitudes de completado se resuelven respectivamente en
`https://your-org.ghe.com/login/device/code`,
`https://api.your-org.ghe.com/copilot_internal/v2/token` y
`https://copilot-api.your-org.ghe.com`. Los tokens de residencia de datos contienen
una marca del inquilino y ninguna indicación de proxy, por lo que la URL base de completado usa como alternativa el
host de Copilot del inquilino en lugar del punto de conexión público.

<Note>
Al cambiar de dominio, siempre se vuelve a ejecutar el inicio de sesión mediante dispositivo. Si ya tiene almacenado
un token de Copilot y elige un dominio diferente (`github.com` público ↔ un inquilino
`*.ghe.com`, o de un inquilino a otro), OpenClaw no reutilizará el token existente:
obligará a iniciar sesión de nuevo para que el token quede limitado al dominio que se escribirá en la
configuración. Al repetir el inicio de sesión para el *mismo* dominio, se sigue ofreciendo reutilizar el
token actual. Al volver al `github.com` público, se elimina el
...
y las solicitudes de completado. Establézcala en su host `*.ghe.com` para configuraciones
completamente desatendidas o de CI. Déjela sin definir (y sin el parámetro de configuración) para usar el
`github.com` público. Los inicios de sesión conservan el dominio para el que emitieron el token
(y lo eliminan al iniciar sesión en el `github.com` público), por lo que el enrutamiento sigue siendo
correcto incluso después de eliminar la variable de entorno.
</Note>
...
sucesivamente a `COPILOT_GITHUB_TOKEN`, `GH_TOKEN` y `GITHUB_TOKEN`. Use
...
<AccordionGroup>
  ...
  <Accordion title="Actualización en vivo del catálogo desde la API de Copilot">
    Una vez que la ruta de autenticación mediante inicio de sesión en dispositivo (o variable de entorno) ha resuelto un token de GitHub,
    OpenClaw actualiza el catálogo de modelos bajo demanda desde `${baseUrl}/models`
    (el mismo punto de conexión que usa Copilot para VS Code), de modo que el entorno de ejecución siga
    los derechos de cada cuenta y las ventanas de contexto exactas sin
    cambios continuos en el manifiesto. Los modelos de Copilot recién publicados se hacen visibles sin
    ...
    del catálogo estático del manifiesto (escenarios sin conexión o con aislamiento de red):
  ...
  <Accordion title="Selección del transporte">
    ...
    correcto según la referencia del modelo.
  </Accordion>
  ...
  <Accordion title="Compatibilidad de las solicitudes">
    ...
    visión de Copilot cuando un turno contiene una imagen de entrada.
  </Accordion>

  <Accordion title="Orden de resolución de las variables de entorno">
    OpenClaw resuelve la autenticación de Copilot a partir de las variables de entorno en el siguiente
    orden de prioridad:
  ...
  <Accordion title="Almacenamiento de tokens">
    ...
    `github-copilot:github`) y lo intercambia por un token de corta duración de la API de
    Copilot cuando se ejecuta OpenClaw. No es necesario gestionar el token manualmente.
  </Accordion>
</AccordionGroup>

## Incrustaciones para la búsqueda en memoria

GitHub Copilot también puede funcionar como proveedor de incrustaciones para la
...
un token de GitHub disponible, OpenClaw detecta los modelos de incrustación disponibles en
...
3. Consulta el punto de conexión `/models` de Copilot para detectar los modelos de incrustación disponibles.
4. Elige el mejor modelo (orden de preferencia: `text-embedding-3-small`,
   `text-embedding-3-large`, `text-embedding-ada-002`).
5. Envía las solicitudes de incrustación al punto de conexión `/embeddings` de Copilot.

La disponibilidad de los modelos depende de su plan de GitHub. Si no hay modelos de incrustación
