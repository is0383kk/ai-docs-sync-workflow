...
read_when:
    ...
    - Se desea una API compatible con OpenAI para los modelos alojados de Baseten
summary: Configuración de Baseten para Inkling y APIs de modelos alojadas
title: Baseten
x-i18n:
    generated_at: "2026-07-26T05:53:06Z"
    model: gpt-5.6
...
[Las API de modelos de Baseten](https://docs.baseten.co/inference/model-apis/overview) proporcionan acceso alojado y compatible con OpenAI a modelos de vanguardia. El Plugin externo oficial utiliza descubrimiento autenticado, por lo que OpenClaw sigue el conjunto completo de modelos habilitados para la cuenta de Baseten. Su alternativa sin conexión contiene todas las API de modelos disponibles cuando se compiló esta versión de OpenClaw.
...
<Steps>
  <Step title="Crear una cuenta de Baseten y una clave de API">
    El plan Basic de Baseten no tiene ninguna cuota mensual de plataforma; las llamadas a las API de modelos se cobran según el uso. Cree una clave en la [configuración de claves de API de Baseten](https://app.baseten.co/settings/api_keys) y consulte las tarifas actuales en la [página de precios](https://www.baseten.co/pricing).
  </Step>
...
```
  ...
  <Step title="Verificar el catálogo activo">
    ```bash
...
Utilice `/model baseten/thinkingmachines/inkling` para cambiar un chat existente.

## Catálogo alternativo incluido

El catálogo activo autenticado es la fuente de referencia. Estas filas permiten que la configuración y la selección de modelos sigan siendo útiles antes de que el descubrimiento se complete correctamente:
...
Todos los modelos incluidos admiten llamadas a herramientas y razonamiento. OpenClaw asigna sus niveles de pensamiento a modelos con `reasoning_effort` nativo. Los modelos GLM, Kimi y Nemotron de Baseten, que requieren activación, tienen el pensamiento desactivado de forma predeterminada; la mayoría ofrecen un control binario de desactivación/activación, mientras que GLM 5.2 ofrece desactivado, alto y máximo. OpenClaw envía estas opciones mediante el control `chat_template_args.enable_thinking` de Baseten y, para GLM 5.2, mediante el parámetro validado de nivel superior `reasoning_effort`.

<Note>
Baseten puede añadir, eliminar o cambiar las API de modelos independientemente de las versiones de OpenClaw. El Plugin actualiza los identificadores de los modelos, los límites de contexto, los límites de salida y los precios de entrada, entrada almacenada en caché y salida desde la API autenticada, al tiempo que conserva la política de transporte de OpenClaw específica de cada modelo.
</Note>
...
<Note>
Si el Gateway se ejecuta como demonio (launchd, systemd, Docker), asegúrese de que `BASETEN_API_KEY` esté disponible para ese proceso. Una clave exportada únicamente en un shell interactivo no es visible para un servicio administrado que ya esté en ejecución.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Preguntas frecuentes sobre modelos" href="/es/help/faq-models" icon="circle-question">
    Solución de problemas de los perfiles de autenticación y la selección de modelos.
  </Card>
</CardGroup>
