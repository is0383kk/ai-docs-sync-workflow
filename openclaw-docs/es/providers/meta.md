...
| Indicador de incorporación | `--auth-choice meta-api-key`             |
| Indicador directo de la CLI | `--meta-api-key <key>`            |
...
```
  ...
  <Step title="Verificar que los modelos estén disponibles">
    ...
    Muestra la entrada estática `muse-spark-1.1` del catálogo. Si no se puede resolver
    `MODEL_API_KEY`, `openclaw models status --json` informa de la credencial que falta en
...
shell interactivo no servirá para un servicio administrado, a menos que el entorno se importe
por separado.
</Note>
