...
read_when:
    ...
    - Está conectando trazas, métricas o registros a Grafana, Datadog, Honeycomb, New Relic, Tempo u otro backend de OTLP
...
summary: Exporte los diagnósticos de OpenClaw a recopiladores de OpenTelemetry o a JSONL en stdout mediante el plugin diagnostics-otel
title: Exportación de OpenTelemetry
x-i18n:
    generated_at: "2026-07-26T04:38:07Z"
    model: gpt-5.6
...
pipelines de registros de contenedores y entornos aislados. Cualquier recopilador o backend que acepte
...
- **`diagnostics-otel`** se suscribe a esos eventos y los exporta como
  **métricas**, **trazas** y **registros** mediante OTLP/HTTP, y puede
  duplicar los registros en JSONL en stdout.
- Las **llamadas a proveedores** reciben un encabezado W3C `traceparent` procedente del
  contexto de span de llamada al modelo de confianza de OpenClaw cuando el transporte del proveedor acepta encabezados
...
También puede habilitarse el plugin desde la CLI: `openclaw plugins enable diagnostics-otel`.

<Note>
`protocol` solo admite `http/protobuf`. Dado que `traces` y `metrics` están habilitados de forma predeterminada, cualquier otro valor (incluido `grpc`) cancela toda la suscripción de diagnostics-otel con una advertencia `unsupported protocol`; esto también detiene la exportación de registros a stdout. Establezca explícitamente `traces: false` y `metrics: false` si solo desea `logsExporter: "stdout"` con un valor de protocolo que no sea OTLP.
</Note>
...
| Señal       | Contenido                                                                                                                                                                                              |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Métricas** | Contadores e histogramas de uso de tokens, coste, duración de ejecución, conmutación por error, uso de Skills, flujo de mensajes, eventos de Talk, carriles de cola, estado/recuperación de sesiones, ejecución de herramientas, exec, memoria, disponibilidad y estado de los exportadores. |
...
| `OTEL_SEMCONV_STABILITY_OPT_IN`                                                                                   | Establézcalo en `gen_ai_latest_experimental` para emitir el formato más reciente de spans de inferencia GenAI: nombres de span `{gen_ai.operation.name} {gen_ai.request.model}`, tipo de span `CLIENT` y `gen_ai.provider.name` en lugar del formato heredado `gen_ai.system`. Las métricas de GenAI siempre usan atributos acotados y de baja cardinalidad. |
| `OPENCLAW_OTEL_PRELOADED`                                                                                         | Establézcalo en `1` cuando otra precarga o proceso host ya haya registrado el SDK global de OpenTelemetry. El plugin omite entonces su propio ciclo de vida de NodeSDK, pero sigue conectando los listeners de diagnóstico y respeta `traces`/`metrics`/`logs`.                                                                                    |

## Privacidad y captura de contenido

El contenido sin procesar de modelos y herramientas **no** se exporta de forma predeterminada. Los spans contienen identificadores
...
OTLP conservan de forma predeterminada la gravedad, el registrador, la ubicación del código, el contexto de traza
de confianza y los atributos saneados; el cuerpo del mensaje de registro sin procesar solo se exporta
cuando `diagnostics.otel.captureContent` es el booleano `true`. Las subclaves
granulares `captureContent.*` nunca habilitan los cuerpos de los registros. Las métricas de Talk solo exportan
...
del entorno de ejecución de agente integrado (`openclaw.content.tool_input` y
`gen_ai.tool.call.arguments` en spans completados/con error;
`openclaw.content.tool_output` y `gen_ai.tool.call.result` en spans completados).
Los nombres `openclaw.content.*` siguen siendo los nombres de atributos estables de OpenClaw;
las copias `gen_ai.tool.call.*` los replican para visores nativos de semconv.
...
canal de confianza exclusivo para listeners y nunca se incluye en el bus público de eventos
de diagnóstico.

## Muestreo y vaciado

- **Trazas:** `diagnostics.otel.sampleRate` establece un `TraceIdRatioBasedSampler`
  solo en el tramo raíz (`0.0` descarta todo, `1.0` conserva todo). Si no se establece, se usa el valor predeterminado
...
- **Correlación de registros de archivo:** los registros de archivo JSONL incluyen `traceId`,
  ...
  de traza de diagnóstico válido, lo que permite a los procesadores de registros asociar las líneas de registro locales con
  los tramos exportados.
- **Correlación de solicitudes:** las solicitudes HTTP y las tramas WebSocket del Gateway crean
  ...
  ámbito heredan de forma predeterminada la traza de la solicitud, mientras que los tramos de ejecución del agente y de llamada
  al modelo se crean como elementos secundarios para que los encabezados `traceparent` del proveedor permanezcan en la
  misma traza.
- **Correlación de llamadas al modelo:** los tramos `openclaw.model.call` incluyen de forma predeterminada tamaños seguros de los componentes
  del prompt y atributos de tokens por llamada cuando el resultado del proveedor
  expone el uso. `openclaw.model.usage` sigue siendo el tramo de contabilización
  de nivel de ejecución para los paneles agregados de coste, contexto y canal, y
...
Cada tramo `openclaw.model.call` identifica lo que mide su ciclo de vida mediante
`openclaw.model_call.observation_unit`:

- `request` - una solicitud observable al modelo o proveedor. Las llamadas nativas al modelo
...
Ambas unidades siguen siendo tramos de llamada al modelo para que los backends de trazas puedan representar la entrada,
la salida, el uso y la jerarquía del modelo. Los tramos de solicitud usan la operación GenAI derivada de la API
(`chat`, `generate_content` o `text_completion`), mientras que los tramos de turno usan
...
Los turnos de la CLI de Claude Code emiten un único tramo sintético de nivel de turno `openclaw.model.call`.
No son tramos de solicitudes HTTP de Anthropic. Usan `openclaw.api =
...
Los diagnósticos de la CLI de Claude solo se crean mientras el distribuidor de diagnósticos
del proceso está habilitado y hay conectado un receptor de eventos interno o de confianza.
Sin un Plugin de observabilidad u otro receptor activo, los turnos de la CLI de Claude omiten
la jerarquía sintética de trazas, los búferes de contenido y la contabilización de bytes
del flujo de diagnóstico. Cuando la captura de contenido está habilitada, los campos de prompt y prompt del sistema
tienen un límite de 128 KiB cada uno; la salida del asistente tiene un límite de 128 KiB repartidos entre
...
OpenClaw asigna a los turnos de la CLI de Claude la misma jerarquía de propiedad utilizada por otros
entornos de ejecución de agentes: `openclaw.harness.run` (`openclaw.harness.id = claude-cli`)
contiene `openclaw.run`, que contiene el tramo `openclaw.model.call`
de Claude. Los tramos del arnés y de ejecución son límites sintéticos de turno de OpenClaw, no
fases internas de Claude Code. Los turnos de una sola ejecución y de stdio administrado usan la misma
jerarquía; un reintento real con una sesión nueva crea otro elemento secundario de llamada al modelo dentro
de la misma ejecución de OpenClaw.

El tramo comienza cuando OpenClaw admite el turno preparado de la CLI y termina únicamente después de que
ese turno tenga éxito o falle. En las sesiones administradas, un resultado provisional satisfactorio
no finaliza el tramo mientras Claude informe de agentes o flujos de trabajo en segundo plano que mantienen
el resultado; el resultado final posterior al vaciado sí lo finaliza. Las cancelaciones, los tiempos de espera agotados, los fallos del proceso,
los fallos de salida o análisis y otros fallos del turno finalizan el mismo tramo con un error.
...
último mensaje del asistente para no modificar la semántica de costes existente; el
tramo de llamada al modelo de nivel de turno usa el uso acumulado final cuando está disponible,
incluidos los tokens de lectura y creación de caché.

En estos tramos de la CLI, los campos de bytes y tiempo describen el límite observable de la
CLI de OpenClaw:

- `openclaw.model_call.request_bytes` es el tamaño UTF-8 del valor del prompt
  enviado mediante stdin/argv en una sola ejecución, o del sobre de usuario JSONL de stdio administrado. No
...
- `openclaw.model_call.time_to_first_byte_ms` es el tiempo hasta la primera salida observable
  en stdout o stderr de la CLI de Claude. No es el TTFB de la red.

Con los campos granulares `captureContent` correspondientes habilitados, el tramo exporta
el prompt efectivo que OpenClaw envía a Claude Code, el prompt del sistema añadido por OpenClaw
y el texto, razonamiento e identidad de llamada a herramienta visibles del asistente mediante
`gen_ai.input.messages`, `gen_ai.output.messages` y
`gen_ai.system_instructions`. Los argumentos de las herramientas, las firmas opacas de pensamiento y
los resultados de las herramientas se omiten del sobre del asistente de Claude. OpenClaw no
afirma tener acceso al prompt privado del sistema de Claude Code, a la carga útil oculta de solicitudes reanudadas o
compactadas, a los esquemas nativos de herramientas internas, a la solicitud HTTP sin procesar de Anthropic,
a los reintentos internos, al identificador de solicitud ascendente ni al TTFB real de la red. Dado que
Claude Code no expone con precisión sus definiciones nativas efectivas de herramientas,
estos tramos no rellenan `gen_ai.tool.definitions`.

Los tramos de herramientas del arnés externo de Claude siguen conteniendo solo metadatos, incluso cuando la captura de contenido
de herramientas está habilitada. Como en todos los tramos del modelo, el contenido capturado de la CLI de Claude usa
la ruta reservada para receptores de confianza y los límites existentes de censura y tamaño
...
- `openclaw.model_call.request_bytes` (histograma, tamaño en bytes UTF-8 de la carga útil final de la solicitud al modelo; para la CLI de Claude Code, la entrada o el sobre observable del prompt descrito anteriormente; sin contenido sin procesar de la carga útil)
- `openclaw.model_call.response_bytes` (histograma, tamaño en bytes UTF-8 de las cargas útiles de los fragmentos de respuesta transmitidos; los deltas de texto, pensamiento y llamadas a herramientas de alta frecuencia cuentan únicamente los bytes `delta` incrementales; para la CLI de Claude Code, bytes de stdout observados; sin contenido sin procesar de la respuesta)
- `openclaw.model_call.time_to_first_byte_ms` (histograma, tiempo transcurrido antes del primer evento de respuesta transmitida; para la CLI de Claude Code, primera salida observable de la CLI en lugar del TTFB de la red)
...
- `session.long_running`: el trabajo integrado activo, las llamadas al modelo o las llamadas a herramientas
  siguen progresando. Las llamadas silenciosas al modelo con propietario también se notifican como de larga duración antes del umbral de cancelación integrado, por lo que los proveedores de modelos lentos o sin transmisión no parecen sesiones del Gateway bloqueadas mientras la cancelación sea observable.
- `session.stalled`: existe trabajo activo, pero la ejecución activa no ha informado
  de progreso reciente. Las llamadas al modelo con propietario cambian de `session.long_running` a
  `session.stalled` al alcanzar o superar el umbral de cancelación integrado; la actividad
  obsoleta del modelo o de herramientas sin propietario no se trata como trabajo inofensivo de larga duración.
  Las ejecuciones integradas bloqueadas permanecen inicialmente en modo de solo observación y, después,
  se cancelan y drenan tras superar el umbral de cancelación sin progreso, para que puedan reanudarse los turnos en cola detrás del carril.
- `session.stuck`: registros internos de sesión obsoletos sin trabajo activo, o una sesión
  inactiva en cola con actividad obsoleta del modelo o de herramientas sin propietario. Esto libera el
  carril de la sesión afectada inmediatamente después de superar las comprobaciones de recuperación.
...
Los diagnósticos `session.stuck` repetidos reducen su frecuencia mientras la sesión permanezca
sin cambios, por lo que los paneles deben generar alertas ante aumentos sostenidos y no
en cada pulso de Heartbeat. Para consultar la opción de configuración y los valores predeterminados, véase
...
- `openclaw.telemetry.exporter.events` (contador, atributos: `openclaw.exporter`, `openclaw.signal`, `openclaw.status`, `openclaw.reason` opcional, `openclaw.errorCategory` opcional; telemetría propia del ciclo de vida o los fallos del exportador)
...
- `openclaw.model.call`
  ...
  - `openclaw.model_call.usage.*` y `gen_ai.usage.*` cuando el resultado incluye el uso de esa solicitud o del turno agregado
  - Evento de tramo `openclaw.provider.request` con el atributo `openclaw.upstreamRequestIdHash` (acotado y basado en hash) cuando el resultado del proveedor ascendente expone un id. de solicitud; los identificadores sin procesar nunca se exportan
...
- `openclaw.tool.execution`
  ...
  - `openclaw.errorCategory`/`openclaw.errorCode` opcionales en caso de error, `openclaw.deniedReason` y `openclaw.outcome=blocked` cuando la política o el entorno aislado deniegan la operación
- `openclaw.exec`
...
Cuando la captura de contenido se habilita explícitamente, los tramos del modelo y de las herramientas también pueden
...
aditivos. Los consumidores de TypeScript deben conservar ramas predeterminadas en lugar de asumir que
cualquiera de las uniones será exhaustiva permanentemente.

**Uso del modelo**

- `model.usage` - tokens, coste, duración, contexto, proveedor/modelo/canal,
  identificadores de sesión. `usage` representa la contabilidad del proveedor/turno para el coste y la telemetría;
...
- `exec.process.completed` - resultado del terminal, duración, destino, modo, código de
  salida y tipo de fallo. No se incluyen el texto del comando ni los directorios
  de trabajo.
...
La salida de los indicadores se envía al archivo de registro estándar (`logging.file`) y
`logging.redactSensitive` continúa ocultando los datos confidenciales. Guía completa:
...
- [Registro](/es/logging) - registros en archivos, salida de consola, seguimiento mediante la CLI y la pestaña Logs de la interfaz de control
- [Funcionamiento interno del registro del Gateway](/es/gateway/logging) - estilos de registro de WS, prefijos de subsistemas y captura de la consola
