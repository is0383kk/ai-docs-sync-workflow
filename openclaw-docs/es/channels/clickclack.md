...
title: ClickClack
x-i18n:
    generated_at: "2026-07-26T04:30:17Z"
    model: gpt-5.6
...
ClickClack conecta OpenClaw con un espacio de trabajo de ClickClack autoalojado mediante tokens de bot de ClickClack con compatibilidad nativa.

Utilice esta opción cuando desee que un agente de OpenClaw aparezca como usuario bot de ClickClack. ClickClack admite bots de servicio independientes y bots propiedad de usuarios; los bots propiedad de usuarios conservan un `owner_user_id` y reciben únicamente los alcances de token que se les conceden.
...
Para orígenes de frontend y API separados o una API montada en una ruta, ClickClack emite
en su lugar un endpoint de reclamación exacto:
...
verifica la conexión e informa si el Gateway en ejecución la detectó.
Para endpoints exactos con versión, OpenClaw valida y guarda la base de API
canónica devuelta por ClickClack, incluido cualquier prefijo de ruta. El código de configuración
no se almacena en la configuración de OpenClaw.

Las reclamaciones mediante código de configuración utilizan HTTPS para servidores públicos. También se admite HTTP sin cifrar para
...
Seleccione **Manual token** en ClickClack al configurar un cliente que no sea OpenClaw o
...
Las cuentas con nombre deben utilizar un token configurado o un archivo de token; la variable de entorno
...
| `apiBaseUrl`            | `baseUrl`           | Endpoint opcional entre servidores para tráfico REST y WebSocket en tiempo real.             |
| `token`                 | ninguno                | Token del bot como cadena de texto sin formato o referencia a secreto (`source: "env" \| "file" \| "exec"`).        |
...
| `replyMode`             | `"agent"`           | `"agent"` ejecuta el pipeline completo del agente; `"model"` envía finalizaciones breves y directas del modelo. |
| `defaultTo`             | `"channel:general"` | Destino utilizado cuando una ruta de salida no especifica ninguno.                                      |
...
| `commandMenu`           | `true`              | Publica comandos nativos en el autocompletado del redactor de ClickClack.                            |
| `reconnectMs`           | `1500`              | Retardo de reconexión en tiempo real (100 a 60000).                                                |
| `discussions`           | desactivado            | Configuración de canal administrada por sesión; consulte [Conversaciones de sesión](#session-discussions).  |
...
El nombre de host público puede permanecer completamente protegido por autenticación para los usuarios del navegador. OpenClaw
utiliza el endpoint de bucle invertido para las solicitudes REST, la verificación de la configuración y el
WebSocket en tiempo real, mientras que los enlaces `embedUrl` y `openUrl` de las conversaciones continúan
utilizando el `baseUrl` público. Si se omite `apiBaseUrl`, todo el tráfico utiliza
`baseUrl`, con lo que se conserva el comportamiento existente.

Si `plugins.allow` es una lista restrictiva no vacía, al seleccionar explícitamente
ClickClack en la configuración de canales o ejecutar `openclaw plugins enable clickclack`
...
Active las conversaciones en una cuenta de ClickClack para proporcionar a cada sesión de OpenClaw un
canal de ClickClack dedicado. El token de la cuenta debe incluir
...
que `workspace` en el nivel de la cuenta, y utiliza ese valor de manera predeterminada. `section` controla
...
Active las conversaciones en exactamente una cuenta de ClickClack. El proveedor del Gateway no dispone
de un selector de cuenta, por lo que se rechazan varias cuentas de conversaciones activadas
...
externamente. El plugin mantiene sincronizados la etiqueta, la categoría y el estado de archivado de la sesión.
...
sesión de OpenClaw, se archiva el canal de ClickClack en lugar de eliminarlo, por lo que su
historial permanece disponible. El plugin reconcilia las vinculaciones cuando se utilizan RPC de conversaciones
...
el mismo id de agente que la sesión principal adjunta. Se indica al agente secundario qué
sesión principal debe observar, y puede utilizar `sessions_history` y `session_status`
(`changesSince` resulta útil para comprobaciones incrementales). Solo utiliza `sessions_send`
cuando las personas de la conversación le piden retransmitir o dirigir la sesión principal.
La vinculación, la referencia de propiedad administrada y la identidad del par de la sesión secundaria incluyen
el id concreto de la sesión de OpenClaw junto con el servidor y el canal de ClickClack fijados.
Al restablecer una clave de sesión reutilizable o redirigir una cuenta, se revoca
localmente el canal anterior, se archiva cuando la credencial anterior sigue siendo utilizable y
no se puede reutilizar su transcripción secundaria. Los mensajes que llegan mediante una
vinculación archivada, restablecida, desactivada o redirigida se descartan en lugar de recurrir
al enrutamiento normal de canales de la cuenta. Las vinculaciones liberadas dejan un marcador
duradero de canal revocado para que los eventos en tiempo real retrasados sigan aplicando un cierre seguro. La
...
local no puede convertir un canal administrado en uno normal.

Mantenga `tools.sessions.visibility` en su valor predeterminado más seguro, `tree`. El plugin
instala una concesión limitada al host únicamente entre cada sesión secundaria y su sesión principal
adjunta, además de un enlace de política de herramientas que bloquea la detección de sesiones y
los destinos entre sesiones. Permite `sessions_history`, `session_status` y
`sessions_send` únicamente para la sesión principal adjunta e impide que la llamada de estado
cambie el modelo de esa sesión. Esas herramientas aún deben estar presentes en la lista efectiva de
herramientas permitidas del agente. El prompt del sistema sirve de orientación; la concesión
del host y el enlace constituyen el límite de autorización.

El servidor ClickClack debe admitir los campos de canal gestionado (`external_managed`,
`external_ref`, `external_url` y `sidebar_section`) al crear y
actualizar canales, y devolverlos en las respuestas de canal. OpenClaw verifica ese contrato
antes de conservar una vinculación. Si se pierde una respuesta de creación, la siguiente apertura adopta
...
los eventos que, de otro modo, no estarían vinculados en el espacio de trabajo de destino. El reconciliador general
...
por una cuenta o un espacio de trabajo no puede volver a adoptar un canal anterior. Las vinculaciones también quedan fijadas a la
URL configurada del servidor ClickClack y se invalidan si se redirige la
cuenta. Cambiar o eliminar `controlUrlBase` actualiza o elimina el vínculo del
canal gestionado en la siguiente pasada de conciliación. Cambiar
`discussions.workspace` archiva y libera la vinculación anterior antes de que se pueda
abrir un canal en el nuevo espacio de trabajo cuando la credencial del espacio de trabajo anterior permanece
configurada. Si el token se sustituyó por una credencial limitada a un espacio de trabajo que
...
La sesión principal adjunta también recibe una herramienta `discussion` exclusivamente de consulta. Lee
los mensajes más recientes y las respuestas recientes de los hilos como un registro con caracteres escapados y atribución
por mensaje, y no tiene efectos secundarios de escritura ni de ciclo de vida. Las consultas de la raíz del canal y de los hilos
...
- `replyMode: "agent"` (predeterminado) envía los mensajes entrantes a través del pipeline normal del agente, incluido el registro de sesiones y la política de herramientas.
- `replyMode: "model"` omite el pipeline del agente y utiliza `llm.complete` del runtime del plugin para las respuestas directas del bot, a las que opcionalmente pueden dar forma `model` y `systemPrompt`. El proveedor y el modelo seleccionados controlan el presupuesto de finalización.
...
Al iniciar el gateway, cada cuenta configurada publica los comandos
nativos de OpenClaw en ClickClack. Aparecen en el autocompletado del cuadro de redacción con una etiqueta que incluye el
...
de forma individual. Es posible que los tokens creados antes de la introducción de los menús de comandos necesiten
que se añada el ámbito o un token de sustitución.

La sincronización se realiza en la medida de lo posible y se ejecuta una vez por cada inicio del gateway. La ausencia de un ámbito o un fallo de
red registra una advertencia; un servidor ClickClack anterior que no disponga del endpoint lo registra
...
de comandos de Skills, plugins o personalizados no se añaden al menú. Si un
nombre también está registrado como un comando de barra HTTP, ClickClack procesa primero ese
...
id de mensaje autoritativo de ClickClack con su forma canónica `msg_<ulid>`, el canal deriva
el id de ejecución determinista de OpenClaw `clickclack:<message-id>`. Cada llamada al modelo
queda entonces visible en los diagnósticos como `clickclack:<message-id>:model:<n>`; cuando ese
...
nonces estables por parte para mensajes y cargas antes de la primera escritura en ClickClack, de modo que
...
- Creación idempotente de mensajes y asociación de archivos adjuntos para el mismo
  nonce y carga limitados al propietario.

Un error 404 genérico de un servidor anterior no se considera una prueba de que un envío esté ausente.
OpenClaw deja la entrega sin resolver en lugar de arriesgarse a crear un duplicado; actualice
ClickClack antes de activar respuestas del agente que generen contenido multimedia.

## Filas de actividad del agente

De forma predeterminada, un canal de ClickClack no muestra nada mientras se ejecuta un turno del agente; solo se publica la respuesta final. Establezca `agentActivity: true` en una cuenta para publicar filas de mensajes duraderas `agent_commentary` y `agent_tool` mientras el turno está en curso:
...
- **Desactivado de forma predeterminada.** Las configuraciones estándar y los servidores ClickClack anteriores no se ven afectados.
...
- Las filas se agrupan por turno (`turn_id`), se consolidan para que cada paso lógico corresponda a una fila y las filas de herramientas utilizan el mismo formato de progreso que Discord/Slack/Telegram (nombre de la herramienta más detalles del comando).
- **Metadatos de atribución.** Las publicaciones creadas por el agente (las filas de actividad y la respuesta final) incluyen los campos `author_model` y `author_thinking`, resueltos a partir del modelo utilizado realmente durante el turno (incluso después de recurrir a una alternativa). Los servidores que no definen estas columnas ignoran los campos JSON desconocidos; los servidores que los conservan pueden responder «qué modelo dijo esta línea y con qué nivel de razonamiento» para cada mensaje.
...
El contenido multimedia de salida utiliza la API de carga de ClickClack y, a continuación, adjunta la carga duradera
al mensaje de canal, la respuesta de hilo o el mensaje directo creados. Los archivos locales y las URL
remotas de contenido multimedia compatibles siguen la política normal de acceso a contenido multimedia de OpenClaw, con un límite de 64 MiB
por archivo. Los envíos duraderos en cola utilizan nonces independientes y limitados al propietario para cada
...
- `commands:write`: publicación del menú de comandos del bot. Incluido en los paquetes actuales `bot:write` y `bot:admin`, y se puede conceder de forma individual.
- `agent_activity:write`: filas duraderas de actividad del agente (`agent_commentary` / `agent_tool`). No se hereda de `bot:write` ni de `bot:admin`; solo se requiere cuando se establece `agentActivity: true`.
...
- Los envíos a canales fallan: compruebe que el bot sea miembro del espacio de trabajo y tenga `bot:write`.
