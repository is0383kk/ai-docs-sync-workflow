...
title: ClawHub
x-i18n:
    generated_at: "2026-07-26T04:31:47Z"
    model: gpt-5.6
...
- Utilice los comandos nativos de `openclaw` para buscar, instalar y actualizar Skills, así como para instalar plugins desde ClawHub.
...
para que los usuarios puedan examinar una Skill o un plugin antes de instalarlo.
...
antes de iniciar la instalación del archivo. Cuando una versión del paquete publica un artefacto
ClawPack, OpenClaw prefiere el `.tgz` exacto de npm-pack que se haya cargado, verifica
la cabecera del resumen de ClawHub y los bytes descargados, y registra los metadatos del artefacto para
...
Estos comandos instalan Skills en `./skills` dentro del directorio de trabajo actual
...
Utilice `--dry-run` para generar el plan de publicación exacto sin cargar nada y `--json`
para obtener una salida adecuada para CI.

Los plugins de código deben incluir los metadatos de compatibilidad con OpenClaw requeridos en
`package.json`, incluidos `openclaw.compat.pluginApi` y
`openclaw.build.openclawVersion`. Consulte [CLI](/es/clawhub/cli) para ver la referencia completa de
comandos y [Formato de las Skills](/es/clawhub/skill-format) para conocer los metadatos de las Skills.

## Seguridad y moderación

ClawHub es abierto de forma predeterminada: cualquiera puede cargar contenido, pero la publicación requiere una cuenta de
GitHub con la antigüedad suficiente para superar el control de carga. Las páginas públicas de detalles resumen el
estado del análisis más reciente antes de la instalación o descarga.

ClawHub ejecuta comprobaciones automatizadas de las Skills y versiones de plugins publicadas. Las versiones retenidas
por el análisis o bloqueadas pueden desaparecer del catálogo público y de las superficies de instalación, aunque
seguirán siendo visibles para su propietario en `/dashboard`.
...
[Uso aceptable](/es/clawhub/acceptable-usage) para conocer los detalles de las políticas y su aplicación.

## Telemetría y entorno

Cuando se ejecuta `clawhub install` con la sesión iniciada, la CLI puede enviar un evento de
instalación sin garantía de entrega para que ClawHub pueda calcular recuentos agregados de instalaciones. Desactive esta función con:
...
Consulte [Telemetría](/es/clawhub/telemetry), [API HTTP](/es/clawhub/http-api) y
