...
x-i18n:
    generated_at: "2026-07-26T23:39:40Z"
    model: gpt-5.6
...
Yalnızca ana makinede çalışan bash komutları `! <cmd>` kullanır (`/bash <cmd>` bunun diğer adıdır).

Bir konuşma bir ACP oturumuna bağlandığında normal metin ACP
çalıştırma çerçevesine yönlendirilir. Gateway yönetim komutları yerel kalır: `/acp ...` her zaman
OpenClaw komut işleyicisine ulaşır; ayrıca yüzeyde komut işleme etkin olduğunda
...
<CardGroup cols={3}>
  <Card title="Komutlar" icon="terminal">
    Gateway tarafından işlenen bağımsız `/...` iletileri. İletinin
    tek içeriği olarak gönderilmelidir.
  </Card>
  <Card title="Yönergeler" icon="sliders">
    `/think`, `/fast`, `/verbose`, `/trace`, `/reasoning`, `/elevated`,
    `/exec`, `/model`, `/queue` — model iletiyi görmeden önce
    iletiden çıkarılır. Tek başına gönderildiğinde oturum ayarlarını kalıcı hâle getirir;
    başka metinlerle gönderildiğinde satır içi ipuçları olarak işlev görür.
  </Card>
  <Card title="Satır içi kısayollar" icon="bolt">
    `/help`, `/commands`, `/status`, `/whoami` — hemen çalıştırılır ve
    model kalan metni görmeden önce iletiden çıkarılır. Yalnızca yetkili gönderenler içindir.
  </Card>
</CardGroup>

<AccordionGroup>
  <Accordion title="Yönerge davranışının ayrıntıları">
    - Yönergeler, model iletiyi görmeden önce iletiden çıkarılır.
    ...
    - Başka metinler içeren **normal sohbet** iletilerinde satır içi ipuçları olarak
      işlev görür ve oturum ayarlarını kalıcı hâle **getirmez**.
    - Yönergeler yalnızca **yetkili gönderenler** için geçerlidir. `commands.allowFrom`
      ...
      kanal izin listelerinden, eşleştirmeden ve her zaman etkin erişim grubu uygulamasından gelir. Yetkisiz
      gönderenlerin yönergeleri düz metin olarak işlenir.
  </Accordion>
</AccordionGroup>
...
<ParamField path="commands.text" type="boolean" default="true">
  ...
  (WhatsApp, WebChat, Signal, iMessage, Google Chat, Microsoft Teams), `false` olarak
  ayarlandığında bile metin komutları çalışır.
</ParamField>

<ParamField path="commands.native" type='boolean | "auto"' default='"auto"'>
  Yerel komutları kaydeder. Otomatik: Discord/Telegram için açık, Slack için kapalıdır;
  yerel desteği olmayan sağlayıcılarda yok sayılır. Kanal başına
  `channels.<provider>.commands.native` ile geçersiz kılın. Discord'da `false`, eğik çizgi komutlarının
...
<ParamField path="commands.nativeSkills" type='boolean | "auto"' default='"auto"'>
  ...
  Discord/Telegram için açık, Slack için kapalıdır. `channels.<provider>.commands.nativeSkills` ile
  geçersiz kılın.
</ParamField>
...
<ParamField path="commands.bashForegroundMs" type="number" default="2000">
  Bash'in arka plan moduna geçmeden önce ne kadar bekleyeceğini belirler (`0`
  hemen arka plana geçirir).
</ParamField>
...
<ParamField path="commands.plugins" type="boolean" default="false">
  `/plugins` özelliğini etkinleştirir (plugin keşfi/durumu ile kurma ve etkinleştirme/devre dışı bırakma). Yazma işlemleri yalnızca sahip içindir.
</ParamField>

<ParamField path="commands.debug" type="boolean" default="false">
  `/debug` özelliğini etkinleştirir (yalnızca çalışma zamanı yapılandırması geçersiz kılmaları). Yalnızca sahip içindir.
</ParamField>
...
<ParamField path="commands.ownerAllowFrom" type="string[]">
  Yalnızca sahibe özel komut yüzeyleri için açık sahip izin listesi. `commands.allowFrom`
  ve DM eşleştirme erişiminden ayrıdır.
</ParamField>

<ParamField path="channels.<channel>.commands.enforceOwnerForCommands" type="boolean" default="false">
  Kanal başına: yalnızca sahibe özel komutlar için sahip kimliği gerektirir. `true` olduğunda
...
<ParamField path="commands.allowFrom" type="object">
  Komut yetkilendirmesi için sağlayıcı başına izin listesi. Yapılandırıldığında komutlar ve
  yönergeler için **tek** yetkilendirme kaynağıdır. Genel varsayılan için `"*"`
  kullanın; sağlayıcıya özel anahtarlar bunu geçersiz kılar.
</ParamField>
...
- **Yerleşik temel komutlar:** `src/auto-reply/commands-registry.shared.ts`
...
<AccordionGroup>
  <Accordion title="Oturumlar ve çalıştırmalar">
    ...
    | `/session idle <duration\|off>` | İş parçacığı bağlamasının boşta kalma süresi dolumunu yönetir |
    | `/session max-age <duration\|off>` | İş parçacığı bağlamasının azami yaş süresi dolumunu yönetir |
    | `/export-session [path]` | Yalnızca sahip içindir. Geçerli oturumu çalışma alanı içinde HTML olarak dışa aktarır. Diğer ad: `/export` |
    | `/export-trajectory [path]` | Geçerli oturum için bir JSONL yörünge paketi dışa aktarır. Diğer ad: `/trajectory` |

    Açıkça belirtilen `/export-session` yolları, çalışma alanındaki mevcut dosyaların
    üzerine yazar. Çakışmaya karşı güvenli bir dosya adı oluşturmak için yolu belirtmeyin.

    <Note>
      Control UI, yeni bir pano oturumu oluşturup buna geçmek için yazılan `/new`
      komutunu yakalar; ancak `session.dmScope: "main"` yapılandırılmışsa
      ve geçerli üst öğe aracının ana oturumuysa `/new`
      ana oturumu yerinde sıfırlar. Yazılan `/reset` yine de Gateway'in
      yerinde sıfırlama işlemini çalıştırır. Sabitlenmiş bir oturum modeli seçimini
      temizlemek istediğinizde `/model default` kullanın.
    </Note>
  ...
  <Accordion title="Model ve çalıştırma denetimleri">
    ...
    | `/reasoning [on\|off\|stream]` | Akıl yürütme görünürlüğünü açar veya kapatır. Diğer ad: `/reason` |
    ...
    | `/login [codex\|openai\|openai-codex]` | Özel sohbetten veya Web UI oturumundan Codex/OpenAI oturum açma işlemini eşleştirir. Yalnızca sahip/yönetici içindir |
    | `/model [name\|#\|status]` | Modeli gösterir veya ayarlar |
    | `/models [provider] [page] [limit=<n>\|all]` | Yapılandırılmış ya da kimlik doğrulaması kullanılabilir sağlayıcıları veya modelleri listeler |
    ...
    <AccordionGroup>
      <Accordion title="ayrıntılı / izleme / hızlı / akıl yürütme güvenliği">
        ...
        - `/trace` yalnızca pluginin sahip olduğu izleme/hata ayıklama satırlarını gösterir; normal ayrıntılı mesajlar kapalı kalır.
        - `/fast auto|on|off` bir oturum geçersiz kılmasını kalıcı hâle getirir; temizlemek için Oturumlar kullanıcı arayüzündeki `inherit` seçeneğini kullanın.
        - `/fast` sağlayıcıya özeldir: OpenAI/Codex bunu `service_tier=priority` ile eşler; doğrudan Anthropic istekleri bunu `service_tier=auto` veya `standard_only` ile eşler.
        - `/reasoning`, `/verbose` ve `/trace` grup ortamlarında risklidir — dahili akıl yürütmeyi veya plugin tanılamalarını açığa çıkarabilir. Bunları grup sohbetlerinde kapalı tutun.

      </Accordion>
      <Accordion title="Model değiştirme ayrıntıları">
        - `/model` yeni modeli oturumda hemen kalıcı hâle getirir.
        - Aracı boşta olduğunda sonraki çalıştırma bunu hemen kullanır.
  ...
  <Accordion title="Keşif ve durum">
    ...
    | `/status plugins` | Ayrıntılı plugin sağlığını gösterir: yükleme hataları, karantinalar, kanal plugini arızaları, bağımlılık sorunları ve uyumluluk bildirimleri. `commands.plugins: true` gerektirir |
    | `/goal [status\|start\|edit\|pause\|resume\|complete\|block\|clear] ...` | Geçerli oturumun kalıcı [hedefini](/tr/tools/goal) yönetir |
    | `/diagnostics [note]` | Yalnızca sahibe özel destek raporu akışı. Her seferinde exec onayı ister |
    | `/openclaw <request>` | Bir sahip DM'sinden OpenClaw kurulum ve onarım yardımcısını çalıştırır |
    ...
    | `/usage off\|tokens\|full\|reset\|cost` | Yanıt başına kullanım alt bilgisini denetler (`reset`/`inherit`/`clear`/`default`, yapılandırılmış varsayılanı yeniden devralmak üzere oturum geçersiz kılmasını temizler) veya yerel bir maliyet özeti yazdırır |
  </Accordion>
  ...
  <Accordion title="Skills, izin listeleri, onaylar">
    ...
    | `/learn [request]` | Geçerli konuşmadan veya adlandırılmış kaynaklardan [Skill Atölyesi](/tr/tools/skill-workshop) aracılığıyla incelenebilir bir skill taslağı oluşturur |
  ...
  <Accordion title="Alt ajanlar ve ACP">
    | Komut | Açıklama |
    | --- | --- |
    | `/subagents list\|log\|info` | Geçerli oturumun alt ajan çalıştırmalarını inceler |
    | `/acp spawn\|cancel\|steer\|close\|sessions\|status\|set-mode\|set\|cwd\|permissions\|timeout\|model\|reset-options\|doctor\|install\|help` | ACP oturumlarını ve çalışma zamanı seçeneklerini yönetir. Çalışma zamanı denetimleri, harici sahip veya dahili Gateway yöneticisi kimliği gerektirir |
    | `/focus <target>` | Geçerli Discord ileti dizisini veya Telegram konusunu bir oturum hedefine bağlar |
    | `/unfocus` | Geçerli ileti dizisi bağlamasını kaldırır |
    | `/agents` | Geçerli oturumun ileti dizisine bağlı ajanlarını listeler |
  </Accordion>

  <Accordion title="Yalnızca sahibe açık yazma işlemleri ve yönetim">
    | Komut | Gereksinim | Açıklama |
    | --- | --- | --- |
    | `/config show\|get\|set\|unset` | `commands.config: true` | `openclaw.json` öğesini okur veya yazar. Yalnızca sahip |
    | `/mcp show\|get\|set\|unset` | `commands.mcp: true` | OpenClaw tarafından yönetilen MCP sunucusu yapılandırmasını okur veya yazar. Yalnızca sahip |
    | `/plugins list\|inspect\|show\|get\|install\|enable\|disable` | `commands.plugins: true` | Plugin durumunu inceler veya değiştirir. Yazma işlemleri yalnızca sahibe açıktır. Diğer ad: `/plugin` |
    ...
    | `/restart` | `commands.restart: true` (varsayılan) | OpenClaw'u yeniden başlatır |
    | `/send on\|off\|inherit` | sahip | Gönderme politikasını ayarlar |
  </Accordion>
  ...
  <Accordion title="Ses, TTS, kanal denetimi">
    ...
    | `/bash <command>` | Ana makinede bir kabuk komutu çalıştırır. Diğer ad: `! <command>`. `commands.bash: true` gerektirir |
    | `!poll [sessionId]` | Arka plandaki bir bash işini denetler |
    | `!stop [sessionId]` | Arka plandaki bir bash işini durdurur |
  </Accordion>
</AccordionGroup>

### Yerleştirme komutları

Yerleştirme komutları, etkin oturumun yanıt rotasını bağlantılı başka bir kanala geçirir.
Kurulum ve sorun giderme için [Kanal yerleştirme](/tr/concepts/channel-docking) bölümüne bakın.
...
Yerleştirme komutları `session.identityLinks` gerektirir. Kaynak gönderen ve hedef eş
...
| Komut                                                   | Açıklama                                                                                                                                                                                      |
| ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/dreaming [on\|off\|status\|help]`                     | Bellek Dreaming özelliğini açar veya kapatır (sahip veya Gateway yöneticisi). Bkz. [Dreaming](/tr/concepts/dreaming)                                                                               |
| `/pair [qr\|status\|pending\|approve\|cleanup\|notify]` | Cihaz eşleştirmeyi yönetir. Bkz. [Eşleştirme](/tr/channels/pairing)                                                                                                                               |
| `/phone status\|arm ...\|disarm`                        | Yüksek riskli Node komutlarını (kamera/ekran/bilgisayar/yazma işlemleri) geçici olarak etkinleştirir. Bkz. [Bilgisayar kullanımı](/tr/nodes/computer-use)                                         |
| `/voice status\|list\|set <voiceId>`                    | Talk ses yapılandırmasını yönetir. Discord yerel adı: `/talkvoice`                                                                                                                       |
| `/card ...`                                             | LINE zengin kart hazır ayarlarını gönderir. Bkz. [LINE](/tr/channels/line)                                                                                                                        |
| `/codex <action> ...`                                   | Codex uygulama sunucusu test düzeneğini bağlar, yönlendirir ve inceler (durum, ileti dizileri, sürdürme, model, hızlı mod, izinler, compact, inceleme, mcp, skills ve daha fazlası). Bkz. [Codex test düzeneği](/tr/plugins/codex-harness) |

Yalnızca QQBot: `/bot-ping`, `/bot-version`, `/bot-help`, `/bot-upgrade`, `/bot-logs`

### Skill komutları

Kullanıcı tarafından çağrılabilen skill'ler eğik çizgi komutları olarak sunulur:

- `/skill <name> [input]` genel giriş noktası olarak her zaman çalışır.
- Skills doğrudan komut olarak kaydolabilir (ör. OpenProse için `/prose`).
...
<AccordionGroup>
  <Accordion title="Skill komutlarını yönlendirme">
    ...
    (belirlenimci, model katılımı yoktur). Örnek: `/prose` (OpenProse plugini)
    — bkz. [OpenProse](/tr/prose).

  </Accordion>
  <Accordion title="Yerel komut bağımsız değişkenleri">
    Discord, gerekli bağımsız değişkenler belirtilmediğinde dinamik seçenekler için otomatik tamamlamayı
    ve düğme menülerini kullanır. Telegram ve Slack, seçenekleri olan komutlar için
    bir düğme menüsü gösterir. Dinamik seçenekler hedef oturum modeline göre çözümlenir; bu nedenle
...
konuşmada kullanabilecekleri** — statik bir yapılandırma kataloğu değildir.
...
/model default     # oturumun model seçimini temizle
/model status      # uç nokta ve API modunu içeren ayrıntılı görünüm
```

Discord'da `/model` ve `/models`, sağlayıcı ve
model açılır listelerini içeren etkileşimli bir seçici açar. Seçici, `provider/*` girdileri
dâhil olmak üzere `agents.defaults.modelPolicy.allow` ayarına uyar. Açık bir izin listesi olmadığında model girdileri ve
diğer adlar seçimi kısıtlamaz.
...
Yapılandırma, yazma işleminden önce doğrulanır. Geçersiz değişiklikler reddedilir. `/config`
...
`/mcp show`; kimlik bilgileri içeren alanları, tanınan kimlik bilgisi bayrağı
değerlerini ve bilinen gizli bilgi biçimindeki bağımsız değişkenleri sansürler. Bir gruptan çalıştırıldığında
...
<Note>
  Yazma işlemleri yalnızca sahibe açıktır. Varsayılan olarak devre dışıdır — `commands.plugins: true` ile etkinleştirin.
</Note>
...
plugin çalışma zamanını çalışırken yeniden yükler. `/plugins install`, plugin kaynak modülleri
değiştiği için yönetilen Gateway'leri otomatik olarak yeniden başlatır. Güvenilir ClawHub
ve resmî katalog kurulumları ek onay gerektirmez. Rastgele npm,
git, arşiv, `npm-pack:` ve yerel yol kaynakları bir köken uyarısı gösterir ve
kaynağı inceledikten sonra sonda `--force` bulunmasını gerektirir. Bu bayrak kaynağı
onaylar ve mevcut bir kurulumun değiştirilmesine izin verir; `security.installPolicy` veya
kurucu güvenlik denetimlerini atlamaz. Risk uyarıları bulunan ClawHub sürümleri yine de
yalnızca kabukta kullanılabilen ayrı `--acknowledge-clawhub-risk` bayrağını gerektirir.
Pazar yeri, bağlantılı ve sabitlenmiş kurulumlar da yalnızca kabuktan yapılabilir.
...
`/trace`, tam ayrıntılı mod olmadan oturum kapsamındaki plugin izleme/hata ayıklama
...
- Codex test düzeneği oturumlarında, geçici bir Codex yan ileti dizisi olarak çalışır.
...
<AccordionGroup>
  <Accordion title="Yüzeye göre oturum kapsamı">
    - **Metin komutları:** normal sohbet oturumunda çalışır (DM'ler `main` öğesini paylaşır, grupların kendi oturumu vardır).
  ...
  <Accordion title="Slack'e özgü ayrıntılar">
    ...
    oluşturun. Slack, `/status` öğesini ayırdığı için `/agentstatus` öğesini (`/status` değil)
    kaydedin. `/status` metni Slack mesajlarında çalışmaya devam eder.
  </Accordion>
  ...
  <Accordion title="Bağımsız değişken notları">
    ...
    - `/new <model>`; bir model takma adını, `provider/model` veya bir sağlayıcı adını (bulanık eşleşme) kabul eder; eşleşme bulunamazsa metin mesaj gövdesi olarak değerlendirilir.
...
- `/status` içindeki **belirteç/önbellek satırları**, canlı oturum anlık görüntüsü seyrek olduğunda en son transkript kullanım girdisine geri dönebilir.
- **Yürütme ve çalışma zamanı:** `/status`, etkin korumalı alan yolu için `Execution`; oturumu kimin çalıştırdığı içinse `Runtime` bildirir: `OpenClaw Default`, `OpenAI Codex`, bir CLI arka ucu veya bir ACP arka ucu.
...
<CardGroup cols={2}>
  <Card title="Skills" href="/tr/tools/skills" icon="puzzle-piece">
    Skills eğik çizgi komutlarının nasıl kaydedildiği ve erişimlerinin nasıl denetlendiği.
  </Card>
  ...
  <Card title="Bu arada" href="/tr/tools/btw" icon="comments">
    Oturum bağlamını değiştirmeden ek sorular sorun.
  </Card>
  <Card title="Yönlendirme" href="/tr/tools/steer" icon="compass">
    `/steer` ile çalışma sırasında agent'ı yönlendirin.
  </Card>
</CardGroup>
