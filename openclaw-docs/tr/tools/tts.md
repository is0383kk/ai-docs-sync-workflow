...
read_when:
    ...
    - Bir TTS sağlayıcısını, yedek zincirini veya kişayı yapılandırma
    - /tts komutlarını veya yönergelerini kullanma
sidebarTitle: Text to speech (TTS)
summary: Giden yanıtlar için metinden sese — sağlayıcılar, kişilikler, eğik çizgi komutları ve kanal başına çıktı
title: Metinden konuşmaya
x-i18n:
    generated_at: "2026-07-27T00:22:19Z"
    model: gpt-5.6
...
ekleri; telefon ve Talk için PCM/Ulaw akışları.

TTS, Talk'ın `stt-tts` modunun konuşma çıktısı yarısıdır (`talk.speak` çağrıları da
...
<Note>
Otomatik TTS varsayılan olarak **kapalıdır**. `tts.provider` ayarlanmamışsa
...
| ----------------- | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| **Azure Speech**  | `AZURE_SPEECH_KEY` + `AZURE_SPEECH_REGION` (ayrıca `AZURE_SPEECH_API_KEY`, `SPEECH_KEY`, `SPEECH_REGION`)       | Yerel Ogg/Opus sesli not çıktısı ve telefon desteği.                                        |
| **DeepInfra**     | `DEEPINFRA_API_KEY`                                                                                               | OpenAI uyumlu TTS. Varsayılanı `hexgrad/Kokoro-82M`.                                         |
| **ElevenLabs**    | `ELEVENLABS_API_KEY` veya `XI_API_KEY`                                                                       | Ses klonlama, çok dillilik, `seed` aracılığıyla deterministik çalışma; Discord sesli oynatımı için akış halinde iletilir. |
| **Google Gemini** | `GEMINI_API_KEY` veya `GOOGLE_API_KEY`                                                                       | Gemini API toplu TTS; `promptTemplate: "audio-profile-v1"` aracılığıyla kişiliğe duyarlıdır.                  |
...
| **Yerel CLI**     | yok                                                                                                              | Yapılandırılmış yerel bir TTS komutunu çalıştırır.                                          |
| **Microsoft**     | yok                                                                                                              | `node-edge-tts` üzerinden herkese açık Edge sinirsel TTS. En iyi çaba esaslıdır, SLA yoktur. |
...
| **OpenAI**        | `OPENAI_API_KEY`                                                                                               | Otomatik özet için de kullanılır; `instructions` kişiliğini destekler.                  |
...
| **Vydra**         | `VYDRA_API_KEY`                                                                                               | Ortak görüntü, video ve konuşma sağlayıcısı.                                                 |
...
diğerleri ise yedek seçeneklerdir. Otomatik özet `summaryModel` (veya
...
<Warning>
Paketle gelen **Microsoft** sağlayıcısı, `node-edge-tts` üzerinden Microsoft Edge'in çevrimiçi
sinirsel TTS hizmetini kullanır. Bu, yayımlanmış bir SLA'sı veya kotası bulunmayan
herkese açık bir web hizmetidir; en iyi çaba esaslı kabul edin. Eski sağlayıcı kimliği `edge`,
  ...
  tts: {
    ...
    providers: {
      google: {
        ...
        // audioProfile: "Sakin bir podcast sunucusu tonuyla konuş.",
        // speakerName: "Alex",
      },
    },
  },
...
Bir agent'ın farklı bir sağlayıcı, ses, model, kişilik veya otomatik TTS moduyla konuşması
gerektiğinde `agents.entries.*.tts` kullanın. Agent bloğu `tts` üzerine
...
Kanal ve hesap geçersiz kılmaları, `tts` ile aynı yapıyı kullanır ve
...
Bir **persona**, sağlayıcılar arasında belirlenimsel olarak uygulanabilen kararlı bir sözlü kimliktir.
Belirli bir sağlayıcıyı tercih edebilir, sağlayıcıdan bağımsız istem
amacını tanımlayabilir ve sesler, modeller, istem
şablonları, seed değerleri ve ses ayarları için sağlayıcıya özgü bağlamalar taşıyabilir.
...
Sağlayıcıdan bağımsız `personas.<id>.prompt.*` yapılandırması kullanımdan kaldırılmıştır. Doctor bu
...
| `provider-defaults` | Persona, bu denemenin istem hazırlığından çıkarılır; diğer sağlayıcılara geri dönüş devam ederken sağlayıcı kendi nötr varsayılanlarını kullanır. |
| `fail`              | Bu sağlayıcı denemesini `reasonCode: "not_configured"` ve `personaBinding: "missing"` ile atlayın. Geri dönüş sağlayıcıları yine denenir.              |
...
`commands.text` ya da yerel komut kaydı etkinleştirilmiş olmalıdır.
</Note>
...
Eğik çizgi komutları, yerel geçersiz kılmaları TTS tercihleri yoluna yazar. Varsayılan değer
`~/.openclaw/settings/tts.json`; bunu `OPENCLAW_TTS_PREFS` ile geçersiz kılın. Doctor,
...
Bunlar, `tts` ile söz konusu ana makinenin etkin
`agents.entries.*.tts` bloğundan elde edilen etkili yapılandırmayı geçersiz kılar.

## Çıktı biçimleri

TTS ses teslimi kanal yetenekleri tarafından yönlendirilir. Kanal pluginleri,
...
| Hedef                                 | Biçim                                                                                                                                |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| Feishu / Matrix / Telegram / WhatsApp | Sesli not yanıtlarında tercihen **Opus** kullanılır (ElevenLabs için `opus_48000_64`, OpenAI için `opus`). 48 kHz / 64 kbps, netlik ile boyut arasında denge sağlar. |
| Diğer kanallar                        | **MP3** (ElevenLabs için `mp3_44100_128`, OpenAI için `mp3`). 44.1 kHz / 128 kbps, konuşma için varsayılan dengeyi sağlar.                  |
...
- `mode: "final"` içinde, metin akışı tamamlandıktan sonra akışla iletilen son yanıtlar için
...
- **Özet kapalıysa**, özetleme başarısız olursa veya özet modeli için API anahtarı
  yoksa: metni `maxLength` karaktere kısaltır ve kısaltılmış metni
  sentezler.
...
Sağlayıcı `apiKey` alanları ham dizeler veya SecretRef'ler olabilir. Soğuk Gateway
...
ve çözümlenmiş değerler yine başlatmanın başarısız olmasına veya güncellemenin reddedilmesine neden olur.
  ...
  <Accordion title="Azure Speech">
    ...
    <ParamField path="endpoint" type="string">İsteğe bağlı Azure Speech uç noktası geçersiz kılması (diğer adı `baseUrl`).</ParamField>
    <ParamField path="speakerVoice" type="string">Azure ses ShortName değeri. Varsayılan `en-US-JennyNeural`. Eski diğer ad: `voice`.</ParamField>
  ...
  <Accordion title="ElevenLabs">
    ...
    <ParamField path="seed" type="number">En iyi çabayla belirlenimlilik için tamsayı `0..4294967295`.</ParamField>
  ...
  <Accordion title="Google Gemini">
    ...
    <ParamField path="speakerName" type="string">İstem adlandırılmış bir konuşmacı kullanıyorsa seslendirilecek metnin başına eklenen isteğe bağlı konuşmacı etiketi.</ParamField>
    <ParamField path="promptTemplate" type='"audio-profile-v1"'>Etkin persona istemi alanlarını deterministik bir Gemini TTS istem yapısıyla sarmalamak için `audio-profile-v1` olarak ayarlayın.</ParamField>
  ...
  <Accordion title="Inworld">
    ...
    <ParamField path="modelId" type="string">Varsayılan: `inworld-tts-1.5-max`. Diğerleri: `inworld-tts-1.5-mini`, `inworld-tts-1-max`, `inworld-tts-1`.</ParamField>
  ...
  <Accordion title="Microsoft (API anahtarı yok)">
    ...
    <ParamField path="speakerVoice" type="string">Microsoft nöral ses adı (ör. `en-US-MichelleNeural`). Eski takma ad: `voice`. Varsayılan İngilizce ses etkinse ve yanıt metninde CJK karakterleri baskınsa OpenClaw otomatik olarak `zh-CN-XiaoxiaoNeural` değerine geçer.</ParamField>
    <ParamField path="lang" type="string">Dil kodu (ör. `en-US`).</ParamField>
    <ParamField path="outputFormat" type="string">Microsoft çıktı biçimi. Varsayılan: `audio-24khz-48kbitrate-mono-mp3`. Tüm biçimler, paketle birlikte sunulan Edge tabanlı aktarım tarafından desteklenmez.</ParamField>
    ...
    <ParamField path="timeoutMs" type="number">İstek zaman aşımını geçersiz kılma değeri (ms).</ParamField>
  ...
  <Accordion title="Volcengine (BytePlus Seed Speech)">
    ...
    <ParamField path="resourceId" type="string">Varsayılan: `seed-tts-1.0`. Ortam değişkeni: `VOLCENGINE_TTS_RESOURCE_ID`. Projeniz TTS 2.0 yetkisine sahipse `seed-tts-2.0` kullanın.</ParamField>
    ...
    <ParamField path="speedRatio" type="number">Sağlayıcıya özgü hız oranı, `0.2..3`.</ParamField>
  ...
  <Accordion title="xAI">
    ...
    <ParamField path="speed" type="number">Sağlayıcıya özgü hız geçersiz kılma değeri, `0.7..1.5`.</ParamField>
  </Accordion>
  ...
  <Accordion title="Xiaomi MiMo">
    ...
    <ParamField path="model" type="string">Varsayılan: `mimo-v2.5-tts`. Ortam değişkeni: `XIAOMI_TTS_MODEL`. `mimo-v2.5-tts-voicedesign` değerini de destekler.</ParamField>
    <ParamField path="speakerVoice" type="string">Hazır ses modelleri için varsayılan: `mimo_default`. Ortam değişkeni: `XIAOMI_TTS_VOICE`. Eski takma ad: `voice`. `mimo-v2.5-tts-voicedesign` için gönderilmez.</ParamField>
...
| Yöntem            | Amaç                                      |
| ----------------- | -------------------------------------------- |
| `tts.status`      | Geçerli TTS durumunu ve son denemeyi okuyun.     |
| `tts.enable`      | Yerel otomatik tercihi `always` olarak ayarlayın.       |
| `tts.disable`     | Yerel otomatik tercihi `off` olarak ayarlayın.          |
| `tts.convert`     | Tek seferlik metin → ses dönüştürme.                        |
| `tts.setProvider` | Yerel sağlayıcı tercihini ayarlayın.               |
| `tts.personas`    | Yapılandırılmış personaları ve etkin olanı listeleyin. |
| `tts.setPersona`  | Yerel persona tercihini ayarlayın.                |
| `tts.providers`   | Yapılandırılmış sağlayıcıları ve durumlarını listeleyin.        |

## Hizmet bağlantıları

- [OpenAI metinden sese dönüştürme kılavuzu](https://platform.openai.com/docs/guides/text-to-speech)
- [OpenAI Audio API referansı](https://platform.openai.com/docs/api-reference/audio)
- [Azure Speech REST metinden sese dönüştürme](https://learn.microsoft.com/azure/ai-services/speech-service/rest-text-to-speech)
- [Azure Speech sağlayıcısı](/tr/providers/azure-speech)
- [ElevenLabs Metinden Sese Dönüştürme](https://elevenlabs.io/docs/api-reference/text-to-speech)
...
- [Microsoft Speech çıkış biçimleri](https://learn.microsoft.com/azure/ai-services/speech-service/rest-text-to-speech#audio-outputs)
- [xAI metinden sese dönüştürme](https://docs.x.ai/developers/rest-api-reference/inference/voice#text-to-speech-rest)
