---
read_when:
    - OpenClaw'u Network Security Group ile güçlendirilmiş olarak Azure üzerinde 7/24 çalıştırmak istiyorsunuz
...
summary: Kalıcı durumla bir Azure Linux VM üzerinde OpenClaw Gateway'i 7/24 çalıştırın
title: Azure
x-i18n:
    generated_at: "2026-07-26T22:49:24Z"
    model: gpt-5.6
...
<Steps>
  ...
  <Step title="VM'yi oluşturun">
    ...
    `--public-ip-address ""` genel IP atanmasını önler. Alt ağ düzeyindeki NSG güvenliği zaten yönettiği için `--nsg ""` NIC başına NSG oluşturulmasını atlar.

    `latest` yerine belirli bir Ubuntu görüntü sürümünü sabitlemek için önce kullanılabilir sürümleri listeleyin:
...
<Steps>
  ...
  <Step title="OpenClaw'ı yükleyin (VM kabuğunda)">
    ```bash
    ...
    Yükleyici, henüz mevcut değillerse Node'u ve bağımlılıkları yükler, OpenClaw'ı yükler ve ilk katılım sürecini başlatır. Ayrıntılar için [Yükleme](/tr/install) bölümüne bakın.
  ...
  <Step title="Gateway'i doğrulayın">
    ...
    Kuruluşunuzun hâlihazırda GitHub Copilot lisansları varsa ilk katılım sırasında ayrı bir model API anahtarı yerine GitHub Copilot sağlayıcısını seçebilirsiniz. Bkz. [GitHub Copilot sağlayıcısı](/tr/providers/github-copilot).
...
- GitHub Copilot model sağlayıcısıyla Azure dağıtımı hakkında daha fazla ayrıntı: [GitHub Copilot ile Azure üzerinde OpenClaw](https://github.com/johnsonshi/openclaw-azure-github-copilot)
