---
read_when:
    - OpenClaw'ı Northflank'e dağıtma
    - Tarayıcı tabanlı Control UI ile tek tıklamalı bir bulut dağıtımı istiyorsunuz
summary: OpenClaw'ı Northflank üzerinde tek tıklamalı şablonla dağıtın
title: Northflank
x-i18n:
    generated_at: "2026-04-23T09:04:33Z"
    model: gpt-5.4
    provider: openai
...
OpenClaw'ı Northflank üzerinde tek tıklamalı bir şablonla dağıtın ve web Control UI üzerinden erişin.
...
- `openclaw.json`,
  ...
  çalışma alanının yeniden dağıtımlarda kalıcı olması için Northflank Volume (`/data`) üzerinden kalıcı depolama
