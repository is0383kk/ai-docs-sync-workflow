...
`scripts/` yerel iş akışları ve operasyon görevleri için yardımcı betikleri içerir. Bir görev açıkça bir betikle bağlantılıysa bunları kullanın; aksi takdirde CLI'ı tercih edin.

## Kurallar

- Betikler, belgelerde veya sürüm kontrol listelerinde belirtilmedikçe **isteğe bağlıdır**.
...
Genel model kimlik doğrulaması [Kimlik Doğrulama](/tr/gateway/authentication) bölümünde ele alınmaktadır. Aşağıdaki betikler, uzak/başsız bir ana makinede **Claude Code CLI abonelik tokenını** izlemek ve bir telefondan yeniden kimlik doğrulaması yapmak için ayrı ve isteğe bağlı bir sistemdir:
...
- `scripts/termux-quick-auth.sh`, `scripts/termux-auth-widget.sh`, `scripts/termux-sync-widget.sh` - ana makineye SSH ile bağlanan, durum bildirimini gösteren ve kimlik doğrulamasının süresi dolduğunda yeniden kimlik doğrulama konsolunu/talimatlarını açan Termux:Widget betikleri.

## GitHub okuma yardımcısı

Normal `gh` yazma işlemleri için kişisel oturumunuzda kalırken, depo kapsamındaki okuma çağrılarında `gh` için bir GitHub App kurulum tokenı kullanmak istediğinizde `scripts/gh-read` kullanın.
