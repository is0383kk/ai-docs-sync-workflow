...
read_when:
    ...
    - Desteklenen mesajlaşma platformlarına hızlıca göz atmanız gerekiyor
summary: OpenClaw'ın bağlanabildiği mesajlaşma platformları
title: Sohbet kanalları
x-i18n:
    generated_at: "2026-07-26T23:29:32Z"
    model: gpt-5.6
...
OpenClaw, hâlihazırda kullandığınız herhangi bir sohbet uygulaması üzerinden sizinle iletişim kurabilir. Her kanal Gateway üzerinden bağlanır.
...
veya `openclaw onboard` / `openclaw channels add` sırasında isteğe bağlı olarak kurulur ve ardından Gateway'in
yeniden başlatılması gerekir. "Haricî plugin" kanalları OpenClaw deposunun dışında sürdürülür.
...
- [iMessage](/tr/channels/imessage) - Çekirdeğe dâhildir. Oturum açılmış bir Mac'te `imsg` köprüsü üzerinden yerel macOS entegrasyonu (veya Gateway başka bir yerde çalışıyorsa SSH sarmalayıcısı); yanıtlar, tapback'ler, efektler, ekler ve grup yönetimi için özel API eylemleri dâhildir.
- [IRC](/tr/channels/irc) - Klasik IRC sunucuları; eşleştirme/izin listesi denetimleriyle kanallar + DM'ler (resmî plugin).
...
- [Reef](/channels/reef) - Farklı kişilere ait OpenClaw ajanları arasında korumalı, uçtan uca şifrelenmiş claw'dan claw'a mesajlaşma (paketle gelen plugin).
...
- [Telegram](/tr/channels/telegram) - Çekirdeğe dâhildir. grammY üzerinden Bot API; grupları destekler.
...
- [WebChat](/tr/web/webchat) - Çekirdeğe dâhildir. WebSocket üzerinden Gateway WebChat kullanıcı arayüzü.
- [WeChat](/tr/channels/wechat) - QR ile oturum açma üzerinden Tencent iLink botu; yalnızca özel sohbetler (haricî plugin).
- [WhatsApp](/tr/channels/whatsapp) - En popüler seçenek; Baileys kullanır ve QR eşleştirmesi gerektirir (resmî plugin).
- [Yuanbao](/tr/channels/yuanbao) - Tencent Yuanbao botu (haricî plugin).
- [Zalo](/tr/channels/zalo) - Zalo Bot API; Vietnam'ın popüler mesajlaşma uygulaması (resmî plugin).
- [Zalo ClawBot](/tr/channels/zaloclawbot) - QR ile oturum açma üzerinden kişisel Zalo asistanı; sahibine bağlıdır (haricî plugin).
...
- `![alt](url)` gibi Markdown resim söz dizimi içeren Telegram yanıtları,
  mümkün olduğunda son giden yolunda medya yanıtlarına dönüştürülür.
...
- WhatsApp kurulumu isteğe bağlı kurulum şeklindedir: ilk katılım, plugin paketi
  kurulmadan önce kurulum akışını gösterebilir ve Gateway, haricî
...
- Desteklenen sürekli etkin odalar, ajandan söz edilmeyen oda sohbetlerinin,
...
- Kanallar eş zamanlı olarak çalışabilir; birden fazlasını yapılandırdığınızda OpenClaw sohbet bazında yönlendirir.
- En hızlı kurulum genellikle **Telegram**'dır (basit bot belirteci, plugin kurulumu yoktur). WhatsApp
  QR eşleştirmesi gerektirir ve diskte daha fazla durum verisi saklar.
