...
summary: ClawHub oturum açma, API belirteçleri, CLI oturumu, belirteç depolama ve iptal etme.
x-i18n:
    generated_at: "2026-07-26T23:13:53Z"
    model: gpt-5.6
...
ClawHub, web oturum açma işlemi için GitHub'ı kullanır. CLI, oturum açılmış bu hesap
...
işlemini tamamlayamaz. Oturum açma işlemi sizi oturumun kapalı olduğu duruma geri
döndürürse hesabınızın durumu uygun olmayabilir. Hesabınız yasaklandıysa veya devre
dışı bırakıldıysa bunun bir hata olduğunu düşünüyorsanız
...
3. GitHub ile oturum açıldıktan sonra ClawHub bir API token'ı oluşturur.
4. Tarayıcı yerel geri çağırma adresine yönlendirilir.
5. CLI, token'ı ClawHub yapılandırma dosyanızda saklar.

Tarayıcınız güvenlik duvarı, VPN veya proxy kuralları nedeniyle yerel geri çağırma
adresine ulaşamıyorsa başsız token akışını kullanın.
...
İptal edilmiş, geçersiz veya eksik token'lar `401 Unauthorized` döndürür.
...
kullanmaya devam edemez. Hesabınız yasaklandıysa veya devre dışı bırakıldıysa bunun
