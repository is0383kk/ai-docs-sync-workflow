...
read_when:
    ...
    - Go'da barındırılan modeller için çalışma zamanı model referanslarına ihtiyacınız var
summary: Paylaşılan OpenCode kurulumuyla OpenCode Go kataloğunu kullanın
title: OpenCode Go
x-i18n:
    generated_at: "2026-07-27T00:14:42Z"
    model: gpt-5.6
...
`OPENCODE_API_KEY` kimlik bilgisini paylaşır, ancak yukarı akıştaki model başına yönlendirmenin
doğru kalması için kendi çalışma zamanı sağlayıcı kimliğini (`opencode-go`) korur.

| Özellik                   | Değer                                              |
| ------------------------- | -------------------------------------------------- |
| Çalışma zamanı sağlayıcısı | `opencode-go`                                 |
| Kimlik doğrulama          | `OPENCODE_API_KEY` (takma ad: `OPENCODE_ZEN_API_KEY`) |
...
Geçerli model listesi için `openclaw models list --provider opencode-go` komutunu çalıştırın.
Paketle gelen satırlar:

| Model referansı                 | Ad                | Bağlam    | En yüksek çıktı | Görüntü girdisi |
| ------------------------------- | ----------------- | --------- | --------------- | --------------- |
...
<AccordionGroup>
  <Accordion title="Yönlendirme davranışı">
    OpenClaw, tüm `opencode-go/...` model referanslarını otomatik olarak yönlendirir. Ek bir
    sağlayıcı yapılandırması gerekmez.
  </Accordion>

  <Accordion title="Çalışma zamanı referansı kuralı">
    Çalışma zamanı referansları açıkça belirtilir: Zen için `opencode/...`, Go için
    `opencode-go/...`. Bu, her iki katalogda da yukarı akıştaki model başına yönlendirmenin doğru kalmasını sağlar.
  </Accordion>
  ...
  <Accordion title="Paylaşılan kimlik bilgileri">
    ...
    anahtarın girilmesi, kimlik bilgilerini her iki çalışma zamanı sağlayıcısı için de kaydeder.
  </Accordion>
</AccordionGroup>

<Tip>
Paylaşılan ilk kurulum genel bakışı ve eksiksiz Zen + Go katalog referansı için
...
<CardGroup cols={2}>
  <Card title="OpenCode (üst öğe)" href="/tr/providers/opencode" icon="server">
    Paylaşılan ilk kurulum, katalog genel bakışı ve gelişmiş notlar.
  </Card>
  <Card title="Model seçimi" href="/tr/concepts/model-providers" icon="layers">
    Sağlayıcıları ve model referanslarını seçme ile yük devretme davranışı.
  </Card>
</CardGroup>
