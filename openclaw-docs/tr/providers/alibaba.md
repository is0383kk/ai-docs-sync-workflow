...
read_when:
    ...
    - Video oluşturmak için Model Studio veya DashScope API anahtarı yapılandırması gerekir
summary: OpenClaw'da Alibaba Model Studio Wan video oluşturma
title: Alibaba Model Studio
x-i18n:
    generated_at: "2026-07-26T23:35:19Z"
    model: gpt-5.6
...
| Plugin                  | paketle birlikte, `enabledByDefault: true`                                            |
...
<Steps>
  <Step title="Bir API anahtarı ayarlayın">
    İlk kurulum aracılığıyla anahtarı `alibaba` sağlayıcısına kaydedin:
  ...
  <Step title="Sağlayıcının yapılandırıldığını doğrulayın">
    ...
    Liste, paketle birlikte gelen beş Wan modelinin tamamını içerir. `MODELSTUDIO_API_KEY` çözümlenemiyorsa `openclaw models status --json`, eksik kimlik bilgisini `auth.unusableProfiles` altında bildirir.

  </Step>
</Steps>

<Note>
  Alibaba Plugin'i ve [Qwen Plugin'i](/tr/providers/qwen), DashScope üzerinden kimlik doğrular ve birbiriyle örtüşen ortam değişkenlerini kabul eder. Özel Wan video yüzeyi için `alibaba/...` model kimliklerini; Qwen sohbeti, gömme veya medya anlama için `qwen/...` kimliklerini kullanın.
</Note>
...
Üç modun tümünde istek başına video sayısı ve süre üst sınırı aynıdır; yalnızca girdi biçimi farklıdır.

| Mod                  | En fazla çıktı videosu | En fazla girdi görüntüsü | En fazla girdi videosu | En fazla süre | Desteklenen denetimler                                    |
| -------------------- | ---------------------- | ------------------------ | ---------------------- | ------------ | --------------------------------------------------------- |
| Metinden videoya     | 1                      | yok                      | yok                    | 10 s         | `size`, `aspectRatio`, `resolution`, `audio`, `watermark` |
| Görüntüden videoya   | 1                      | 1                        | yok                    | 10 s         | `size`, `aspectRatio`, `resolution`, `audio`, `watermark` |
| Referanstan videoya  | 1                      | yok                      | 4                      | 10 s         | `size`, `aspectRatio`, `resolution`, `audio`, `watermark` |

`durationSeconds` belirtilmeyen bir istekte DashScope'un kabul ettiği varsayılan değer olan **5 saniye** kullanılır. Süreyi 10 s'ye kadar uzatmak için [video oluşturma aracında](/tr/tools/video-generation) `durationSeconds` değerini açıkça ayarlayın.

<Warning>
  Referans görüntü ve video girdileri uzak `http(s)` URL'leri olmalıdır; DashScope'un referans modları yerel dosya yollarını reddeder. Önce nesne depolama alanına yükleyin veya zaten genel bir URL üreten [medya aracı](/tr/tools/media-overview) akışını kullanın.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Kimlik doğrulama ortam değişkeni önceliği">
    ...
    Yapılandırılmış `auth.profiles` girdileri (`openclaw models auth login` aracılığıyla ayarlanır) ortam değişkeni çözümlemesini geçersiz kılar. Profil döndürme, bekleme süresi ve geçersiz kılma mekanizmaları için [modeller SSS bölümündeki kimlik doğrulama profillerine](/tr/help/faq-models#auth-profiles-what-they-are-and-how-to-manage-them) bakın.

  </Accordion>

  <Accordion title="Qwen Plugin'iyle ilişkisi">
    Paketle birlikte gelen her iki Plugin de DashScope ile iletişim kurar ve birbiriyle örtüşen API anahtarlarını kabul eder. Şunları kullanın:

    - `alibaba/wan*.*` kimlikleri, bu sayfada belgelenen özel Wan video sağlayıcısı içindir.
    - `qwen/*` kimlikleri, Qwen sohbeti, gömme ve medya anlama içindir (bkz. [Qwen](/tr/providers/qwen)).

    Kimlik doğrulama ortam değişkeni listeleri bilinçli olarak birbiriyle örtüştüğünden, `MODELSTUDIO_API_KEY` değerini bir kez ayarlamak her iki Plugin'in de kimliğini doğrular; her Plugin için ayrı ayrı ilk kurulum yapılması gerekmez.
