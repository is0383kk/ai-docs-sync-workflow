...
summary: OpenClaw'u SGLang ile çalıştırma (OpenAI uyumlu, kendi sunucunuzda barındırılan sunucu)
title: SGLang
x-i18n:
    generated_at: "2026-07-26T22:59:50Z"
    model: gpt-5.6
...
SGLang, açık ağırlıklı modelleri OpenAI uyumlu bir HTTP API üzerinden sunar. OpenClaw, kullanılabilir modellerin otomatik keşfiyle `openai-completions` sağlayıcı ailesini kullanarak SGLang'a bağlanır.

| Özellik                   | Değer                                                        |
| ------------------------- | ------------------------------------------------------------ |
| Sağlayıcı kimliği         | `sglang`                                                     |
| Plugin                    | paketle birlikte sunulur, `enabledByDefault: true`                            |
...
| Fiyatlandırma             | Harici ve ücretsiz olarak işaretlenir (`modelPricing.external: false`)        |

OpenClaw ayrıca `SGLANG_API_KEY` ile etkinleştirdiğinizde SGLang'daki kullanılabilir modelleri **otomatik olarak keşfeder**. Özel bir SGLang temel URL'si de yapılandırırken keşfi dinamik tutmak için `agents.defaults.models` içinde `sglang/*` kullanın. Aşağıdaki [Model keşfi (örtük sağlayıcı)](#model-discovery-implicit-provider) bölümüne bakın.
...
`models.providers.sglang` tanımlanmadığında OpenClaw şunu sorgular:
...
bildirdiğiniz modelleri kullanır. OpenClaw'ın yapılandırılmış sağlayıcının
`/models` uç noktasını sorgulamasını ve duyurulan tüm SGLang modellerini
dahil etmesini istediğinizde `agents.defaults.models` öğesine `"sglang/*": {}` ekleyin.
</Note>
...
<AccordionGroup>
  <Accordion title="Proxy tarzı davranış">
    SGLang, yerel bir OpenAI uç noktası olarak değil, proxy tarzı OpenAI uyumlu bir
    `/v1` arka ucu olarak değerlendirilir.

    | Davranış | SGLang |
    |----------|--------|
    | Yalnızca OpenAI'a özgü istek biçimlendirmesi | Uygulanmaz |
    | `service_tier`, Responses `store`, istem önbelleği ipuçları | Gönderilmez |
    | Akıl yürütme uyumluluğu yük biçimlendirmesi | Uygulanmaz |
  ...
  <Accordion title="Sorun giderme">
    ...
    **Kimlik doğrulama hataları**
    ...
    yapılandırmanızla eşleşen gerçek bir `SGLANG_API_KEY` ayarlayın veya
    sağlayıcıyı `models.providers.sglang` altında açıkça yapılandırın.

    <Tip>
    SGLang'ı kimlik doğrulama olmadan çalıştırıyorsanız model keşfini
    etkinleştirmek için `SGLANG_API_KEY` değişkeninde boş olmayan herhangi bir değer yeterlidir.
    </Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="Yapılandırma referansı" href="/tr/gateway/configuration-reference" icon="gear">
    Sağlayıcı girdilerini de içeren eksiksiz yapılandırma şeması.
  </Card>
</CardGroup>
