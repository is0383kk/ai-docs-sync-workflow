...
title: Vercel AI gateway'i
x-i18n:
    generated_at: "2026-07-27T00:16:01Z"
    model: gpt-5.6
...
[Vercel AI Gateway](https://vercel.com/ai-gateway), yüzlerce modele tek bir uç nokta üzerinden
erişmek için birleşik bir API sağlar.
...
<Tip>
OpenClaw, Gateway `/v1/models` kataloğunu otomatik olarak keşfeder; böylece hem
`/models vercel-ai-gateway` sohbet komutu hem de
`openclaw models list --provider vercel-ai-gateway`, `vercel-ai-gateway/openai/gpt-5.5` ve
...
OpenClaw, Claude kısa gösterimli model referanslarını çalışma zamanında normalleştirir:

| Kısa gösterimli girdi               | Normalleştirilmiş model referansı              |
| ----------------------------------- | --------------------------------------------- |
...
<AccordionGroup>
  <Accordion title="Daemon işlemleri için ortam değişkeni">
    ...
    `AI_GATEWAY_API_KEY` değişkeninin bu işlem tarafından kullanılabildiğinden emin olun.

    <Warning>
    Yalnızca etkileşimli bir kabukta dışa aktarılan anahtar, ortam açıkça içe
    aktarılmadığı sürece launchd/systemd daemon'u tarafından görülemez. Gateway
    işleminin anahtarı okuyabilmesini sağlamak için anahtarı `~/.openclaw/.env`
  ...
  <Accordion title="Sağlayıcı yönlendirmesi">
    Vercel AI Gateway, her isteği model referansı önekinde adı belirtilen üst
    ...
    `vercel-ai-gateway/moonshotai/kimi-k2.6`, MoonshotAI üzerinden yönlendirilir. Tek bir
    `AI_GATEWAY_API_KEY`, tüm üst sağlayıcılar için kimlik doğrulaması yapar.
  </Accordion>
  <Accordion title="Düşünme düzeyleri">
    OpenClaw öneki tanıdığında `/think` seçenekleri üst model önekini
    izler. `vercel-ai-gateway/anthropic/...`, Claude 4.6 modellerinin uyarlanabilir varsayılanı
    dâhil olmak üzere Claude düşünme profilini kullanır. Güvenilir
    `vercel-ai-gateway/openai/...` referansları (`gpt-5.2` ve daha yenileri ile
    `gpt-5.1-codex` sürümüne kadar Codex varyantları), `/think xhigh`
