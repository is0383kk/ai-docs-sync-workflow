...
summary: OpenCode Zen ve Go kataloglarını OpenClaw ile kullanma
title: OpenCode
x-i18n:
    generated_at: "2026-07-27T00:15:36Z"
    model: gpt-5.6
...
Her iki katalog da tek bir OpenCode API anahtarını paylaşır (`OPENCODE_API_KEY`, diğer adı
`OPENCODE_ZEN_API_KEY`). OpenClaw, üst sistemde model başına yönlendirmenin doğru kalması için
çalışma zamanı sağlayıcı kimliklerini ayrı tutar; ancak ilk katılım ve belgelerde bunlar
tek bir OpenCode kurulumu olarak ele alınır.

## Başlarken

<Tabs>
  <Tab title="Zen kataloğu">
    **En uygun kullanım:** özenle seçilmiş OpenCode çok modelli proxy'si (Claude, GPT, Gemini, GLM,
  ...
  <Tab title="Go kataloğu">
    **En uygun kullanım:** OpenCode tarafından barındırılan Kimi, GLM, MiniMax, Qwen ve DeepSeek model yelpazesi.
...
satırlarını da içeren güncel listenin tamamı için `openclaw models list --provider opencode`
komutunu çalıştırın.
...
Go modellerinin tam tablosu için [OpenCode Go](/tr/providers/opencode-go) sayfasına bakın.
...
<AccordionGroup>
  ...
  <Accordion title="Paylaşılan kimlik bilgileri">
    ...
    sağlayıcısının kimlik bilgileri de saklanır. Her katalog için ayrı ayrı ilk katılım
    yapmanız gerekmez.
  </Accordion>

  <Accordion title="API anahtarı edinme">
    Bir OpenCode hesabı oluşturun ve
    [opencode.ai/auth](https://opencode.ai/auth) adresinde bir API anahtarı oluşturun. Faturalandırma ve katalog
    kullanılabilirliği OpenCode kontrol panelinden yönetilir.
  </Accordion>

  <Accordion title="Gemini yeniden oynatma davranışı">
    Gemini destekli OpenCode başvuruları proxy-Gemini yolunda kalır; böylece OpenClaw,
  ...
  <Accordion title="Gemini dışı yeniden oynatma davranışı">
    Gemini dışı OpenCode başvuruları, asgari OpenAI uyumlu yeniden oynatma politikasını korur.
  </Accordion>
</AccordionGroup>
