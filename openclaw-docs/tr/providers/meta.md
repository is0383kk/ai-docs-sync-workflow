---
read_when:
    - OpenClaw ile Meta kullanmak istiyorsunuz
    - MODEL_API_KEY ortam değişkeni veya CLI kimlik doğrulama seçeneği gereklidir
summary: Meta kurulumu (kimlik doğrulama + muse-spark-1.1 model seçimi)
title: Meta
x-i18n:
    generated_at: "2026-07-26T23:37:14Z"
    model: gpt-5.6
...
**Meta API**, `muse-spark-1.1` akıl yürütme modeli için OpenAI uyumlu **Responses API**'yi (`POST /v1/responses`) kullanır. Sağlayıcı, paketlenmiş bir OpenClaw plugin'i olarak sunulur.

| Özellik               | Değer                              |
| --------------------- | ---------------------------------- |
| Sağlayıcı kimliği     | `meta`                 |
| Plugin                | paketlenmiş sağlayıcı              |
...
```
  ...
  <Step title="Modellerin kullanılabilir olduğunu doğrulayın">
    ...
    Statik `muse-spark-1.1` katalog girdisini listeler. `MODEL_API_KEY` çözümlenemezse
    `openclaw models status --json`, eksik kimlik doğrulama bilgisini
...
| Model referansı       | Ad             | Akıl yürütme | Bağlam penceresi | Azami çıktı |
| --------------------- | -------------- | ------------ | ---------------- | ----------- |
...
- Akıl yürütme çabası: `minimal`, `low`, `medium`, `high`, `xhigh` (varsayılan: `high`)
- Durumsuz şifreli akıl yürütme yeniden oynatımı (`store: false`, `include: ["reasoning.encrypted_content"]`)

<Warning>
`muse-spark-1.1`, `reasoning.effort: "none"` kabul etmez. OpenClaw, bu sağlayıcı için
`--thinking off` değerini `minimal` değerine eşler.
</Warning>
...
`MODEL_API_KEY` değerinin bu işlem tarafından kullanılabildiğinden emin olun; örneğin
`~/.openclaw/.env` içinde veya `env.shellEnv` aracılığıyla. Yalnızca etkileşimli bir
kabukta dışa aktarılan anahtar, ortam ayrı olarak içe aktarılmadıkça yönetilen bir
hizmete yardımcı olmaz.
</Note>
...
Canlı testler, `POST /v1/responses` üzerinde `muse-spark-1.1` kullanır.
...
<CardGroup cols={2}>
  ...
  <Card title="Düşünme modları" href="/tr/tools/thinking" icon="brain">
    muse-spark-1.1 için akıl yürütme çabası düzeyleri.
  </Card>
