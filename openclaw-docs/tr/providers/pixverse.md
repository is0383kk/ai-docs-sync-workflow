...
<Steps>
  ...
  <Step title="API anahtarını ayarlayın">
    ...
    Sihirbaz, sağlayıcı yapılandırmasına `region` ve `baseUrl` yazmadan önce Uluslararası veya CN uç noktasını (aşağıdaki API bölümü bölümüne bakın) sorar.
    Etkileşimsiz çalıştırmalar (`--pixverse-api-key` veya `PIXVERSE_API_KEY` üzerinden alınan anahtar)
    varsayılan olarak Uluslararası bölgeyi kullanır.

    İlk kurulum, henüz varsayılan bir video modeli yapılandırılmamışsa
    `agents.defaults.mediaModels.video.primary` değerini de
    `pixverse/v6` olarak ayarlar.
  ...
  <Step title="Video üretin">
    Agent'tan bir video üretmesini isteyin. PixVerse otomatik olarak kullanılır.
  </Step>
</Steps>
...
Yerel görüntü referansları, görüntüden videoya isteğinden önce PixVerse'e yüklenir. Uzak görüntü URL'leri, `image_url` olarak PixVerse görüntü yükleme uç noktasına aktarılır.

| Seçenek          | Desteklenen değerler                                                                                                                 |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| Süre        | 1-15 saniye (varsayılan 5)                                                                                                         |
| Çözünürlük      | `360P`, `540P`, `720P`, `1080P` (varsayılan `540P`; `480P` istekleri `540P` ile eşlenir)                                                  |
...
<Note>
PixVerse görüntü şablonu üretimi henüz `image_generate` üzerinden sunulmamaktadır. Bu API şablon kimliğiyle çalışırken OpenClaw'ın paylaşılan görüntü üretimi sözleşmesinde şu anda PixVerse'e özgü türü belirlenmiş bir seçenek paketi yoktur.
</Note>
...
| Seçenek                               | Tür   | Etki                                        |
| ------------------------------------ | ------ | --------------------------------------------- |
| `seed`                               | sayı | Belirlenimci başlangıç değeri, 0 ile 2147483647 arası           |
| `negativePrompt` / `negative_prompt` | dize | Negatif istem                               |
| `quality`                            | dize | `720p` gibi PixVerse kalite ayarı               |
...
<AccordionGroup>
  <Accordion title="API bölgesi">
    ...
    `models.providers.pixverse.region` değerini elle ayarlayın veya kurulum sihirbazında
    bir bölge seçmek için
    ...
    ```json5
    {
      models: {
        providers: {
          pixverse: {
            region: "cn", // "international" veya "cn"
  ...
  <Accordion title="Görev yoklaması">
    ...
    başarılı olana, başarısız olana veya zaman aşımına ulaşana kadar her 5 saniyede bir
    `/openapi/v2/video/result/{video_id}` yoklaması yapar (varsayılan 5 dakika; değiştirmek için
    `agents.defaults.mediaModels.video.timeoutMs` kullanın).
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Yapılandırma referansı" href="/tr/gateway/config-agents#agent-defaults" icon="gear">
    Video üretim modeli dahil Agent varsayılan ayarları.
  </Card>
</CardGroup>
