...
read_when:
    - 向新手介紹 OpenClaw
summary: OpenClaw 是可在任何作業系統上執行的 AI 代理多管道閘道。
title: OpenClaw
x-i18n:
    generated_at: "2026-07-26T08:35:47Z"
    model: gpt-5.6
...
<p align="center">
  <strong>跨越 Discord、Google Chat、iMessage、Matrix、Microsoft Teams、Signal、Slack、Telegram、WhatsApp、Zalo 等平台，適用於任何作業系統的 AI 代理程式閘道。</strong><br />
  傳送訊息，即可從口袋中的裝置取得代理程式回覆。以單一閘道執行多個頻道外掛、WebChat 與行動節點。<br />
  由非營利組織 <a href="https://openclaw.org">OpenClaw 基金會</a>公開開發。
</p>
...
<Columns>
  ...
  <Card title="連接頻道" href="/zh-TW/channels" icon="message-circle">
    連結 Discord、Signal、Telegram、WhatsApp 等服務，隨時隨地開始聊天。
  </Card>
  <Card title="開啟控制介面" href="/zh-TW/web/control-ui" icon="layout-dashboard">
    啟動瀏覽器儀表板，以進行聊天、設定與工作階段管理。
  </Card>
</Columns>

## 瀏覽文件

行動瀏覽器可能只會顯示區段選單，而不會顯示完整的桌面版分頁列。請使用
...
OpenClaw 是一個**自行託管的閘道**，透過頻道外掛，將你喜愛的聊天應用程式（Discord、Google Chat、iMessage、Matrix、Microsoft Teams、Signal、Slack、Telegram、WhatsApp、Zalo 等）連接至 AI 程式設計代理程式。你可以在自己的電腦（或伺服器）上執行單一閘道程序，使其成為訊息應用程式與隨時可用的 AI 助理之間的橋樑。

**適合哪些人？** 希望擁有能從任何地方傳訊息互動的個人 AI 助理，同時不放棄資料控制權，也不依賴託管服務的開發人員與進階使用者。

**有何不同？**

- **自行託管**：在你的硬體上，依照你的規則執行
- **多頻道**：單一閘道可同時服務所有已設定的頻道外掛
- **代理程式原生**：專為支援工具使用、工作階段、記憶與多代理程式路由的程式設計代理程式打造
- **開放原始碼**：採用 MIT 授權，由社群共同推動

**需要什麼？** Node 24.15+（建議）、為相容性使用 Node 22 LTS（`22.22.3+`），或 Node 25.9+、所選供應商提供的 API 金鑰，以及 5 分鐘。為獲得最佳品質與安全性，請使用可用的最強最新世代模型。
...
<Columns>
  <Card title="多頻道閘道" icon="network" href="/zh-TW/channels">
    透過單一閘道程序使用 Discord、iMessage、Signal、Slack、Telegram、WhatsApp、WebChat 等服務。
  </Card>
  ...
  <Card title="行動節點" icon="smartphone" href="/zh-TW/nodes">
    配對 iOS 與 Android 節點，以支援 Canvas、相機與語音工作流程。
  </Card>
</Columns>
...
<Steps>
  ...
  <Step title="聊天">
    ...
    或連接頻道（[Telegram](/zh-TW/channels/telegram) 最快），並從手機開始聊天。
...
- 如果你**不進行任何操作**，OpenClaw 會使用內建的 OpenClaw 代理程式執行環境；私訊共用代理程式的主要工作階段，而每個群組聊天都有自己的工作階段。
- 如果你想限制存取，請從 `channels.whatsapp.allowFrom` 開始，並針對群組設定提及規則。
...
<Columns>
  ...
  <Card title="頻道" href="/zh-TW/channels/telegram" icon="message-square">
    Discord、Feishu、Microsoft Teams、Telegram、WhatsApp 等服務的頻道特定設定。
  </Card>
  <Card title="節點" href="/zh-TW/nodes" icon="smartphone">
    支援配對、Canvas、相機與裝置動作的 iOS 和 Android 節點。
  </Card>
  <Card title="說明" href="/zh-TW/help" icon="life-buoy">
    常見修正方式與疑難排解入口。
  </Card>
