...
x-i18n:
    generated_at: "2026-07-26T18:19:36Z"
    model: gpt-5.6
...
- `--workdir <dir>`: Arbeitsverzeichnis (Standard: cwd; greift auf den Clawdbot-Workspace zurück, falls konfiguriert)
- `--dir <dir>`: Installationsverzeichnis unter dem Arbeitsverzeichnis (Standard: `skills`)
...
Wenn eine dieser Variablen festgelegt ist, leitet die CLI ausgehende Anfragen über
...
(z. B. Docker-Container, Hetzner-VPS mit ausschließlich über Proxy verfügbarem Internet,
...
Wenn keine Proxy-Variable festgelegt ist, bleibt das Verhalten unverändert (direkte Verbindungen).
...
- Interaktiv auf Remote-Systemen/ohne grafische Oberfläche: `clawhub login --device` gibt einen Code aus und wartet, während Sie ihn unter `<site>/cli/device` autorisieren.
...
- Fügt Ihren Lesezeichen ein Skill hinzu oder entfernt ihn daraus. Die Befehlsnamen bleiben aus Kompatibilitätsgründen `star` und
  `unstar`.
...
- Die Ausgabe enthält den Skill-Slug, den Handle des Eigentümers, den Anzeigenamen und die Relevanzbewertung.
- Die Suche bevorzugt exakte Token-Übereinstimmungen bei Slug und Name vor der Downloadpopularität. Ein eigenständiges Slug-Token wie `map` stimmt stärker mit `personal-map` überein als mit der Teilzeichenfolge in `amap`.
- Die Popularität dient nur als gering gewichtete Grundlage für die Rangfolge und garantiert keine Spitzenplatzierung.
- Wenn ein Skill erscheinen sollte, aber nicht angezeigt wird, führen Sie im angemeldeten Zustand `clawhub inspect @owner/slug` aus, um für Eigentümer sichtbare Moderationsdiagnosen zu prüfen, bevor Sie Metadaten umbenennen.
...
- `--file <path>`: Unverarbeitete Dateibytes abrufen (Limit: 10 MB).
- `--json`: Maschinenlesbare Ausgabe; `--file` enthält die exakten Bytes als Base64 sowie, sofern verfügbar, als UTF-8-Text.
...
- Verweigert das Überschreiben fixierter Skills; führen Sie zuerst `clawhub unpin <skill>` aus.
- Schreibt:
...
- Markiert einen installierten Skill im Lockfile als fixiert.
- `--reason <text>` erfasst, warum der Skill fixiert ist.
...
- Entfernt die Lockfile-Fixierung eines installierten Skills, sodass er durch künftige Aktualisierungen geändert werden kann.
...
- Wenn der Fingerabdruck nicht übereinstimmt:
  - verweigert den Vorgang standardmäßig
  - überschreibt mit `--force` (oder fragt bei interaktiver Ausführung nach)
...
- `update --all` überspringt fixierte Slugs und gibt eine Zusammenfassung der weiterhin fixierten Elemente aus.

### `skill publish <path>`

- Vergleicht den lokalen Bundle-Fingerabdruck mit ClawHub und wird erfolgreich beendet, wenn
...
- `--owner <handle>` veröffentlicht unter dem Handle einer Organisation oder eines Benutzerkontos, wenn der
  Akteur über Veröffentlichungszugriff verfügt.
- `--migrate-owner` verschiebt einen vorhandenen Skill nach `--owner`, während eine neue
  Version veröffentlicht wird. Erfordert Administrator- oder Eigentümerzugriff bei beiden Herausgebern.
- Das Eigentümer- und Prüfverhalten wird in `docs/publishing.md` erläutert.
- Die Veröffentlichung eines Skills bedeutet, dass er auf ClawHub unter `MIT-0` freigegeben wird.
- Veröffentlichte Skills dürfen ohne Namensnennung kostenlos verwendet, verändert und weitergegeben werden.
...
von ClawHub ruft `skill publish` für einen `skill_path` oder für jeden direkt unter
`root` liegenden Skill-Ordner auf (Standard: `skills`). Er überspringt unveränderte Skills und verwendet
dasselbe automatische Verhalten für Patch-Versionen.

Legen Sie `dry_run: true` fest, um eine Vorschau ohne Token anzuzeigen. Tatsächliche Veröffentlichungen erfordern das
...
- Vergleicht den Fingerabdruck jedes lokalen Skills mit ClawHub und veröffentlicht nur neue oder
  geänderte Skills.
- Neue Skills werden als `1.0.0` veröffentlicht; geänderte Skills werden standardmäßig als nächste Patch-Version
  veröffentlicht. Verwenden Sie `--bump minor|major` für Aktualisierungsbatches, die einen
  größeren SemVer-Schritt vollziehen sollen.
- `--dry-run` zeigt den Veröffentlichungsplan ohne Upload an; `--json` gibt einen
  maschinenlesbaren Plan aus.
- `--all` veröffentlicht jeden neuen oder geänderten Skill ohne Rückfrage. Ohne
...
- `sync` dient ausschließlich der einseitigen Veröffentlichung. Es installiert, aktualisiert oder lädt nichts herunter und
  meldet keine Installations- oder Downloadtelemetrie.
...
- Scans werden asynchron ausgeführt und können einige Zeit in Anspruch nehmen. Während ein Scan in der Warteschlange steht, zeigt der Terminal-Spinner seine aktuelle priorisierte Position und die Anzahl der vorausliegenden Scans an.
- Scans veröffentlichter Skills erfordern Eigentümerzugriff oder Verwaltungszugriff beim Herausgeber. Moderatoren und Administratoren können dasselbe Backend über `clawhub-admin` verwenden.
...
- `--json` gibt die vollständige Polling-Antwort für Automatisierungszwecke aus.
...
einen offiziellen wiederverwendbaren Workflow für Skill-Repositories und Katalog-Repositories bereit.
...
- Die Veröffentlichung von V1-Skills verwendet `clawhub_token`; vertrauenswürdige Veröffentlichung über GitHub OIDC ist derzeit nur für Pakete verfügbar.
...
- `--version <version>` zieht eine eigene, nicht neueste Version über eine nach dem Fail-Closed-Prinzip arbeitende,
  versionsspezifische Route zurück. Die Versionsnummer bleibt reserviert und kann nicht mit
  anderen Inhalten erneut veröffentlicht werden. Veröffentlichen Sie einen Ersatz, bevor Sie die derzeit neueste Version löschen. Das
  Plattformpersonal umgeht bei diesem ausschließlich versionsbezogenen Ablauf die Eigentümerschaft nicht.
- `--reason <text>` zeichnet eine Moderationsnotiz zur vorläufigen Löschung des gesamten Skills und im Auditprotokoll auf.
...
- Übertragungen an Organisations-/Publisher-Handles werden nur dann sofort wirksam, wenn der Akteur
  sowohl auf den aktuellen Eigentümer als auch auf den Ziel-Publisher Administratorzugriff hat.
- Unterbefehle:
...
- Durchsucht den einheitlichen Paketkatalog über `GET /api/v1/packages` und `GET /api/v1/packages/search` oder zeigt ihn an.
...
- Bei ClawPack-npm-pack-Artefakten werden zusätzlich die npm-`sha512`-Integrität,
  die npm-Prüfsumme und Name/Version aus `package.json` des Tarballs verifiziert.
- Ältere ZIP-Versionen werden über die ältere ZIP-Route heruntergeladen.
- Flags:
...
- Berechnet ClawHub SHA-256, die npm-`sha512`-Integrität und die npm-Prüfsumme für ein lokales
  Artefakt.
...
- Mit direkten Digest-Flags erfolgt die Verifizierung ohne Netzwerksuche.
- Flags:
...
- Schwerwiegende Kompatibilitätsfehler führen zu einem Exit-Code ungleich null. Befunde, die nur Warnungen darstellen, werden ausgegeben,
  führen jedoch zum Exit-Code null.
- Flags:
...
[Korrekturen für die Plugin-Validierung](/de/clawhub/plugin-validation-fixes) und führen Sie den Befehl anschließend erneut aus.

### `package delete <name>`

- Ohne `--version` werden ein Paket und alle Releases vorläufig gelöscht.
- `--version <version>` zieht ein eigenes, nicht neuestes Release über eine nach dem Fail-Closed-Prinzip arbeitende,
  versionsspezifische Route zurück. Die Versionsnummer bleibt reserviert und kann nicht mit
  anderen Inhalten erneut veröffentlicht werden. Veröffentlichen Sie einen Ersatz, bevor Sie das derzeit neueste Release löschen. Dieser
  ausschließlich versionsbezogene Ablauf erfordert den Paketeigentümer oder einen Administrator des Organisations-Publishers; das Plattformpersonal
  umgeht die Paketeigentümerschaft nicht.
- Die vorläufige Löschung des gesamten Pakets erfordert den Paketeigentümer, einen Eigentümer/Administrator des Organisations-Publishers, einen
  Plattformmoderator oder Plattformadministrator.
- Flags:
...
- Erfordert Administratorzugriff sowohl auf den aktuellen Paketeigentümer als auch auf den Ziel-
  Publisher, sofern die Übertragung nicht durch einen Plattformadministrator erfolgt.
...
- Meldungen gelten auf Paketebene, können optional mit einer Version verknüpft werden und sind
  für Moderatoren zur Prüfung sichtbar.
- Meldungen blenden Pakete nicht automatisch aus und blockieren selbst keine Downloads.
- Flags:
...
- Befehl für Eigentümer zum Prüfen der Sichtbarkeit eines Pakets für die Moderation.
...
- Zeigt den aktuellen Scanstatus des Pakets, die Anzahl offener Meldungen, den manuellen
  Moderationsstatus der neuesten Version, den Download-Sperrstatus und die Moderationsgründe.
- Flags:
  - `--json`: maschinenlesbare Ausgabe.
...
- Meldet Hindernisse hinsichtlich offiziellem Status, ClawPack-Verfügbarkeit, Artefakt-Digest,
...
- Zeigt den betreiberorientierten Migrationsstatus für ein Paket, das möglicherweise ein
...
- Erstellt einen Org-Publisher, dessen Eigentümer der authentifizierte Benutzer ist.
- Das Handle wird in Kleinbuchstaben normalisiert und kann mit oder ohne `@` übergeben werden.
- Neu erstellte Org-Publisher sind standardmäßig weder vertrauenswürdig noch offiziell.
- Schlägt fehl, wenn das Handle bereits von einem vorhandenen Publisher oder Benutzer beziehungsweise einer reservierten Route verwendet wird.
...
- `<source>` akzeptiert:
  - Lokalen Ordnerpfad: `./my-plugin`
  - Lokalen ClawPack-npm-pack-Tarball: `./my-plugin-1.2.3.tgz`
...
- Ordner mit Code-Plugins werden vor dem Upload in einen ClawPack-npm-Tarball gepackt, damit
  OpenClaw-Installationen das exakte Artefakt verifizieren können. Ordner mit Bundle-Plugins
  verwenden weiterhin den Veröffentlichungspfad für extrahierte Dateien.
- Bei GitHub-Quellen wird die Quellenangabe automatisch aus dem Repository, dem aufgelösten Commit, der Referenz und dem Unterpfad ausgefüllt.
- Bei lokalen Ordnern wird die Quellenangabe automatisch aus dem lokalen Git erkannt, wenn das Origin-Remote auf GitHub verweist.
...
- `--owner <handle>` veröffentlicht unter dem Handle eines Benutzer- oder Org-Publishers, wenn der Akteur über Publisherzugriff verfügt.
...
die Quellenangabe prüfen können, bevor Sie eine Live-Version erstellen:
...
- Der wiederverwendbare Workflow setzt `source` standardmäßig auf das aufrufende Repository.
- Übergeben Sie bei Monorepositorys `source_path`, damit der Workflow den Ordner des
  Plugin-Pakets veröffentlicht, beispielsweise `source_path: extensions/codex`.
- Fixieren Sie den wiederverwendbaren Workflow auf ein stabiles Tag oder den vollständigen Commit-SHA. Führen Sie keine Release-Veröffentlichung von `@main` aus.
...
- Fügt einem vorhandenen Paket die Konfiguration für einen vertrauenswürdigen GitHub-Actions-Publisher hinzu
  oder ersetzt sie.
- Das Paket muss zuerst durch reguläres manuelles oder tokenauthentifiziertes
  `clawhub package publish` erstellt werden.
- Nach dem Festlegen der Konfiguration können zukünftige unterstützte GitHub-Actions-Veröffentlichungen
...
- ClawHub verifiziert das konfigurierte GitHub-Repository, wenn dieser Befehl ausgeführt wird.
  Öffentliche Repositorys können anhand öffentlicher GitHub-Metadaten verifiziert werden. Bei privaten
  Repositorys muss ClawHub über GitHub-Zugriff auf dieses Repository verfügen,
...
- Flags:
  ...
  - `--workflow-filename <file>`: Name der Workflow-Datei, beispielsweise `package-publish.yml`.
...
- Entfernt die Konfiguration für den vertrauenswürdigen Publisher aus einem Paket.
...
- Zukünftige echte Veröffentlichungen müssen die reguläre authentifizierte Veröffentlichung verwenden, bis die Konfiguration
...
- Wird nach `clawhub install <slug>` gesendet, wenn eine Anmeldung besteht, sofern
