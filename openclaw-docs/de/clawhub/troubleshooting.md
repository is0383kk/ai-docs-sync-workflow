...
x-i18n:
    generated_at: "2026-07-26T18:20:28Z"
    model: gpt-5.6
...
Die CLI startet während der Browseranmeldung kurzzeitig einen lokalen Callback-Server.
...
- `Retry-After`: Sekunden, die vor einem erneuten Versuch gewartet werden müssen.
...
- Prüfen Sie den exakten Slug oder die Eigentümerseite, falls bekannt.
...
Für Eigentümer sichtbare Diagnosen können den Status der Prüfung, der Upload-Sperre oder der Moderation erläutern.

## Die Veröffentlichung schlägt wegen fehlender erforderlicher Metadaten fehl

Prüfen Sie für Skills das Frontmatter in `SKILL.md`. Erforderliche Umgebungsvariablen und
Tools sollten deklariert werden, damit Benutzer und Scanner das Paket nachvollziehen können.
...
## Die Veröffentlichung schlägt wegen eines GitHub-Eigentümer- oder Quellfehlers fehl
...
Wenn eine Veröffentlichung fehlschlägt, weil das Eigentümer-Handle, der Organisations-Namespace, der Paketbereich, der Skill-
...
müssen bereichsbezogene Namen wie `@example-org/example-plugin` unter dem
...
[Beanspruchung von Organisationen und Namespaces](/de/clawhub/namespace-claims) finden Sie Hinweise zu Nachweisen und dazu, welche
...
Verweisen Sie auf die Stammverzeichnisse, die durchsucht werden sollen:
...
- Beachten Sie die Retry-Header `429` und speichern Sie öffentliche Listen-/Suchantworten zwischen.
...
Weitere Einzelheiten zu den Endpunkten finden Sie unter [HTTP-API](/de/clawhub/http-api).
