...
title: Mistral
x-i18n:
    generated_at: "2026-07-26T18:43:57Z"
    model: gpt-5.6
...
Das gebündelte Plugin `mistral` registriert vier Verträge: Chat Completions, Medienverständnis (Voxtral-Batch-Transkription), Echtzeit-STT für Voice Call (Voxtral Realtime) und Memory Embeddings (`mistral-embed`).
...
| Voxtral-Batch     | `voxtral-mini-latest` (Audio-Transkription)    |
...
| `mistral/mistral-small-latest`   | Text, Bild  | 262,144 | 16,384           | Neueste Version von Mistral Small 4; anpassbares `reasoning_effort` |
| `mistral/mistral-small-2603`     | Text, Bild  | 262,144 | 16,384           | Fixierte Version von Mistral Small 4; anpassbares `reasoning_effort` |
...
openclaw infer model run --local \
  ...
  --prompt "Antworten Sie exakt mit: mistral-ok" \
  --json
```
...
| Codierung     | `...mistral.encoding`                                                     | `pcm_mulaw`                      |
...
<AccordionGroup>
  <Accordion title="Anpassbares Reasoning">
    `mistral/mistral-small-latest`, `mistral/mistral-small-2603` und `mistral/mistral-medium-3-5` unterstützen [anpassbares Reasoning](https://docs.mistral.ai/studio-api/conversations/reasoning/adjustable) in der Chat-Completions-API über `reasoning_effort` (`none` minimiert zusätzliches Denken in der Ausgabe; `high` gibt vor der endgültigen Antwort vollständige Denkspuren aus).
