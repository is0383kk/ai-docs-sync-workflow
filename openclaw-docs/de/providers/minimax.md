...
title: MiniMax
x-i18n:
    generated_at: "2026-07-26T18:34:48Z"
    model: gpt-5.6
...
<Tabs>
  <Tab title="OAuth (Coding Plan)">
    **Am besten geeignet für:** schnelle Einrichtung mit dem MiniMax Coding Plan über OAuth, kein API-Schlüssel erforderlich.
    ...
    <Note>
    OAuth-Einrichtungen verwenden die Provider-ID `minimax-portal`. Modellreferenzen folgen dem Format `minimax-portal/MiniMax-M3`.
    </Note>
  ...
  <Tab title="API-Schlüssel">
    **Am besten geeignet für:** gehostetes MiniMax mit Anthropic-kompatibler API.
    ...
    <Warning>
    Der Anthropic-kompatible Streaming-Endpunkt von MiniMax-M2.x gibt `reasoning_content` in Delta-Chunks im OpenAI-Stil statt in nativen Anthropic-Thinking-Blöcken aus. Dadurch gelangt internes Reasoning in die sichtbare Ausgabe, wenn Thinking implizit aktiviert bleibt. OpenClaw deaktiviert Thinking für M2.x standardmäßig, sofern Sie `thinking` nicht ausdrücklich selbst festlegen. MiniMax-M3 (und vorwärtskompatible M3.x-Versionen) ist davon ausgenommen: M3 gibt korrekte Anthropic-Thinking-Blöcke aus und benötigt aktives Thinking, um sichtbare Inhalte zu erzeugen. Daher belässt OpenClaw M3 im adaptiven Thinking-Pfad des Providers. Weitere Informationen finden Sie unten im Abschnitt „Thinking-Standardeinstellungen“ unter „Erweiterte Konfiguration“.
    </Warning>
...
Das MiniMax-Plugin registriert das Modell `image-01` für das Tool `image_generate` sowohl unter `minimax` als auch unter `minimax-portal` und verwendet dabei denselben `MINIMAX_API_KEY` oder dieselbe OAuth-Authentifizierung wie die Textmodelle.
...
- Reihenfolge der Authentifizierungsauflösung: `tts.providers.minimax.apiKey`, dann OAuth-/Token-Authentifizierungsprofile für `minimax-portal`, anschließend Token-Plan-Umgebungsschlüssel (`MINIMAX_OAUTH_TOKEN`, `MINIMAX_CODE_PLAN_KEY`, `MINIMAX_CODING_API_KEY`) und danach `MINIMAX_API_KEY`
...
- Normale Audioanhänge bleiben im MP3-Format. Für Sprachnachrichten vorgesehene Ziele (Feishu, Telegram und andere Kanäle, die einen mit Sprachnachrichten kompatiblen Anhang anfordern) werden mit `ffmpeg` von MiniMax-MP3 in Opus mit 48 kHz transkodiert, da beispielsweise die Datei-API von Feishu/Lark für native Audionachrichten ausschließlich `file_type: "opus"` akzeptiert
- MiniMax T2A akzeptiert `speed` und `vol` als Dezimalwerte, `pitch` wird jedoch als Ganzzahl gesendet; OpenClaw schneidet bei `pitch` Dezimalstellen vor der API-Anfrage ab

| Einstellung                         | Umgebungsvariable                | Standardwert                       | Beschreibung                      |
| ------------------------------- | ---------------------- | ----------------------------- | -------------------------------- |
...
| `tts.providers.minimax.voiceId` | `MINIMAX_TTS_VOICE_ID` | `English_expressive_narrator` | Für die Sprachausgabe verwendete Stimmen-ID. |
...
- Prompt-Steuerung: `lyrics`, `instrumental`
- Ausgabeformat: `mp3`
...
- Kompatibilitäts-Fallback: `MINIMAX_API_KEY`, wenn die Variable bereits auf Anmeldedaten für einen Token-Plan verweist
...
- Die Suche verwendet weiterhin die Provider-ID `minimax`; die globale bzw. CN-OAuth-Einrichtung kann die Region indirekt über `models.providers.minimax-portal.baseUrl` steuern und über `MINIMAX_OAUTH_TOKEN` eine Bearer-Authentifizierung bereitstellen
...
<AccordionGroup>
  <Accordion title="Konfigurationsoptionen">
    | Option | Beschreibung |
    | --- | --- |
    | `models.providers.minimax.baseUrl` | `https://api.minimax.io/anthropic` bevorzugen (Anthropic-kompatibel); `https://api.minimax.io/v1` ist für OpenAI-kompatible Nutzdaten optional |
    | `models.providers.minimax.api` | `anthropic-messages` bevorzugen; `openai-completions` ist für OpenAI-kompatible Nutzdaten optional |
  ...
  <Accordion title="Standardwerte für das Denken">
    Bei `api: "anthropic-messages"` fügt OpenClaw für MiniMax-M2.x-Modelle `thinking: { type: "disabled" }` ein, sofern nicht bereits ein vorheriger Wrapper das Feld `thinking` in den Nutzdaten festgelegt hat. Dadurch wird verhindert, dass der Streaming-Endpunkt von M2.x `reasoning_content` in Delta-Blöcken im OpenAI-Stil ausgibt, wodurch interne Überlegungen in der sichtbaren Ausgabe offengelegt würden.

    MiniMax-M3 (und M3.x) ist davon ausgenommen: M3 gibt bei deaktiviertem Denken ein leeres `content`-Array mit `stop_reason: "end_turn"` zurück. Daher entfernt OpenClaw für M3 den impliziten Standardwert für deaktiviertes Denken und erzwingt stattdessen `thinking: { type: "adaptive" }`, wenn eine Denkstufe festgelegt ist.
  ...
  <Accordion title="Schnellmodus">
    `/fast on` oder `params.fastMode: true` schreibt `MiniMax-M2.7` im Anthropic-kompatiblen Streaming-Pfad (`api: "anthropic-messages"`, Provider `minimax` oder `minimax-portal`) in `MiniMax-M2.7-highspeed` um.
  </Accordion>

  <Accordion title="Fallback-Beispiel">
    **Am besten geeignet für:** Das leistungsstärkste Modell der neuesten Generation als Primärmodell beibehalten und bei einem Fehler auf MiniMax M2.7 ausweichen. Das folgende Beispiel verwendet Opus als konkretes Primärmodell; ersetzen Sie es durch Ihr bevorzugtes Primärmodell der neuesten Generation.
  ...
  <Accordion title="Details zur Nutzung des Coding Plan">
    ...
    - Die Nutzungsabfrage leitet den Host aus `models.providers.minimax-portal.baseUrl` oder `models.providers.minimax.baseUrl` ab, sofern konfiguriert. Daher fragen globale Einrichtungen, die `https://api.minimax.io/anthropic` verwenden, `api.minimax.io` ab. Fehlende oder fehlerhafte Basis-URLs behalten aus Kompatibilitätsgründen den CN-Fallback bei.
    - OpenClaw normalisiert die Coding-Plan-Nutzung von MiniMax auf dieselbe `% left`-Darstellung, die auch von anderen Providern verwendet wird. Die Rohfelder `usage_percent` / `usagePercent` von MiniMax geben das verbleibende und nicht das verbrauchte Kontingent an, weshalb OpenClaw sie invertiert. Anzahlbasierte Felder haben Vorrang, wenn sie vorhanden sind.
    - Wenn die API `model_remains` zurückgibt, bevorzugt OpenClaw den Chatmodell-Eintrag, leitet bei Bedarf die Fensterbezeichnung aus `start_time` / `end_time` ab und nimmt den Namen des ausgewählten Modells in die Planbezeichnung auf, damit sich Coding-Plan-Fenster leichter unterscheiden lassen.
    - Nutzungsmomentaufnahmen behandeln `minimax`, `minimax-cn`, `minimax-portal` und `minimax-portal-cn` als dieselbe MiniMax-Kontingentoberfläche und bevorzugen gespeichertes MiniMax-OAuth, bevor auf Umgebungsvariablen für Coding-Plan-Schlüssel zurückgegriffen wird.
...
<AccordionGroup>
  <Accordion title='"Unbekanntes Modell: minimax/MiniMax-M3"'>
    Dies bedeutet in der Regel, dass der **MiniMax-Provider nicht konfiguriert ist** (kein passender Provider-Eintrag und kein MiniMax-Authentifizierungsprofil bzw. Umgebungsschlüssel gefunden). So beheben Sie das Problem:
    ...
    Stellen Sie sicher, dass bei der Modell-ID die **Groß-/Kleinschreibung beachtet** wird:
...
<CardGroup cols={2}>
  ...
  <Card title="MiniMax-Suche" href="/de/tools/minimax-search" icon="magnifying-glass">
    Websuchkonfiguration über den MiniMax Token Plan.
  </Card>
