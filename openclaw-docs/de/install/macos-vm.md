...
title: macOS-VMs
x-i18n:
    generated_at: "2026-07-26T17:54:32Z"
    model: gpt-5.6
...
- **Kleiner Linux-VPS** für einen ständig verfügbaren Gateway zu niedrigen Kosten. Siehe [VPS-Hosting](/de/vps).
- **Dedizierte Hardware** (Mac mini oder Linux-Rechner), wenn Sie vollständige Kontrolle und eine **private IP-Adresse** für die Browserautomatisierung wünschen. Viele Websites blockieren IP-Adressen von Rechenzentren, daher funktioniert lokales Browsen häufig besser.
- **Hybrid**: Betreiben Sie den Gateway auf einem günstigen VPS und verbinden Sie Ihren Mac als **Node**, wenn Sie Browser-/UI-Automatisierung benötigen. Siehe [Nodes](/de/nodes) und [Gateway aus der Ferne](/de/gateway/remote).

Verwenden Sie eine macOS-VM nur, wenn Sie ausdrücklich Funktionen benötigen, die ausschließlich unter macOS verfügbar sind, etwa iMessage, oder wenn Sie eine strikte Trennung von Ihrem täglich verwendeten Mac wünschen.
...
Wenn Sie macOS in der Cloud nutzen möchten, funktionieren auch gehostete Mac-Provider:
...
3. Schließen Sie den Systemassistenten ab und aktivieren Sie Remote Login (SSH).
4. `lume run openclaw --no-display`
5. Stellen Sie eine SSH-Verbindung her, installieren Sie OpenClaw und konfigurieren Sie die Kanäle.
6. Fertig.
...
Wenn sich `~/.local/bin` nicht in Ihrem PATH befindet:
...
## 3) Systemassistenten abschließen
...
2. Aktivieren Sie für den Betrieb der VM ohne Anzeige die automatische Anmeldung: System Settings -> Users & Groups, wählen Sie "Automatically log in as:" und anschließend den VM-Benutzer aus.
...
Die VM wird im Hintergrund ausgeführt; der Daemon von OpenClaw hält den Gateway in Betrieb. So prüfen Sie den Status:
...
3. Gewähren Sie dem Prozess, der OpenClaw/`imsg` ausführt, Festplattenvollzugriff und die Berechtigung zur Automation.
...
Starten Sie den Gateway neu. Ihr Agent kann nun iMessages senden und empfangen. Ausführliche Einrichtungsinformationen: [iMessage-Kanal](/de/channels/imessage).
...
| Problem                       | Lösung                                                                                          |
| ----------------------------- | ----------------------------------------------------------------------------------------------- |
| Keine SSH-Verbindung zur VM   | Prüfen Sie, ob "Remote Login" in den System Settings der VM aktiviert ist                        |
...
| WhatsApp-QR wird nicht erkannt | Stellen Sie sicher, dass Sie bei der Ausführung von `openclaw channels login` an der VM (nicht am Host) angemeldet sind |

## Verwandte Dokumentation

- [VPS-Hosting](/de/vps)
- [Nodes](/de/nodes)
- [Gateway aus der Ferne](/de/gateway/remote)
