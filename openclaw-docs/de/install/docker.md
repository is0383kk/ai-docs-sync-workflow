...
title: Docker
x-i18n:
    generated_at: "2026-07-26T17:52:43Z"
    model: gpt-5.6
...
Docker ist **optional**. Verwenden Sie es für eine isolierte, temporäre Gateway-Umgebung oder einen Host ohne lokale Installationen. Wenn Sie bereits auf Ihrem eigenen Rechner entwickeln, verwenden Sie stattdessen den normalen Installationsablauf.
...
Hosten Sie mehrere Benutzer? Unter [Mandantenfähiges Hosting](/de/gateway/multi-tenant-hosting) finden Sie das Modell mit einer Zelle pro Mandant.
...
- Prüfen Sie auf einem VPS/öffentlichen Host die [Sicherheitshärtung bei Netzwerkexposition](/de/gateway/security), insbesondere die Docker-Firewall-Kette `DOCKER-USER`
...
<Steps>
  ...
  <Step title="Control UI öffnen">
    Öffnen Sie `http://127.0.0.1:18789/` und fügen Sie das nach `.env` geschriebene Token unter Settings ein. Wenn Sie den Container auf Passwortauthentifizierung umgestellt haben, verwenden Sie stattdessen dieses Passwort.
...
Wenn Sie das OpenClaw-Image ersetzen, aber denselben eingebundenen Zustand und dieselbe Konfiguration beibehalten, führt das neue Gateway vor der Bereitschaft startkompatible Upgrade-Migrationen und die Plugin-Konvergenz aus. Routinemäßige Image-Upgrades sollten keinen separaten Durchlauf von `openclaw doctor --fix` erfordern.
...
| `OPENCLAW_DOCKER_BUILD_SKIP_DTS`                | Deklarationsausgabe bei lokalen, ausschließlich für die Laufzeit vorgesehenen Image-Builds überspringen (Standard: `1`) |
| `OPENCLAW_INSTALL_BROWSER`                      | Chromium und Xvfb zum Build-Zeitpunkt in das Image integrieren                                                    |
...
| `OPENCLAW_DISABLE_BUNDLED_SOURCE_OVERLAYS`      | Bind-Mount-Overlays für Quellen gebündelter Plugins deaktivieren                                                  |
...
Distribution. Diese ausschließlich für Docker vorgesehene Paketierung ändert den npm- oder ClawHub-
Artefaktvertrag des Plugins nicht. Unbekannte, ungültige oder mehrdeutige IDs lassen den Image-Build fehlschlagen.
Bekannte reine Abhängigkeits-/Quell-IDs behalten ihr bestehendes Quell- und Abhängigkeits-
Staging, ohne einen kompilierten Root-Distributionseintrag zu erhalten. Ein ausgewähltes Plugin mit
vereinheitlichten Build-Einträgen muss erfolgreich kompiliert werden; nicht ausgewählte externe Plugin-
Quellen und Runtime-Ausgaben werden entfernt.
...
bereits Teil der Root-Runtime von OpenClaw, daher wählt das ClickClack-Image nur
...
Nachweise erfordern eine Registry oder eine andere Buildx-Ausgabe, die Attestierungen bewahrt. Prüfen
Sie nach dem Push das Manifest und stellen Sie den unveränderlichen Digest statt des
veränderlichen Source-SHA-Tags bereit:
...
Die Route ist durch die Gateway-Authentifizierung geschützt; stellen Sie keinen separaten öffentlichen Port `/metrics` und keinen nicht authentifizierten Reverse-Proxy-Pfad bereit. Siehe [Prometheus-Metriken](/de/gateway/prometheus).
...
```bash
curl -fsS http://127.0.0.1:18789/healthz   # Verfügbarkeit
curl -fsS http://127.0.0.1:18789/readyz     # Bereitschaft
```

Das im Image integrierte `HEALTHCHECK` pingt `/healthz`; wiederholte Fehler markieren den Container als `unhealthy`, sodass Orchestratoren ihn neu starten oder ersetzen können.

Authentifizierte detaillierte Zustandsmomentaufnahme:
...
| Provider  | Standard-URL des Hosts   | Docker-Einrichtungs-URL             |
| --------- | ------------------------ | ----------------------------------- |
...
Das offizielle Image installiert Claude Code nicht vor. Installieren Sie es und melden Sie sich innerhalb des Container-Benutzers `node` an. Persistieren Sie anschließend das Home-Verzeichnis des Containers, damit Image-Upgrades weder die Binärdatei noch den Authentifizierungsstatus löschen.

Aktivieren Sie bei einer Neuinstallation ein persistentes `/home/node`-Volume, bevor Sie die Einrichtung ausführen:
...
Docker-Bridge-Netzwerke leiten Bonjour-/mDNS-Multicast (`224.0.0.251:5353`) üblicherweise nicht zuverlässig weiter. Wenn `OPENCLAW_DISABLE_BONJOUR` nicht festgelegt ist, deaktiviert das gebündelte Bonjour-Plugin die LAN-Ankündigung automatisch, sobald es erkennt, dass es in einem Container ausgeführt wird. Dadurch gerät es nicht in eine Absturzschleife, während es wiederholt versucht, Multicast-Daten zu senden, die von der Bridge verworfen werden. Legen Sie `OPENCLAW_DISABLE_BONJOUR=1` fest, um es unabhängig von der Erkennung zu deaktivieren, oder `0`, um es zu aktivieren (nur bei Host-Netzwerken, macvlan oder einem anderen Netzwerk, in dem mDNS-Multicast nachweislich funktioniert).

Verwenden Sie andernfalls die veröffentlichte Gateway-URL, Tailscale oder Wide-Area-DNS-SD für Docker-Hosts. Hinweise zu Fallstricken und zur Fehlerbehebung finden Sie unter [Bonjour-Erkennung](/de/gateway/bonjour).

### Speicherung und Persistenz

Docker Compose bind-mountet `OPENCLAW_CONFIG_DIR` nach `/home/node/.openclaw`, `OPENCLAW_WORKSPACE_DIR` nach `/home/node/.openclaw/workspace` und `OPENCLAW_AUTH_PROFILE_SECRET_DIR` nach `/home/node/.config/openclaw`, sodass diese Pfade einen Containeraustausch überdauern. Wenn eine Variable nicht festgelegt ist, greift `docker-compose.yml` ersatzweise auf einen Pfad unter `${HOME}` zurück oder auf `/tmp`, falls `HOME` selbst fehlt, sodass `docker compose up` in einfachen Umgebungen niemals eine Volume-Spezifikation mit leerer Quelle erzeugt.
...
Das Geheimnisverzeichnis des Authentifizierungsprofils speichert den lokalen Verschlüsselungsschlüssel für das Tokenmaterial OAuth-basierter Authentifizierungsprofile. Bewahren Sie es zusammen mit dem Zustand Ihres Docker-Hosts auf, jedoch getrennt von `OPENCLAW_CONFIG_DIR`.

Installierte herunterladbare Plugins speichern ihren Paketzustand unter dem gemounteten OpenClaw-Home-Verzeichnis, sodass Installationsdatensätze und Paket-Roots einen Containeraustausch überdauern; beim Start des Gateways werden die Abhängigkeitsbäume gebündelter Plugins nicht neu erzeugt.

Ausführliche Informationen zur vollständigen VM-Persistenz finden Sie unter [Docker-VM-Runtime – Wo welche Daten persistiert werden](/de/install/docker-vm-runtime#what-persists-where).
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI Codex OAuth (Docker ohne Benutzeroberfläche)">
...
Unter [Hetzner (Docker-VPS)](/de/install/hetzner) und [Docker-VM-Laufzeit](/de/install/docker-vm-runtime) finden Sie Schritte für die Bereitstellung auf gemeinsam genutzten VMs, einschließlich der Integration von Binärdateien, Persistenz und Aktualisierungen.
...
<AccordionGroup>
  ...
  <Accordion title="Während des Image-Builds wegen Speichermangels beendet (Exit 137)">
...
- [ClawDock](/de/install/clawdock) — Docker-Compose-Einrichtung der Community
