...
title: Verzeichnis
x-i18n:
    generated_at: "2026-07-26T18:21:47Z"
    model: gpt-5.6
...
Verzeichnissuchen für Kanäle, die sie unterstützen: Kontakte/Peers, Gruppen und „me“ (selbst).
...
Die standardmäßige Ausgabe (nicht JSON) besteht aus `id` (und manchmal `name`), getrennt durch einen Tabulator.

## Hinweise

- Bei vielen Kanälen basieren die Ergebnisse auf der Konfiguration (Zulassungslisten/konfigurierte Gruppen) und nicht auf einem Live-Provider-Verzeichnis.
...
- Ein bereits installiertes Kanal-Plugin unterstützt möglicherweise keine Verzeichnisse. In diesem Fall meldet der Befehl den nicht unterstützten Vorgang; er versucht nicht, das Plugin neu zu installieren oder zu aktualisieren, um die Unterstützung hinzuzufügen.
...
| Signal                              | Konfigurierte Aliasse werden in E.164-/UUID-Ziele für Direktnachrichten oder Gruppenziele vom Typ `group:<id>` aufgelöst |
