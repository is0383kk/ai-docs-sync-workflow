---
read_when:
    - Sie möchten eine Terminal-Benutzeroberfläche für den Gateway (für den Remote-Zugriff geeignet)
...
title: TUI
x-i18n:
    generated_at: "2026-07-26T17:44:04Z"
    model: gpt-5.6
...
| `--deliver`                  | `false`                                   | Stellt Antworten des Assistenten über konfigurierte Kanäle zu.                    |
...
- Ohne explizite URL oder expliziten Port folgt `tui` dem aktiven lokalen Gateway-Port,
...
- Beim Start aus einem konfigurierten Agent-Arbeitsbereichsverzeichnis wählt die TUI automatisch
  diesen Agent als Standardwert für den Sitzungsschlüssel aus (sofern `--session` nicht ausdrücklich
...
- Der lokale Modus verwendet die eingebettete Agent-Runtime direkt. Die meisten lokalen Tools funktionieren,
  aber Funktionen, die ausschließlich über das Gateway verfügbar sind, stehen nicht zur Verfügung.
- Der lokale Modus fügt `/auth [provider]` zur Befehlsoberfläche der TUI hinzu.
- Plugin-Genehmigungssperren gelten weiterhin im lokalen Modus: Tools, die eine Genehmigung erfordern,
...
# bei Ausführung innerhalb eines Agent-Arbeitsbereichs wird dieser Agent automatisch abgeleitet
...
Wenn `openclaw config validate` bereits fehlschlägt, führen Sie zuerst `openclaw configure` oder
