...
title: Grok-Suche
x-i18n:
    generated_at: "2026-07-26T19:16:36Z"
    model: gpt-5.6
...
OpenClaw unterstützt Grok als `web_search`-Provider und verwendet durch xAI webgestützte
Antworten, um KI-synthetisierte Antworten zu erzeugen, die auf Live-Suchergebnissen
...
`x_search`-Tool für die Suche nach Beiträgen auf X (ehemals Twitter) und dem `code_execution`-Tool.
Wenn der Schlüssel unter `plugins.entries.xai.config.webSearch.apiKey` gespeichert wird, kann
...
zur Eingabe eines separaten Websuchschlüssels aufzufordern. Ohne OAuth wird ersatzweise die Einrichtung eines xAI-API-Schlüssels verwendet.
...
- kann im selben Ablauf optional das Modell `x_search` festlegen
...
<Steps>
  ...
  <Step title="Einen API-Schlüssel als Fallback verwenden">
...
**Alternative Anmeldedaten:** `openclaw models auth login --provider xai
--method oauth`, `XAI_API_KEY` in der Gateway-Umgebung oder
`plugins.entries.xai.config.webSearch.apiKey`. Legen Sie bei einer Gateway-Installation die
Umgebungsvariablen in `~/.openclaw/.env` ab.

## Funktionsweise

Grok verwendet durch xAI webgestützte Antworten, um Antworten mit eingebetteten
Quellenangaben zu synthetisieren, ähnlich dem Grounding-Ansatz von Gemini mit der Google-Suche.

## Unterstützte Parameter

Die Grok-Suche unterstützt `query`. `count` wird aus Kompatibilitätsgründen mit der gemeinsamen `web_search`
akzeptiert, Grok gibt jedoch stets eine einzige synthetisierte Antwort mit Quellenangaben
anstelle einer Liste mit N Ergebnissen zurück. Provider-spezifische Filter werden nicht unterstützt.

Grok verwendet standardmäßig ein Zeitlimit von 60 Sekunden, da webgestützte
Suchen mit xAI Responses länger dauern können als der gemeinsame Standardwert von `web_search`. Überschreiben Sie ihn
...
sendet nach dem Entfernen abschließender Schrägstriche Anfragen an `<baseUrl>/responses`. `x_search`
greift auf denselben `webSearch.baseUrl` zurück, sofern
