...
title: SearXNG-Suche
x-i18n:
    generated_at: "2026-07-26T18:52:46Z"
    model: gpt-5.6
...
Auflösungsreihenfolge: konfigurierte `baseUrl`-Zeichenfolge, dann eine eingebettete Umgebungs-SecretRef in
...
- **JSON-API** -- verwendet den nativen `format=json`-Endpunkt von SearXNG und kein HTML-Scraping
...
- **Netzwerkschutz** -- `http://`-Basis-URLs müssen auf einen vertrauenswürdigen privaten oder
  ...
  während `https://`-Basis-URLs, die öffentlich aufgelöst werden, den strengen SSRF-Schutz beibehalten
- **Reihenfolge der automatischen Erkennung** -- SearXNG erfordert eine konfigurierte `baseUrl` (Reihenfolge
  200 unter Providern, deren erforderliche Anmeldedaten bereits vorhanden sind). Schlüsselfreie
...
- **Selbst gehostet** -- Sie kontrollieren die Instanz, die Abfragen und die vorgelagerten Suchmaschinen
- **Kategorien** verwenden standardmäßig `general`, wenn sie nicht konfiguriert sind
- **Kategorie-Fallback** -- wenn eine Kategorieanfrage außerhalb von `general` erfolgreich ist, aber
...
- **Ergebnis-Caching** -- identische Abfragen (gleiche Suchanfrage, Anzahl, Kategorien,
