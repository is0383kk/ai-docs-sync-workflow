...
read_when:
    ...
    - Sie möchten kompakte Auszüge, die nach ihrer Effizienz für den LLM-Kontext eingestuft sind.
summary: Parallele Suche -- LLM-optimierte, kompakte Auszüge aus Webquellen
title: Parallele Suche
x-i18n:
    generated_at: "2026-07-26T18:52:14Z"
    model: gpt-5.6
...
Provider bereit, die beide nach Relevanz geordnete, für LLMs optimierte Auszüge aus einem
...
<Note>
  ...
  gehostete native Websuche, wenn `tools.web.search.provider` nicht gesetzt, leer,
  `"auto"` oder `"openai"` ist -- daher umgehen sie Parallel
  standardmäßig. Setzen Sie `tools.web.search.provider` auf `parallel-free` oder
  `parallel`, um sie stattdessen über Parallel zu leiten. Siehe
...
Gateway-Umgebung. Bei einer Gateway-Installation tragen Sie es in `~/.openclaw/.env` ein.
...
Cloudflare AI Gateway). OpenClaw normalisiert reine Hostnamen, indem
...
Beide Provider stellen Parallels natives Suchformat bereit, sodass das Modell ein
natürlichsprachliches Ziel und einige kurze Schlüsselwortanfragen ausfüllt -- die Kombination,
die Parallel für optimale Ergebnisse [empfiehlt](https://docs.parallel.ai/search/best-practices).
...
Zeichen). Geben Sie für optimale Ergebnisse 2-3 unterschiedliche Anfragen an.
</ParamField>
...
`parallel-free` Search MCP begrenzt sie auf 100. Eine ID oberhalb des Grenzwerts wird verworfen
(kostenpflichtig) oder neu erzeugt (kostenlos).
</ParamField>

<ParamField path="client_model" type="string">
Optionale Kennung des Modells, das den Aufruf ausführt (z. B. `claude-opus-4-7`,
`gpt-5.6-sol`), maximal 100 Zeichen. Ermöglicht Parallel, die Standardeinstellungen an die
Fähigkeiten Ihres Modells anzupassen. Übergeben Sie den exakten Slug des aktiven Modells; kürzen Sie ihn nicht auf einen
Familienalias.
</ParamField>

## Hinweise

- Parallel ordnet und komprimiert Ergebnisse nach ihrem Nutzen für LLM-Schlussfolgerungen, nicht für
  menschliche Klicks; erwarten Sie daher kompakte Auszüge pro Ergebnis statt vollständiger
  Seiteninhalte.
- Ergebnisauszüge werden als Array `excerpts` zurückgegeben und außerdem
  für die Kompatibilität mit dem generischen Vertrag `web_search` in
  `description` zusammengeführt.
- Beide Provider geben eine `session_id` zurück; OpenClaw stellt sie als `sessionId` in
  der Tool-Nutzlast bereit, damit Aufrufer Folgesuchen gruppieren können. Eine
  von Parallel generierte Sitzungs-ID (die nicht vom Aufrufer bereitgestellt wurde) wird aus
  dem Cache-Eintrag ausgeschlossen, da nicht zusammengehörige Aufgaben mit identischen Anfragen
  sie nicht übernehmen sollten.
- `searchId`, `warnings` und `usage` von Parallel werden weitergegeben, wenn
  sie vorhanden sind.
- OpenClaw übermittelt immer eine aufgelöste Ergebnisanzahl als
  `advanced_settings.max_results` (`parallel`) an Parallel oder wendet `count`
  clientseitig nach Parallels Antwort mit fester Größe an (`parallel-free`). Das Argument
  `count` des Aufrufers hat Vorrang, danach `tools.web.search.maxResults`, andernfalls gilt
  OpenClaws generischer Standardwert `web_search` (5) -- Parallels eigener API-Standardwert
  ist 10.
- Ergebnisse werden standardmäßig 15 Minuten zwischengespeichert (`cacheTtlMinutes`).
- `parallel-free` erzeugt über seinen MCP-Handshake bei jedem Aufruf eine neue `session_id`,
