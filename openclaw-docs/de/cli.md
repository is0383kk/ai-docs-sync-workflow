...
title: CLI-Referenz
x-i18n:
    generated_at: "2026-07-26T18:52:25Z"
    model: gpt-5.6
...
die Befehle, globalen Flags und Regeln für die Ausgabegestaltung auf, die in der gesamten CLI gelten.

Einrichtungsbefehle nach Zweck:

- `openclaw setup` und `openclaw onboard` prüfen zuerst die Inferenz und starten dann OpenClaw zur Einrichtung von Gateway, Arbeitsbereich, Kanälen, Skills und Systemzustand.
...
- `openclaw configure` ändert gezielt Teile einer bestehenden Einrichtung: Modellauthentifizierung, Gateway, Kanäle, Plugins oder Skills.
- `openclaw channels add` konfiguriert Kanalkonten, nachdem die Basiskonfiguration vorhanden ist; bei alleiniger Auswahl eines Kanals wird die geführte Einrichtung verwendet, während Flags für Konto, Anmeldedaten oder Kanalkonfiguration den direkten Pfad für Skripte verwenden.
...
| Flag                    | Zweck                                                                                                 |
| ----------------------- | ------------------------------------------------------------------------------------------------------- |
| `--dev`                 | Zustand unter `~/.openclaw-dev` isolieren, den standardmäßigen Gateway-Port 19001 verwenden und abgeleitete Ports verschieben              |
...
- `--json` (und `--plain`, sofern unterstützt) deaktiviert die Gestaltung für eine bereinigte Ausgabe.
...
Diese Übersicht deckt die Kernbefehle und ihre wichtigsten Unterbefehle ab. Durch Plugins hinzugefügte
Unterbefehle (zum Beispiel unter `skills`, `plugins` und `wiki`) entwickeln sich
...
Weitere Einzelheiten finden Sie unter [Nutzungserfassung](/de/concepts/usage-tracking).
