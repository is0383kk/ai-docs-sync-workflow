...
read_when:
    ...
    - Sie installieren das Kanal-Plugin openclaw-zaloclawbot oder beheben Probleme damit
...
Wählen Sie **Zalo ClawBot** im Kanalmenü aus. Der Assistent installiert das Plugin aus dem offiziellen Katalog (integritätsgeprüft), zeigt den Anmelde-QR-Code im Terminal an und schließt die Kanaleinrichtung ab, sobald Sie ihn mit der Zalo-App scannen.
...
Verwenden Sie exakt die festgelegte Version, damit OpenClaw das Paket während der Installation anhand des Integritäts-Hashes im Katalog überprüft.
...
Scannen Sie den im Terminal angezeigten QR-Code mit der mobilen Zalo-App, akzeptieren Sie die Nutzungsbedingungen innerhalb der Zalo Mini App und autorisieren Sie die Sitzung.
...
Im Gegensatz zum standardmäßigen Zalo-Kanal, für den Sie Ihr eigenes Zalo Official Account (OA) registrieren und statische Entwickler-Anmeldedaten konfigurieren müssen, ist Zalo ClawBot ein **inhabergebundener persönlicher Assistent** auf gemeinsam genutzter offizieller Infrastruktur:
...
3. **Offizieller API-Pfad:** Das Plugin verwendet APIs der Zalo Bot Platform und keine Browser- oder Websitzungsautomatisierung.

## Technische Funktionsweise

Das Plugin kommuniziert über eine persistente Long-Polling-Schleife (`getUpdates`) mit Zalo. Webhooks sind bei lokalen Gateway-Ausführungen auf dem Desktop oder im Terminal standardmäßig deaktiviert. Nachrichten werden clientseitig verarbeitet und Ihrer lokalen Agent-Laufzeit zugeordnet.

Das Plugin verwaltet Bot-Anmeldedaten im OpenClaw-Zustandsverzeichnis. Behandeln Sie dieses Verzeichnis als vertraulich und beziehen Sie es in dieselben Richtlinien für Zugriffskontrolle und Sicherungen ein wie den übrigen OpenClaw-Zustand.

Die Laufzeit dieses Plugins befindet sich vollständig im externen Paket `@zalo-platforms/openclaw-zaloclawbot`; die nachfolgenden Verhaltensdetails, die über Installation und Konfiguration hinausgehen, entsprechen den Angaben der Plugin-Betreuer und wurden nicht anhand des OpenClaw-Core-Quellcodes überprüft.
